...
export default function GroupChats() {
  ...
  function backtodashboard() {
    window.location.href = 'dashboard.jsx';
  }
  ...
  useEffect(() => {
    ...
    let mouseX = 0, mouseY = 0, targetOffsetX = 0, targetOffsetY = 0, currentOffsetX = 0, currentOffsetY = 0;
    ...
    function drawSpaceFlight() {
      ...
      animId = requestAnimationFrame(drawSpaceFlight);
    }
    animId = requestAnimationFrame(drawSpaceFlight);
  ...
  /* Notification Long Press Delete */
  ...
  return (
    <>
      {/* GLOBAL CSS STYLES */}
      <style>{`
        :root{--bg-deep:#020308;--bg-panel:rgba(11, 17, 38, 0.75);--bg-card:rgba(18, 27, 56, 0.65);--border-glow:rgba(0, 243, 255, 0.25);--border-glow-active:rgba(0, 243, 255, 0.85);--neon-cyan:#00f3ff;--neon-purple:#b000ff;--neon-blue:#2260ff;--neon-pink:#ff007f;--text-main:#f0f4ff;--text-muted:#8a9bb8}
        *{box-sizing:border-box;margin:0;padding:0;font-family:'Segoe UI',-apple-system,BlinkMacSystemFont,Roboto,sans-serif;scrollbar-width:thin;scrollbar-color:var(--neon-cyan) var(--bg-deep)}
        ...
        .viewport-frame{position:fixed;top:0;left:0;width:100vw;height:100vh;z-index:2;pointer-events:none;box-shadow:inset 0 0 100px rgba(0, 0, 0, 0.9), inset 0 0 40px rgba(0, 243, 255, 0.15);background:radial-gradient(circle at 50% 50%, transparent 60%, rgba(2, 5, 15, 0.7) 85%, rgba(1, 2, 6, 0.95) 100%), linear-gradient(180deg, rgba(0, 243, 255, 0.03) 0%, transparent 5%, transparent 95%, rgba(0, 243, 255, 0.03) 100%)}
        .viewport-frame::before{content:'';position:absolute;top:0;left:0;right:0;bottom:0;background:linear-gradient(rgba(18, 16, 16, 0) 50%, rgba(0, 0, 0, 0.25) 50%);background-size:100% 4px;opacity:0.3;z-index:3}
        ...
        .side-nav-drawer{position:absolute;top:80px;left:-320px;width:300px;height:100%;background:rgba(8, 13, 30, 0.95);border-right:1px solid transparent;backdrop-filter:blur(20px);z-index:90;transition:left 0.4s cubic-bezier(0.16, 1, 0.3, 1);padding:80px 24px 24px 24px;display:flex;flex-direction:column;gap:20px;box-shadow:15px 0 30px rgba(0, 0, 0, 0.8)}
        ...
        .top-header{height:70px;border-bottom:1px solid var(--border-glow);display:flex;align-items:center;justify-content:space-between;padding:0 30px;background:rgba(5, 8, 20, 0.65);backdrop-filter:blur(15px);position:relative;z-index:95}
        ...
        .chat-room-title .online-status{color:#4be486}
        ...
        .chat-time-divider::before,.chat-time-divider::after{display:none}
        ...
        .message-bubble.incoming::before{content:"";position:absolute;left:-5px;bottom:0;width:10px;height:12px;background:#1a273d;clip-path:polygon(100% 0, 100% 100%, 0 100%)}
        ...
        .message-bubble.outgoing::after{content:"";position:absolute;right:-5px;bottom:0;width:10px;height:12px;background:#113d52;clip-path:polygon(0 0, 100% 100%, 0 100%)}
        ...
        .message-text{color:#e8f1fa;white-space:pre-wrap}
        .message-meta{display:flex;justify-content:flex-end;align-items:center;gap:4px;margin-top:2px;color:rgba(190, 211, 230, 0.58);font-size:0.62rem;line-height:1}
        .message-bubble.outgoing .message-meta{color:rgba(179, 230, 238, 0.65)}
        .message-checks{color:#55dceb;font-size:0.72rem;letter-spacing:-3px}
        ...
        .chat-attach-button{width:40px;height:40px;flex:0 0 40px;display:flex;align-items:center;justify-content:center;border:none;background:transparent;color:#7d91a8;border-radius:50%;cursor:pointer;transition:color 0.2s ease, background 0.2s ease, transform 0.2s ease}
        .chat-attach-button:hover{color:#d9e8f6;background:rgba(255, 255, 255, 0.06)}
        .chat-attach-button:active{transform:scale(0.9)}
        ...
        .chat-input-bar button svg{width:18px;height:18px;stroke:currentColor;fill:none;stroke-width:2.2;stroke-linecap:round;stroke-linejoin:round}
        .chat-new-message-button{position:absolute;right:18px;bottom:78px;z-index:15;width:42px;height:42px;display:flex;align-items:center;justify-content:center;border-radius:50%;border:1px solid rgba(255, 255, 255, 0.08);background:rgba(19, 34, 55, 0.94);color:#8bdce8;box-shadow:0 8px 25px rgba(0, 0, 0, 0.35);backdrop-filter:blur(15px);cursor:pointer;animation:floatingButtonIn 0.25s ease-out}
        .chat-new-message-button:hover{background:rgba(27, 48, 73, 0.98);color:#c8f6fb}
        @keyframes messageAppear{from{opacity:0;transform:translateY(5px) scale(0.985)}to{opacity:1;transform:translateY(0) scale(1)}}
        @keyframes floatingButtonIn{from{opacity:0;transform:translateY(8px) scale(0.85)}to{opacity:1;transform:translateY(0) scale(1)}}
        @media (max-width: 700px){
          .chat-room-container{width:100%;border:none;border-radius:0}
          .chat-room-header{min-height:60px;padding:7px 10px}
          .chat-room-title-area{gap:9px}
          .chat-room-title-area .chat-avatar{width:40px;height:40px;flex-basis:40px}
          .chat-room-title h3{font-size:0.9rem}
          .chat-room-title span{font-size:0.68rem}
          .chat-room-actions button,.chat-room-actions svg{width:34px;height:34px}
          .chat-messages-area{padding:14px 9px 90px;gap:5px}
          .message-bubble{max-width:84%;padding:7px 10px 6px;font-size:0.87rem;border-radius:12px}
          .chat-input-floating-wrapper{padding:7px 7px calc(7px + env(safe-area-inset-bottom))}
          .chat-input-bar{border-radius:19px;padding:4px}
          .chat-input-bar textarea{min-height:39px;padding:9px 8px}
          .chat-input-bar button{width:38px;height:38px;flex-basis:38px}
          .chat-attach-button{width:38px;height:38px;flex-basis:38px}
          .chat-new-message-button{right:12px;bottom:70px;width:38px;height:38px}
        }
        @media (max-width: 380px){
          .chat-room-actions button:nth-child(n+2){display:none}
          .message-bubble{max-width:89%}
          .chat-room-header{padding-left:7px;padding-right:7px}
        }
        @media (prefers-reduced-motion: reduce){
          .message-bubble,.chat-new-message-button{animation:none}
          .chat-messages-area{scroll-behavior:auto}
          .chat-input-bar button,.chat-room-actions button{transition:none}
        }
        ...
        .options-dots{color:var(--text-muted);font-size:1.2rem;letter-spacing:2px;padding:6px;transition:color 0.2s}
        ...
        .decorated-prompt-box::before{content:'';position:absolute;top:0;left:0;width:4px;height:100%;background:linear-gradient(180deg, var(--neon-cyan), var(--neon-purple))}
        ...
        <main className="workspace">
          ...
          <header className="top-header">
            ...
            <div className="top-header-right">
              ...
              {/* Three Dots in app bar opens Group Chats page description & contains button back to main dashboard */}
      ...
      <div className={`overlay-screen ${modals.groupOverviewModal ? 'active' : ''}`} id="groupOverviewModal">
        <div className="modal-box" style={{ textAlign: 'center' }}>
          ...
          <div className="modal-body" style={{ alignItems: 'center' }}>
            ...
            <button className="primary-action-btn" onClick={backtodashboard}>Exit This Page</button>
          </div>
        </div>
      </div>
