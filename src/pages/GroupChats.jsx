    ...
    <>
      ...
      <style>{`
        ...
        :root{
          ...
          /* Ultra 3D Cyber Token System */
          --3d-card-bg: linear-gradient(145deg, rgba(14, 28, 46, 0.9), rgba(5, 12, 22, 0.95));
          --3d-card-border: 1px solid rgba(0, 240, 255, 0.25);
          --3d-card-shadow: 0 12px 30px rgba(0, 0, 0, 0.85), inset 0 1px 0 rgba(0, 240, 255, 0.35), 0 0 15px rgba(0, 240, 255, 0.1);
          
          --3d-button-bg: linear-gradient(180deg, #00f0ff 0%, #0077c8 100%);
          --3d-button-shadow: 0 4px 0 #004488, 0 8px 18px rgba(0, 240, 255, 0.4);
        ...
        *{box-sizing:border-box;margin:0;padding:0;font-family:var(--font-body);scrollbar-width:thin;scrollbar-color:var(--tg-accent) var(--bg-deep)}
        ...
        .icon-3d {
          ...
          align-items: center;
          justify-content: center;
          filter: drop-shadow(0 4px 6px rgba(0, 240, 255, 0.35));
        ...
        .side-nav-header h3{font-family:var(--font-cyber);font-size:var(--fs-sm);letter-spacing:0.12em;color:var(--tg-accent);font-weight:900;text-shadow:0 0 8px var(--tg-accent)}
        ...
        .side-nav-item{
          ...
          cursor:pointer;transition:all 0.15s ease;font-size:var(--fs-sm);font-weight:700;letter-spacing:0.05em;
        ...
        .top-header{
          height:65px;border-bottom:1px solid rgba(0, 240, 255, 0.2);
          display:flex;align-items:center;justify-content:space-between;padding:0 18px;
          background:rgba(5, 12, 22, 0.95);backdrop-filter:blur(20px);position:relative;z-index:95;
        ...
        .icon-action-btn{
          width:40px;height:40px;border-radius:10px;background:var(--3d-btn-dark);
        ...
        .brand-title-area{display:flex;align-items:center;gap:10px}
        .brand-title-area h1{
          font-family:var(--font-cyber);font-size:var(--fs-lg);font-weight:900;letter-spacing:0.1em;text-transform:uppercase;
          color:#fff;text-shadow:0 0 12px rgba(0, 240, 255, 0.6);
        }
        ...
        .avatar{
          ...
          border:2px solid var(--tg-accent);box-shadow:0 0 14px rgba(0, 240, 255, 0.6);
          cursor:pointer;pointer-events:auto;
        }
        .avatar img{width:100%;height:100%;object-fit:cover}
        
        /* Action Bar */
        .action-bar-section{display:flex;align-items:center;justify-content:space-between;padding:14px 18px 8px 18px;gap:12px;background:transparent}
        .search-box{position:relative;flex:1;max-width:380px}
        .search-box input{
          width:100%;background:rgba(14, 28, 46, 0.75);border:1px solid rgba(0, 240, 255, 0.3);
          border-radius:12px;padding:9px 14px 9px 38px;color:var(--text-main);font-size:var(--fs-sm);
          outline:none;box-shadow:inset 0 2px 6px rgba(0,0,0,0.8);pointer-events:auto;
        }
        .search-box input:focus{border-color:var(--tg-accent);box-shadow:0 0 12px rgba(0,240,255,0.4)}
        .search-box svg{position:absolute;left:12px;top:50%;transform:translateY(-50%);width:16px;height:16px;stroke:var(--tg-accent);fill:none;stroke-width:2.5}
        .action-bar-right{display:flex;align-items:center;gap:10px}
        .filter-btn{padding:8px 14px;font-size:var(--fs-xs)}
        .create-group-btn{padding:8px 16px;font-size:var(--fs-xs)}
        
        /* Main Workspace Container */
        .main-content{flex:1;overflow-y:auto;padding:8px 18px 24px 18px;display:flex;flex-direction:column;gap:16px;margin:0 auto;width:100%;max-width:1200px}
        ...
        .section-title{font-family:var(--font-cyber);font-size:var(--fs-xs);letter-spacing:0.18em;text-transform:uppercase;color:var(--tg-accent);margin-bottom:6px;font-weight:900;text-shadow:0 0 8px rgba(0, 240, 255, 0.4)}
        ...
        /* Group Tabs */
        .group-tabs{display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid rgba(0, 240, 255, 0.15);padding-bottom:10px}
        .tab-buttons{display:flex;gap:10px;overflow-x:auto;padding-bottom:2px}
        .tab-btn{padding:8px 14px;font-size:var(--fs-xs);height:34px;white-space:nowrap;pointer-events:auto}
        ...
        .chat-room-header{
          position:relative;height:65px;padding:8px 18px;display:flex;align-items:center;
          justify-content:space-between;background:rgba(8, 20, 34, 0.95);
          border-bottom:1px solid rgba(0, 240, 255, 0.2);z-index:10;
          box-shadow:0 4px 15px rgba(0,0,0,0.6);flex-shrink:0;
        }
        .chat-room-title-area{min-width:0;display:flex;align-items:center;gap:12px;cursor:pointer;user-select:none;pointer-events:auto}
        .chat-room-title-area .chat-avatar{
          position:relative;width:40px;height:40px;flex:0 0 40px;display:flex;
          align-items:center;justify-content:center;border-radius:50%;
          background:linear-gradient(135deg, var(--tg-accent), #0066cc);
          color:#fff;font-family:var(--font-cyber);font-weight:900;font-size:var(--fs-sm);border:1px solid rgba(255, 255, 255, 0.3);
          box-shadow:0 0 10px rgba(0, 240, 255, 0.4);
        }
        .chat-room-title-area .chat-avatar::after{
          content:"";position:absolute;width:9px;height:9px;right:1px;bottom:1px;
          border-radius:50%;background:var(--tg-online);border:2px solid var(--tg-dark-panel);
          box-shadow:0 0 8px var(--tg-online);
        }
        
        .chat-messages-area{
          position:relative;flex:1;min-height:0;padding:16px clamp(12px, 3vw, 24px) 100px;
          overflow-y:auto;overscroll-behavior:contain;display:flex;flex-direction:column;gap:12px;background:var(--bg-deep);
        }
        .chat-time-divider{
          position:relative;z-index:1;display:flex;align-items:center;justify-content:center;
          width:fit-content;margin:12px auto 8px;padding:5px 14px;color:var(--tg-accent);
          font-size:var(--fs-xs);font-weight:800;background:var(--3d-card-bg);border:var(--3d-card-border);
          box-shadow:var(--3d-card-shadow);border-radius:8px;letter-spacing:0.08em;font-family:var(--font-cyber);
        }
        
        /* 3D Dynamic Message Bubbles */
        .message-bubble{
          position:relative;z-index:1;width:fit-content;max-width:min(82%, 520px);
          padding:12px 16px;color:#fff;font-size:var(--fs-sm);line-height:1.45;border-radius:16px;
          box-shadow:0 8px 20px rgba(0,0,0,0.6);word-wrap:break-word;pointer-events:auto;
        }
        ...
        .message-sender{margin-bottom:3px;color:var(--tg-accent);font-family:var(--font-cyber);font-size:var(--fs-xs);font-weight:800;cursor:pointer}
        ...
        .chat-input-bar{
          width:min(100%, 820px);margin:0 auto;display:flex;align-items:flex-end;gap:8px;
          padding:8px 12px;background:var(--tg-dark-panel);border:var(--3d-card-border);
          border-radius:22px;box-shadow:0 0 25px rgba(0, 240, 255, 0.3);
        }
        .chat-input-bar textarea{
          flex:1;width:100%;min-height:38px;max-height:120px;padding:8px;
          background:transparent;border:none;outline:none;resize:none;color:#fff;
          font-family:inherit;font-size:var(--fs-sm);line-height:1.35;pointer-events:auto;
        }
        
        .voice-btn,.send-btn{width:38px;height:38px;flex:0 0 38px;border-radius:50%;pointer-events:auto}
        .voice-btn.listening{background:rgba(255, 42, 85, 0.2);color:var(--tg-destructive);border:1px solid var(--tg-destructive);box-shadow:0 0 12px var(--tg-destructive)}
        ...
        .futuristic-banner-prompt {
          background: linear-gradient(135deg, rgba(8, 20, 34, 0.95), rgba(0, 240, 255, 0.08));
          border: 1px solid var(--tg-accent);
          box-shadow: 0 0 25px rgba(0, 240, 255, 0.3), inset 0 0 10px rgba(0, 240, 255, 0.15);
          border-radius: 16px;
          padding: 14px 18px;
          margin-bottom: 14px;
          display: flex;
        ...
        @keyframes promptGlow {
          ...
          100% { border-color: rgba(0, 240, 255, 0.9); box-shadow: 0 0 30px rgba(0, 240, 255, 0.4); }
        }

        /* Error Notification Banner */
        .error-notification-banner{
          display:none;background:var(--3d-card-bg);border:1px solid var(--tg-destructive);
          border-radius:12px;padding:12px 16px;margin-bottom:12px;align-items:center;
          justify-content:space-between;box-shadow:0 0 20px rgba(255, 42, 85, 0.35);pointer-events:auto;
        }
        .error-notification-banner.active{display:flex}
        .error-content-wrapper{display:flex;align-items:center;gap:12px}
        .error-icon-box{width:30px;height:30px;background:rgba(255, 42, 85, 0.25);border-radius:50%;display:flex;align-items:center;justify-content:center;color:var(--tg-destructive);font-weight:bold;font-size:var(--fs-sm);flex-shrink:0}
        ...
        .group-item{
          ...
          cursor:pointer;transition:all 0.25s cubic-bezier(0.16, 1, 0.3, 1);user-select:none;pointer-events:auto;
          transform-style:preserve-3d;
        }
        .group-item.pinned-item{border-color:var(--tg-accent);background:linear-gradient(135deg, rgba(0, 240, 255, 0.12), rgba(14, 28, 46, 0.95))}
        .group-item:hover{transform:translateY(-3px) scale(1.01);border-color:var(--tg-accent);box-shadow:0 15px 30px rgba(0, 240, 255, 0.3)}
        ...
        .group-item-avatar{
          ...
          display:flex;align-items:center;justify-content:center;font-size:var(--fs-sm);font-weight:900;
          color:#fff;font-family:var(--font-cyber);overflow:hidden;flex-shrink:0;border:2px solid rgba(255, 255, 255, 0.3);
          box-shadow:0 0 12px rgba(0, 240, 255, 0.5);
        }
        .group-item-avatar img{width:100%;height:100%;object-fit:cover;border-radius:50%}
        .group-item-info h4{font-size:var(--fs-base);color:#fff;margin-bottom:3px;font-weight:800;letter-spacing:0.04em;display:flex;align-items:center;gap:8px}
        .group-item-info p{font-size:var(--fs-xs);color:var(--text-muted)}
        .group-item-right{display:flex;align-items:center;gap:12px}
        .unread-badge{
          background:var(--tg-accent);color:#000;font-size:var(--fs-xs);font-weight:900;
          padding:3px 10px;border-radius:12px;box-shadow:0 0 12px var(--tg-accent);font-family:var(--font-cyber);
        }

        /* Modals & Cyber Slide-in Overlays */
        .overlay-screen{
          position:fixed;top:0;left:0;width:100vw;height:100vh;
          background:rgba(2, 5, 10, 0.88);backdrop-filter:blur(14px);z-index:180;
        ...
        .modal-box{
          ...
          box-shadow:0 20px 60px rgba(0, 240, 255, 0.3);border-radius:20px;
        ...
        .modal-header{padding:18px 22px;border-bottom:1px solid rgba(0, 240, 255, 0.2);display:flex;align-items:center;justify-content:space-between}
        ...
        .decorated-prompt-box{
          background:var(--3d-card-bg);border:var(--3d-card-border);box-shadow:var(--3d-card-shadow);
          border-radius:16px;padding:20px;position:relative;pointer-events:auto;
        }
        ...
        .tag-admin{background:linear-gradient(135deg, #ff2a55, #a80024);box-shadow:0 0 10px rgba(255, 42, 85, 0.5)}
        .tag-member{background:linear-gradient(135deg, var(--tg-accent), #0077c8);color:#000;box-shadow:0 0 10px rgba(0, 240, 255, 0.5)}
        ...
        .form-group input[type="text"],.form-group textarea,.form-group select,.form-group input[type="number"]{
          background:rgba(14, 28, 46, 0.85);border:1px solid rgba(0, 240, 255, 0.3);
          border-radius:12px;padding:12px 14px;color:#fff;font-size:var(--fs-sm);outline:none;
          box-shadow:inset 0 2px 4px rgba(0,0,0,0.6);pointer-events:auto;
        }
        .form-group input:focus,.form-group textarea:focus,.form-group select:focus{border-color:var(--tg-accent);box-shadow:0 0 10px rgba(0, 240, 255, 0.3)}
        
        .radio-options{display:flex;flex-direction:column;gap:10px}
        .radio-card{background:var(--3d-card-bg);border:1px solid rgba(0, 240, 255, 0.2);border-radius:14px;padding:14px;display:flex;align-items:center;gap:12px;cursor:pointer;pointer-events:auto}
        .radio-card.selected{border-color:var(--tg-accent);background:rgba(0, 240, 255, 0.12);box-shadow:0 0 15px rgba(0, 240, 255, 0.25)}
        ...
        .notification-item-card{
          ...
          gap:12px;cursor:pointer;pointer-events:auto;transition:transform 0.2s ease;
        }
        .notification-item-card:hover{transform:translateY(-2px)}
        .notification-left-details{display:flex;align-items:center;gap:12px}
        .notification-icon-3d{width:38px;height:38px;border-radius:12px;background:rgba(0, 240, 255, 0.15);border:1px solid rgba(0, 240, 255, 0.4);display:flex;align-items:center;justify-content:center;color:var(--tg-accent);font-size:var(--fs-base);flex-shrink:0}
        ...
        @media (max-width: 768px) {
          ...
          .main-content { padding: 8px 12px 20px 12px; }
        ...
        <main className="workspace">
          ...
          <header className="top-header">
            <div className="top-header-left">
              ...
              <div className="brand-title-area">
                ...
                <div className="icon-3d" style={{ width: '32px', height: '32px' }}>
            ...
            <div className="top-header-right">
              ...
              <button className="icon-action-btn" title="Group Description & Navigation" onClick={() => openModal('groupOverviewModal')}>
                <span className="icon-3d">ℹ️</span>
              </button>
          ...
          <div className="action-bar-section">
            ...
            <div className="action-bar-right">
              ...
              <button className="btn-3d btn-3d-primary create-group-btn" onClick={() => openModal('createGroupModal')}>
                <span className="icon-3d" style={{ marginRight: '4px' }}>✨</span> + Create
              </button>
            </div>
          </div>
          ...
          <div className="main-content" id="mainContentArea">
            ...
            <div className={`view-section ${activeMainView === 'chats' ? 'active' : ''}`} id="view-chats">
              ...
              <div className="futuristic-banner-prompt">
                <div className="flex-row" style={{ gap: '12px' }}>
                  <div className="icon-3d" style={{ fontSize: '1.5rem' }}>🚀</div>
                  <div>
                ...
                <button className="btn-3d btn-3d-primary" style={{ padding: '6px 12px', fontSize: 'var(--fs-xs)' }} onClick={() => openModal('createGroupModal')}>EXPLORE</button>
              </div>
              ...
              <div>
                ...
                <div className="group-list" id="groupListContainer" style={{ marginTop: '14px' }}>
                    ...
                    return (
                      ...
                      >
                        <div className="group-item-left">
                          ...
                          <div className="group-item-info">
                            <h4>
                              {g.name} <span className="icon-3d" style={{ fontSize: '0.9rem' }}>📌</span>
                            </h4>
                            <p>{g.description || ''}</p>
                          </div>
                        </div>
                  ...
                  {normalGroupsList.length === 0 && pinnedGroupsList.length === 0 ? (
                    <p style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-muted)', padding: '12px 0' }}>No active channels available in this section.</p>
                  ) : (
            ...
            <div className={`view-section ${activeMainView === 'userHub' ? 'active' : ''}`} id="view-userHub">
              ...
              <div style={{ background: 'var(--3d-card-bg)', border: 'var(--3d-card-border)', boxShadow: 'var(--3d-card-shadow)', borderRadius: '18px', padding: '20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
                <div className="flex-row" style={{ gap: '16px' }}>
                  <div className="avatar" style={{ width: '60px', height: '60px' }} id="hubUserAvatarLarge">
                ...
                <button className="btn-3d btn-3d-dark" style={{ padding: '8px 16px', fontSize: 'var(--fs-xs)' }} onClick={() => openModal('editProfileModal')}>Edit Profile</button>
              </div>
            ...
            <div className={`view-section ${activeMainView === 'predictions' ? 'active' : ''}`} id="view-predictions">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                ...
                <button className="btn-3d btn-3d-danger" style={{ padding: '6px 14px', fontSize: 'var(--fs-xs)' }} onClick={promptClearAllNotifications}>Clear All</button>
              </div>
                ...
                {notifications.length === 0 ? (
                  <div className="notification-item-card">
                    <div className="notification-left-details">
                      <div className="notification-icon-3d"><span className="icon-3d">🔔</span></div>
                      <div>
                    ...
                    >
                      <div className="notification-left-details">
                        ...
                        <div>
                          ...
                          <span style={{ fontSize: '0.65rem', color: 'var(--tg-accent)' }}>{n.time}</span>
                        </div>
                      </div>
                    </div>
                  ))
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '4px', gap: '8px' }}>
                      ...
                      {isOutgoing && (
                        <div className="flex-row" style={{ gap: '4px' }}>
                          <span style={{ cursor: 'pointer', fontSize: '0.7rem' }} onClick={() => editMessage(m.id, m.content)}>✏️</span>
                          <span style={{ cursor: 'pointer', fontSize: '0.7rem' }} onClick={() => deleteMessage(m.id)}>🗑️</span>
                        </div>
                      )}
                    </div>
                  </div>
      ...
      <div className={`overlay-screen ${modals.createGroupModal ? 'active' : ''}`}>
        <div className="modal-box">
          ...
          <div className="modal-body">
            ...
            <button className="btn-3d btn-3d-primary" style={{ padding: '12px', width: '100%', marginTop: '8px' }} onClick={createNewGroupSubmit}>
              INITIALIZE CHANNEL
            </button>
          </div>
        </div>
            ...
            {currentOpenGroup && (
              <>
                <div className="decorated-prompt-box">
                  <h3 style={{ fontSize: 'var(--fs-base)', color: '#fff', fontFamily: 'var(--font-cyber)', marginBottom: '4px' }}>{currentOpenGroup.name}</h3>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  {isMember(currentOpenGroup, currentUser?.id) ? (
                    <>
                      <button className="btn-3d btn-3d-dark" style={{ flex: 1, padding: '10px' }} onClick={() => inviteMember(currentOpenGroup.id)}>Invite Member</button>
                      <button className="btn-3d btn-3d-danger" style={{ flex: 1, padding: '10px' }} onClick={() => exitGroup(currentOpenGroup.id)}>Leave Group</button>
                    </>
                  ) : (
                    <button className="btn-3d btn-3d-primary" style={{ width: '100%', padding: '10px' }} onClick={() => joinGroup(currentOpenGroup.id)}>
                      JOIN GROUP CHANNEL
                    </button>
                  )}
                </div>
                ...
                {isGroupAdmin(currentOpenGroup, currentUser?.id) && (
                  <button className="btn-3d btn-3d-danger" style={{ width: '100%', padding: '10px' }} onClick={() => deleteGroup(currentOpenGroup.id)}>
                    TERMINATE CHANNEL
                  </button>
                )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '200px', overflowY: 'auto' }}>
                  {groupAboutMembers.map(m => (
                    <div key={m.user_id} className="notification-item-card" style={{ padding: '10px' }}>
                      <div className="flex-row">
              ...
              pendingApprovalGroups.map(g => (
                <div key={g.id} className="notification-item-card">
                  ...
                  {currentProfile?.is_global_admin && (
                    <button className="btn-3d btn-3d-primary" style={{ padding: '6px 12px', fontSize: 'var(--fs-xs)' }} onClick={() => approveGroup(g.id)}>Approve</button>
                  )}
                </div>
              ))
            )}
      ...
      <div className={`overlay-screen ${modals.editProfileModal ? 'active' : ''}`}>
        <div className="modal-box">
          ...
          <div className="modal-body">
            ...
            <button className="btn-3d btn-3d-primary" style={{ padding: '12px', width: '100%', marginTop: '8px' }} onClick={saveProfileChanges}>
      ...
      <div className={`overlay-screen ${modals.groupOverviewModal ? 'active' : ''}`}>
        <div className="modal-box">
          ...
          <div className="modal-body">
            <div className="decorated-prompt-box">
              <h4 style={{ color: 'var(--tg-accent)', fontSize: 'var(--fs-sm)', fontFamily: 'var(--font-cyber)', marginBottom: '6px' }}>SYSTEM ARCHITECTURE</h4>
              <p style={{ fontSize: 'var(--fs-xs)', color: '#fff', lineHeight: '1.5' }}>
            ...
            <div className="modal-body">
              ...
              {customPrompt.type === 'input' && (
                <input
                  ...
                  style={{ background: 'rgba(14, 28, 46, 0.9)', border: '1px solid rgba(0,240,255,0.4)', padding: '10px', borderRadius: '10px', color: '#fff' }}
                />
              )}
              ...
              {customPrompt.type === 'textarea' && (
                <textarea
                  id="customPromptInput"
                  defaultValue={customPrompt.defaultValue || ''}
                  style={{ background: 'rgba(14, 28, 46, 0.9)', border: '1px solid rgba(0,240,255,0.4)', padding: '10px', borderRadius: '10px', color: '#fff', minHeight: '80px' }}
                />
              )}

              <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                <button className="btn-3d btn-3d-dark" style={{ flex: 1, padding: '10px' }} onClick={() => setCustomPrompt(null)}>Cancel</button>
                <button
                  className="btn-3d"
                  style={{ flex: 1, padding: '10px', background: customPrompt.confirmBg || 'var(--3d-button-bg)', color: customPrompt.confirmColor || '#fff' }}
                  onClick={() => {
