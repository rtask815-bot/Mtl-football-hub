...
export default function GroupChats() {
  ...
  function handleUserClick(targetUserId, targetUsername) {
    ...
    setCustomPrompt({
      ...
      confirmColor: '#fff',
      onConfirm: async () => {
        const reqMessage = `CHAT REQUEST from ${currentProfile?.username || 'User'}: "I would like to start a private discussion."`;
  ...
  return (
    <>
      {/* GLOBAL CSS STYLES WITH COMPACT COMPOSITIONS, 3D CONTAINERS & 3D BUTTONS */}
      <style>{`
        :root{
          --bg-deep:#0b1118;
          --bg-panel:#16222f;
        ...
        *{box-sizing:border-box;margin:0;padding:0;font-family:'Segoe UI',-apple-system,BlinkMacSystemFont,Roboto,sans-serif;scrollbar-width:thin;scrollbar-color:var(--tg-accent) var(--bg-deep)}
        ...
        /* Main Container with Margins to Avoid Screen Overflow */
        ...
        /* 3D Pinned Grid & Compact Containers */
        .pinned-grid{display:grid;grid-template-columns:repeat(auto-fit, minmax(220px, 1fr));gap:12px}
        .pinned-card{background:var(--3d-card-bg);border:var(--3d-card-border);box-shadow:var(--3d-card-shadow);border-radius:14px;padding:14px;cursor:pointer;position:relative;overflow:hidden;transition:all 0.15s ease;display:flex;flex-direction:column;gap:8px}
        .pinned-card:hover{transform:translateY(-2px);border-color:rgba(51, 144, 236, 0.5)}
        .pinned-card-header{display:flex;align-items:center;justify-content:space-between}
        .pinned-card-icon{width:32px;height:32px;border-radius:8px;background:rgba(51, 144, 236, 0.2);border:1px solid rgba(51, 144, 236, 0.4);display:flex;align-items:center;justify-content:center;color:var(--tg-accent);font-weight:bold;font-size:0.75rem}
        .pinned-card h3{font-size:0.9rem;color:#fff;font-weight:700}
        .pinned-card p{font-size:0.72rem;color:var(--text-muted);line-height:1.3}
        .pinned-card-footer{font-size:0.7rem;color:var(--tg-accent);display:flex;align-items:center;justify-content:space-between;font-weight:700;margin-top:auto}
        
        .section-divider-solid{height:1px;width:100%;margin:4px 0;background:rgba(255, 255, 255, 0.08)}
                  ...
                  {bigThreeGroups.map((g, index) => (
                    <div className="pinned-card" key={g.id} onClick={() => openChatRoom(g.id)}>
                      <div className="pinned-card-header">
                        <div className="pinned-card-icon">#{index + 1}</div>
                        <div className="solid-animated-tag tag-admin">PINNED</div>
                      </div>
                        ...
                        >
                          <div className="group-item-left">
                            ...
                            <div className="group-item-info">
                              <h4>
                                {g.name} {g.is_big_three ? <span style={{ color: 'var(--tg-accent)' }}>📌</span> : ''} {!g.is_approved ? <span className="solid-animated-tag tag-approval" style={{ fontSize: '0.55rem' }}>Pending Approval</span> : ''}
                              </h4>
                              <p>{g.description || ''}</p>
                            </div>
                          </div>
              ...
              if (!isMemberUser && !currentProfile?.is_global_admin && group.creator_id !== currentUser?.id) {
                return (
                  <>
                    ...
                    {/* Join Group Prompt (Disappears permanently once clicked/joined) */}
