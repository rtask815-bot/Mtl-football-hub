...
export default function GroupChats() {
  ...
  // Prevent background scrolling when any modal/chat overlay is open
  useEffect(() => {
    const isAnyOverlayActive = Object.values(modals).some(Boolean) || isSideNavOpen || customPrompt !== null;
  ...
  }, [modals, isSideNavOpen, customPrompt]);
  ...
  return (
    <>
                  ...
                  notifications.map(n => (
                    ...
                    >
                      <div className="notification-left-details">
                        <div className="notification-icon-3d">⚡</div>
                        <div>
                          <h4 style={{ fontSize: '0.82rem', color: '#fff', marginBottom: '2px' }}>{n.title}</h4>
                          <p style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>{n.message}</p>
                        </div>
                      </div>
                      <span style={{ fontSize: '0.62rem', color: 'var(--tg-accent)', fontWeight: 'bold' }}>{n.time}</span>
                    </div>
                  ))
                )}
              </div>
      ...
      {/* ============================================================
         FLOATING OVERLAY LAYER: CHAT ROOM CONTAINER
         ============================================================ */}
      {modals.chatRoomModal && currentOpenGroup && (
        <div className="chat-room-container" ref={chatContainerRef}>
          <div className="chat-room-header">
            <div className="flex-row" style={{ gap: '10px' }}>
              <button className="icon-action-btn" title="Exit Channel" onClick={() => closeModal('chatRoomModal')}>
                &larr;
              </button>
              <div className="chat-room-title-area" onClick={() => openGroupAbout(currentOpenGroup.id)}>
                ...
                <div>
                  <h3 style={{ fontSize: '0.88rem', color: '#fff', fontWeight: '800' }}>{currentOpenGroup.name}</h3>
                  <p style={{ fontSize: '0.65rem', color: 'var(--tg-online)', fontWeight: 'bold' }}>● ONLINE MATRIX CHANNEL</p>
                </div>
              </div>
            </div>
            <button className="icon-action-btn" title="Group Details" onClick={() => openGroupAbout(currentOpenGroup.id)}>
              ⚙️
            </button>
          </div>
          ...
          <div className="chat-messages-area" ref={chatMessagesAreaRef}>
            {messages.length === 0 ? (
              <div style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.75rem', marginTop: 'auto', marginBottom: 'auto' }}>
                Channel initialized. Transmit first data packet below.
              </div>
            ) : (
              messages.map((msg, index) => {
                ...
                // Group messages chronologically every 3 hours
                const showDivider = !prevMsg || (new Date(msg.created_at) - new Date(prevMsg.created_at)) > (3 * 3600 * 1000);
                
                return (
                  <React.Fragment key={msg.id}>
                    {showDivider && (
                      <div className="chat-time-divider">
                        {get3HourTimeDivider(msg.created_at)}
                      </div>
                    )}
                    <div className={`message-bubble ${isOutgoing ? 'outgoing' : 'incoming'}`}>
                      {!isOutgoing && (
                        <div
                          ...
                          onClick={() => handleUserClick(msg.sender_id, msg.profiles?.username || 'User')}
                        >
                          {msg.profiles?.username || 'User'}
                        </div>
                      )}
                      <div>{formatMentions(msg.content)}</div>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px', marginTop: '4px' }}>
                        <span style={{ fontSize: '0.58rem', color: 'rgba(255, 255, 255, 0.5)', display: 'block' }}>
                        ...
                        {isOutgoing && (
                          <div style={{ display: 'flex', gap: '6px' }}>
                            <button
                              ...
                              style={{ background: 'none', border: 'none', color: 'var(--tg-accent)', fontSize: '0.65rem', cursor: 'pointer' }}
                            >
                              Edit
                            </button>
                            <button
                              onClick={() => deleteMessage(msg.id)}
                              style={{ background: 'none', border: 'none', color: 'var(--tg-destructive)', fontSize: '0.65rem', cursor: 'pointer' }}
                            >
                              Delete
                            </button>
                          </div>
            ...
            {/* LIVE VOICE LISTENING TYPING INDICATOR */}
            {isListening && (
              <div className="typing-indicator-container">
                <span style={{ fontSize: '0.65rem', color: 'var(--tg-accent)', fontWeight: 'bold' }}>LISTENING</span>
          ...
          <div className="chat-input-floating-wrapper">
            <div className="chat-input-bar">
              ...
              <button
                className="btn-3d btn-3d-primary send-btn"
                title="Send Message"
      ...
      {/* ============================================================
         MODAL 1: GROUP ABOUT & MEMBER MANAGEMENT
         ============================================================ */}`
      {modals.groupAboutModal && currentOpenGroup && (
        <div className="overlay-screen active" onClick={(e) => e.target === e.currentTarget && closeModal('groupAboutModal')}>
          <div className="modal-box">
            <div className="modal-header">
              <h3>{currentOpenGroup.name.toUpperCase()}</h3>
              <button className="close-modal-btn" onClick={() => closeModal('groupAboutModal')}>&times;</button>
            </div>
            <div className="modal-body">
              <div className="decorated-prompt-box">
                <h4 style={{ fontSize: '0.85rem', color: 'var(--tg-accent)', marginBottom: '4px' }}>ABOUT CHANNEL</h4>
                <p style={{ fontSize: '0.75rem', color: '#fff' }}>{currentOpenGroup.description || 'No overview provided.'}</p>
                <div style={{ marginTop: '8px', fontSize: '0.68rem', color: 'var(--text-muted)' }}>
                  CREATED: {formatDetailedTimestamp(currentOpenGroup.created_at)}
                </div>
              </div>

              <div>
                <h4 style={{ fontSize: '0.8rem', color: 'var(--tg-accent)', marginBottom: '8px' }}>CHANNEL MEMBERS ({groupAboutMembers.length})</h4>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '180px', overflowY: 'auto' }}>
                  {groupAboutMembers.map(m => {
                    const isAdmin = m.role === 'admin';
                    const isSelf = m.user_id === currentUser?.id;
                    const canManage = isGroupAdmin(currentOpenGroup, currentUser?.id) && !isSelf;

                    return (
                      <div key={m.user_id} className="notification-item-card" style={{ padding: '8px 10px' }}>
                        <div className="notification-left-details">
                          ...
                          <div>
                            <div style={{ fontSize: '0.78rem', color: '#fff', fontWeight: 'bold' }}>
                              {m.profiles?.username || 'User'} {isSelf ? '(You)' : ''}
                            </div>
                            <span className={`solid-animated-tag ${isAdmin ? 'tag-admin' : 'tag-member'}`}>
                              {m.role.toUpperCase()}
                            </span>
                          </div>
                        </div>
                        ...
                        {canManage && (
                          <div style={{ display: 'flex', gap: '4px' }}>
                            {isAdmin ? (
                              <button className="btn-3d btn-3d-dark" style={{ padding: '4px 8px', fontSize: '0.6rem' }} onClick={() => demoteAdmin(currentOpenGroup.id, m.user_id)}>
                                Demote
                              </button>
                            ) : (
                              <button className="btn-3d btn-3d-primary" style={{ padding: '4px 8px', fontSize: '0.6rem' }} onClick={() => promoteUser(currentOpenGroup.id, m.user_id)}>
                                Promote
                              </button>
                            )}
                            <button className="btn-3d btn-3d-danger" style={{ padding: '4px 8px', fontSize: '0.6rem' }} onClick={() => suspendMember(currentOpenGroup.id, m.user_id)}>
                              Suspend
                            </button>
                          </div>
                        )}
              ...
              <div style={{ display: 'flex', gap: '8px', marginTop: '10px' }}>
                {!isMember(currentOpenGroup, currentUser?.id) ? (
                  <button className="btn-3d btn-3d-primary" style={{ flex: 1, padding: '10px' }} onClick={() => joinGroup(currentOpenGroup.id)}>
                    {isJoiningGroup ? 'CONNECTING...' : 'JOIN GROUP'}
                  </button>
                ) : (
                  <>
                    <button className="btn-3d btn-3d-dark" style={{ flex: 1, padding: '10px' }} onClick={() => inviteMember(currentOpenGroup.id)}>
                      INVITE
                    </button>
                    <button className="btn-3d btn-3d-danger" style={{ flex: 1, padding: '10px' }} onClick={() => exitGroup(currentOpenGroup.id)}>
                      EXIT GROUP
                    </button>
                  </>
                )}
              ...
              {isGroupAdmin(currentOpenGroup, currentUser?.id) && (
                <button className="btn-3d btn-3d-danger" style={{ width: '100%', padding: '10px', marginTop: '4px' }} onClick={() => deleteGroup(currentOpenGroup.id)}>
                  DELETE GROUP
                </button>
              )}
            </div>
      ...
      {/* ============================================================
         MODAL 2: CREATE NEW GROUP
         ============================================================ */}
      {modals.createGroupModal && (
        <div className="overlay-screen active" onClick={(e) => e.target === e.currentTarget && closeModal('createGroupModal')}>
          <div className="modal-box">
            <div className="modal-header">
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                <div className="radio-options">
                  <div
                    className={`radio-card ${selectedNewGroupType === 'Public' ? 'selected' : ''}`}
                    onClick={() => setSelectedNewGroupType('Public')}
                  >
                    <div>
                      <div style={{ fontSize: '0.8rem', color: '#fff', fontWeight: 'bold' }}>Public Channel</div>
                      <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)' }}>Visible to all matrix network users.</div>
                    </div>
                  </div>

                  <div
                    className={`radio-card ${selectedNewGroupType === 'Private' ? 'selected' : ''}`}
                    onClick={() => setSelectedNewGroupType('Private')}
                  >
                    <div>
                      <div style={{ fontSize: '0.8rem', color: '#fff', fontWeight: 'bold' }}>Private Node</div>
                      <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)' }}>Accessible only through invitation links.</div>
                    </div>
                  </div>
                </div>
              </div>

              <button className="btn-3d btn-3d-primary" style={{ width: '100%', padding: '12px', marginTop: '8px' }} onClick={createNewGroupSubmit}>
                INITIALIZE CHANNEL
              </button>
            </div>
          </div>
      ...
      {/* ============================================================
         MODAL 3: GROUP OVERVIEW
         ============================================================ */}
      {modals.groupOverviewModal && (
        <div className="overlay-screen active" onClick={(e) => e.target === e.currentTarget && closeModal('groupOverviewModal')}>
          <div className="modal-box">
            <div className="modal-header">
            ...
            <div className="modal-body">
              <div className="decorated-prompt-box">
                <h4 style={{ fontSize: '0.85rem', color: 'var(--tg-accent)', marginBottom: '6px' }}>SYSTEM CHANNELS HUB</h4>
                <p style={{ fontSize: '0.75rem', color: '#fff', lineHeight: '1.5' }}>
                  Encrypted multi-node communication hub. Standard users can browse public channels, create encrypted direct groups, or manage local archive records.
                </p>
              </div>
              <button className="btn-3d btn-3d-dark" style={{ width: '100%', padding: '10px' }} onClick={() => closeModal('groupOverviewModal')}>
                DISMISS
              </button>
            </div>
          </div>
      ...
      {modals.editProfileModal && (
        <div className="overlay-screen active" onClick={(e) => e.target === e.currentTarget && closeModal('editProfileModal')}>
          <div className="modal-box">
            <div className="modal-header">
              <h3>EDIT IDENTITY</h3>
            ...
            <div className="modal-body">
              ...
              <button className="btn-3d btn-3d-primary" style={{ width: '100%', padding: '12px', marginTop: '8px' }} onClick={saveProfileChanges}>
                SAVE IDENTITY
              </button>
            </div>
          </div>
      ...
      {/* ============================================================
         MODAL 5: GROUP APPROVAL (ADMIN CHECK)
         ============================================================ */}
      {modals.deleteRequestModal && (
        <div className="overlay-screen active" onClick={(e) => e.target === e.currentTarget && closeModal('deleteRequestModal')}>
          <div className="modal-box">
            <div className="modal-header">
            ...
            <div className="modal-body">
              {pendingApprovalGroups.length === 0 ? (
                <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>No channel requests awaiting clearance.</p>
              ) : (
                pendingApprovalGroups.map(p => (
                  <div key={p.id} className="notification-item-card">
                    <div>
                      <h4 style={{ fontSize: '0.8rem', color: '#fff' }}>{p.name}</h4>
                      <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>{p.description}</p>
                    </div>
                    {currentProfile?.is_global_admin && (
                      <button className="btn-3d btn-3d-primary" style={{ padding: '6px 10px', fontSize: '0.65rem' }} onClick={() => approveGroup(p.id)}>
                        Approve
                      </button>
                    )}
                  </div>
      ...
      {/* ============================================================
         GLOBAL CUSTOM PROMPT / CONFIRMATION MODAL
         ============================================================ */}
      {customPrompt && (
        <div className="overlay-screen active" style={{ zindex: 200 }}>
          <div className="modal-box">
            <div className="modal-header">
            ...
            <div className="modal-body">
              {customPrompt.message && (
                <p style={{ fontSize: '0.8rem', color: '#fff', lineHeight: '1.4' }}>{customPrompt.message}</p>
              )}
              ...
              {customPrompt.type === 'input' && (
                <input
                  ...
                  style={{ background: 'rgba(14, 28, 46, 0.8)', border: '1px solid rgba(0, 240, 255, 0.25)', borderRadius: '10px', padding: '10px', color: '#fff' }}
                />
              )}
              ...
              <div style={{ display: 'flex', gap: '8px', marginTop: '10px' }}>
                ...
                <button
                  ...
                  style={{
                    flex: 1,
                    padding: '10px',
                    background: customPrompt.confirmBg || 'var(--3d-button-bg)',
                    color: customPrompt.confirmColor || '#fff'
                  }}
                  onClick={() => {
