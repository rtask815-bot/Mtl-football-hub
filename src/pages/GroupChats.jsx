...
export default function GroupChats() {
  ...
  // Dynamic Text Cycling for Full-Screen Loader Prompt
  useEffect(() => {
    if (isLoading) {
    ...
    <>
      ...
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;font-family:'Segoe UI',-apple-system,BlinkMacSystemFont,Roboto,sans-serif;scrollbar-width:thin;scrollbar-color:var(--tg-accent) var(--bg-deep)}
        ...
        /* Telegram Side Nav Drawer with Close X Button */
        .side-nav-drawer{position:absolute;top:0;left:-100vw;width:100vw;max-width:320px;height:100%;background:var(--tg-dark-panel);border-right:var(--3d-card-border);backdrop-filter:blur(20px);z-index:110;transition:left 0.3s cubic-bezier(0.16, 1, 0.3, 1);padding:16px 12px;display:flex;flex-direction:column;gap:10px;box-shadow:15px 0 30px rgba(0, 0, 0, 0.8)}
        .side-nav-drawer.open{left:0}
        .side-nav-header{display:flex;align-items:center;justify-content:space-between;padding-bottom:12px;border-bottom:1px solid rgba(255, 255, 255, 0.08);margin-bottom:6px}
        .side-nav-header h3{font-size:0.9rem;letter-spacing:0.08em;color:var(--tg-accent);font-weight:800}
        .side-nav-close-btn{background:none;border:none;color:var(--text-muted);font-size:1.4rem;cursor:pointer;line-height:1;transition:color 0.15s}
        ...
        /* FULL SCREEN OVERLAY IMAGE-STYLE FUTURISTIC LOADER */
        ...
        .loader-card-container {
          ...
          box-shadow: 0 0 35px rgba(0, 180, 255, 0.15), inset 0 0 15px rgba(0, 240, 255, 0.05);
          border-radius: 20px;
        ...
        .loader-action-button {
          width: 100%;
          background: linear-gradient(90deg, #162a3d, #12344d);
        ...
        /* Messages Styling with Slide In Animations */
        ...
        .message-bubble.incoming{align-self:flex-start;background:var(--3d-card-bg);border:var(--3d-card-border);border-bottom-left-radius:2px;animation:slideInLeft 0.25s cubic-bezier(0.16, 1, 0.3, 1) forwards}
        .message-bubble.outgoing{align-self:flex-end;background:linear-gradient(145deg, #2b5278, #1c3854);border:1px solid rgba(51, 144, 236, 0.3);border-bottom-right-radius:2px;animation:slideInRight 0.25s cubic-bezier(0.16, 1, 0.3, 1) forwards}
        ...
        /* Modals & Soft Slide-in Overlays */
        .overlay-screen{position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(0, 0, 0, 0.82);backdrop-filter:blur(8px);z-index:100;display:none;align-items:center;justify-content:center;padding:14px;opacity:0;transition:opacity 0.2s ease}
        .overlay-screen.active{display:flex;opacity:1}
        .modal-box{background:var(--tg-dark-panel);border:var(--3d-card-border);box-shadow:0 12px 40px rgba(0, 0, 0, 0.8);border-radius:16px;width:100%;max-width:440px;max-height:88vh;overflow-y:auto;display:flex;flex-direction:column;transform:translateY(20px);transition:transform 0.25s cubic-bezier(0.16, 1, 0.3, 1)}
        .overlay-screen.active .modal-box{transform:translateY(0)}
      ...
      {/* FULL-SCREEN FUTURISTIC CONTAINER LOADER OVERLAY (Disappears after loading finishes) */}
      {isLoading && (
            ...
            <div className={`view-section ${activeMainView === 'chats' ? 'active' : ''}`} id="view-chats">
              ...
              <div>
                ...
                <div className="group-list" id="groupListContainer" style={{ marginTop: '12px' }}>
                  ...
                  {/* REGULAR NON-PINNED CHATS LIST (Correctly shows Archived chats when 'archived' filter is selected) */}
          ...
          <div className="chat-messages-area" id="chatMessagesArea" ref={chatMessagesAreaRef}>
            ...
            {isListening && (
              <div className="typing-indicator-container">
                <span style={{ fontSize: '0.68rem', color: 'var(--tg-accent)', fontWeight: 'bold', marginRight: '4px' }}>Listening</span>
              ...
              if (!isMemberUser && !currentProfile?.is_global_admin && group.creator_id !== currentUser?.id) {
                return (
                  <>
                    ...
                    {/* Join Group Prompt Container (Closes completely once joined) */}
                    <div style={{ background: 'var(--3d-card-bg)', border: 'var(--3d-card-border)', boxShadow: 'var(--3d-card-shadow)', padding: '12px', borderRadius: '12px', width: '100%' }}>
                      ...
                      >
                        {isJoiningGroup ? 'Joining Group...' : 'Join Group Now'}
                      </button>
                    </div>
                  </>
                );
      ...
      <div className={`overlay-screen ${modals.deleteRequestModal ? 'active' : ''}`} id="deleteRequestModal">
        <div className="modal-box" style={{ textAlign: 'center' }}>
          ...
          <div className="modal-body" style={{ alignItems: 'center' }}>
            <div style={{ width: '42px', height: '42px', borderRadius: '50%', background: 'rgba(51,144,236,0.15)', border: '1px solid var(--tg-accent)', display: 'flex', itemsCenter: 'center', justifyContent: 'center', color: 'var(--tg-accent)' }}>
              🛡️
            </div>
