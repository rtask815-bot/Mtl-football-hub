...
export default function DirectUserChats() {
  ...
  // Chat Request Status: 'approved' | 'pending' | 'rejected' | 'none'
  ...
  // Requested Filter Tabs: 'all', 'my_chats', 'archived'
  ...
  const fetchAllUsersAndDirectChats = useCallback(async (myUserId) => {
    ...
    // Fetch incoming chat_request records for user approval
    ...
    if (!msgErr && directMsgs) {
      ...
      LocalStore.set('unread_direct', unreadMap);
    }
  }, []);

  const verifyChatApproval = useCallback(async (peerUserId) => {
    if (!currentUser || !peerUserId) return 'none';
    ...
    if (!error && data && data.length > 0) {
      const latestReq = data[0];
      setChatRequestStatus(latestReq.status);
      LocalStore.set(cacheKey, latestReq.status);
      return latestReq.status;
    }

    setChatRequestStatus('none');
    LocalStore.set(cacheKey, 'none');
    return 'none';
  }, [currentUser]);
  ...
  const setupRealtimeSubscriptions = useCallback(() => {
    ...
    const reqChannel = supabaseClient
      .channel('public-chat-request')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'chat_request' }, payload => {
        if (payload.new) {
  ...
  /* ============================================================
     CHAT REQUEST LOGIC USING chat_request TABLE
     ============================================================ */
  async function sendChatRequest(peerId) {
    if (!currentUser) return false;
    
    // UPSERT chat request: Insert or update if rejected previously
    const { data, error } = await supabaseClient
      .from('chat_request')
      .upsert({
    ...
    if (!error) {
      ...
      addLocalNotification("Chat Request Sent", "Your chat request has been sent for approval.");
      return true;
    } else {
      showErrorBanner("Request Error", "Could not send chat request.");
      return false;
    }
  }
  ...
  async function handleRejectChatRequest(requestId, senderId) {
    ...
    if (!error) {
      ...
      if (senderId) {
        LocalStore.set(`chat_request_status_${senderId}`, 'rejected');
      }
      if (currentUser) {
        fetchAllUsersAndDirectChats(currentUser.id);
      }
      if (currentActivePeer && currentActivePeer.id === senderId) {
        setChatRequestStatus('rejected');
      }
    }
  }
  ...
  async function sendChatMessage() {
    ...
    const status = await verifyChatApproval(currentActivePeer.id);

    if (status !== 'approved') {
      showErrorBanner("Chat Locked", "Messaging is disabled until the chat request is approved.");
      return;
    }

    // Deliver message on approval
    const { error } = await supabaseClient
      .from('messages')
      .insert([{
        sender_id: currentUser.id,
  ...
  /* ============================================================
     ...
     Filter Buttons: 'All Chats', 'My chats', 'Archived'
     ============================================================ */
  const getDisplayChats = () => {
    ...
    // Apply strict tab filters: 'all', 'my_chats', 'archived'
    if (currentTabFilter === 'archived') {
      chatsArr = chatsArr.filter(c => archivedChatIds.includes(c.id));
    } else if (currentTabFilter === 'my_chats') {
      chatsArr = chatsArr.filter(c => !archivedChatIds.includes(c.id) && c.isRecent);
    } else {
      // 'all' chats tab (excluding archived ones)
      chatsArr = chatsArr.filter(c => !archivedChatIds.includes(c.id));
    }
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;font-family:'Segoe UI',-apple-system,BlinkMacSystemFont,Roboto,sans-serif;scrollbar-width:thin;scrollbar-color:var(--tg-accent) var(--bg-deep)}
        ...
        /* FUTURISTIC DECORATED 3D LOADER PROMPT */
        ...
        /* 3D FUTURISTIC BUTTONS & CARDS */
        .btn-3d{position:relative;background:linear-gradient(135deg, #0088cc, #005580);color:#fff;border:none;border-radius:10px;padding:10px 18px;font-weight:700;font-size:0.85rem;cursor:pointer;box-shadow:0 6px 0 #003350, 0 10px 20px rgba(0,0,0,0.4);transition:all 0.15s ease;display:inline-flex;align-items:center;justify-content:center;gap:8px;text-transform:uppercase;letter-spacing:0.5px}
        .btn-3d:active{transform:translateY(4px);box-shadow:0 2px 0 #003350, 0 4px 10px rgba(0,0,0,0.4)}
        .btn-3d-danger{background:linear-gradient(135deg, #e17076, #9e3035);box-shadow:0 6px 0 #611a1d, 0 10px 20px rgba(0,0,0,0.4)}
        .btn-3d-danger:active{transform:translateY(4px);box-shadow:0 2px 0 #611a1d, 0 4px 10px rgba(0,0,0,0.4)}
        .btn-3d-success{background:linear-gradient(135deg, #2ecc71, #1b8a4b);box-shadow:0 6px 0 #10522c, 0 10px 20px rgba(0,0,0,0.4)}
        .btn-3d-success:active{transform:translateY(4px);box-shadow:0 2px 0 #10522c, 0 4px 10px rgba(0,0,0,0.4)}

        .futuristic-card{background:linear-gradient(145deg, rgba(24, 37, 56, 0.8), rgba(18, 27, 41, 0.9));border:1px solid rgba(82, 136, 193, 0.25);border-radius:14px;box-shadow:0 8px 32px 0 rgba(0, 0, 0, 0.37);backdrop-filter:blur(10px)}
        ...
        /* HORIZONTAL SWITCH TAB BUTTONS ('All Chats', 'My chats', 'Archived') */
        ...
        .tab-pill{background:var(--tg-pill-bg);color:var(--text-muted);border:none;padding:8px 16px;border-radius:16px;font-size:0.82rem;font-weight:600;cursor:pointer;white-space:nowrap;display:flex;align-items:center;gap:6px;transition:all 0.2s}
        ...
        .chat-row-item{display:flex;align-items:center;justify-content:space-between;padding:12px 16px;background:transparent;border-bottom:1px solid rgba(255, 255, 255, 0.03);cursor:pointer;transition:background 0.15s;user-select:none;position:relative}
        ...
        .chat-row-right-end{display:flex;align-items:center;gap:8px;margin-left:auto;flex-shrink:0}
        .pin-icon{color:var(--text-muted);font-size:0.85rem}
        
        /* UNREAD COUNTER SHOWN ON FURTHEST END AS NUMBERS */
        .unread-number-counter{background:var(--tg-accent);color:#fff;font-size:0.72rem;font-weight:700;padding:2px 8px;border-radius:12px;min-width:20px;text-align:center;display:inline-flex;align-items:center;justify-content:center;box-shadow:0 2px 5px rgba(0,136,204,0.4)}
        ...
        /* DYNAMIC CHAT REQUEST PROMPT BANNERS */
        .chat-prompt-card{padding:16px;margin:14px auto;max-width:440px;text-align:center;display:flex;flex-direction:column;align-items:center;gap:10px;border-radius:12px}
        .chat-prompt-card.pending{background:rgba(0, 136, 204, 0.1);border:1px solid var(--tg-accent)}
        .chat-prompt-card.rejected{background:rgba(225, 112, 118, 0.1);border:1px solid var(--tg-destructive)}
        .chat-prompt-card.initial{background:var(--bg-card);border:1px solid rgba(255, 255, 255, 0.1)}
      ...
      <div className="app-container">
        <main className="workspace">
          ...
          {/* SECTION 3: EXACT TAB BUTTONS IMPLEMENTATION: 'All Chats', 'My chats', 'Archived' */}
          <div className="tab-pills-container">
            <button className={`tab-pill ${currentTabFilter === 'all' ? 'active' : ''}`} onClick={() => setCurrentTabFilter('all')}>
              All Chats
            </button>
            <button className={`tab-pill ${currentTabFilter === 'my_chats' ? 'active' : ''}`} onClick={() => setCurrentTabFilter('my_chats')}>
              My chats
            </button>
          ...
          <div className="main-content">
            ...
            <div className={`view-section ${activeMainView === 'chats' ? 'active' : ''}`}>
              ...
              {/* PENDING CHAT REQUEST APPROVALS */}
              {pendingChatRequests.length > 0 && (
                <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <h4 style={{ fontSize: '0.78rem', color: 'var(--tg-accent)', textTransform: 'uppercase', letterSpacing: '1px' }}>
                    Pending Request Approvals ({pendingChatRequests.length})
                  </h4>
                  {pendingChatRequests.map(req => (
                    <div key={req.id} className="futuristic-card" style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div className="flex-row">
                        ...
                        <div>
                          ...
                          <p style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>Sent a chat request</p>
                        </div>
                      </div>
              ...
              <div className="chat-list-group">
                ...
                {/* CHAT ROWS WITH UNREAD NEW MESSAGE COUNTER AT THE FURTHEST END */}
                  ...
                  filteredDisplayList.map(item => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Unread message counter as numbers at the furthest end */}
                        <div className="chat-row-right-end">
                          {unread > 0 && (
                            <span className="unread-number-counter" title={`${unread} unread messages`}>
                              {unread}
                            </span>
                          )}
                        </div>
      ...
      {/* MODAL: DIRECT CHAT ROOM WITH SPECIFIED CHAT APPROVAL PROMPTS */}
      <div className={`overlay-screen ${modals.chatRoomModal ? 'active' : ''}`} id="chatRoomModal">
        <div className="chat-room-container" ref={chatContainerRef}>
          ...
          <div className="chat-messages-area" ref={chatMessagesAreaRef}>
            ...
            {/* DYNAMIC PROMPT HANDLING BASED ON REQUEST STATUS */}
            {chatRequestStatus === 'none' && (
              <div className="chat-prompt-card initial">
                <h4 style={{ color: '#fff', fontSize: '0.9rem' }}>Send Chat Request</h4>
                <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem' }}>
                  You need to send a chat request to this user to open the direct conversation.
                </p>
                <button
                  className="btn-3d"
                  style={{ marginTop: '6px' }}
                  onClick={() => sendChatRequest(currentActivePeer?.id)}
                >
                  Send Chat Request
                </button>
              </div>
            ...
            {chatRequestStatus === 'pending' && (
              <div className="chat-prompt-card pending">
                <h4 style={{ color: 'var(--tg-accent)', fontSize: '0.9rem' }}>Waiting for Request Approvals</h4>
                <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem' }}>
                  Your chat request has been sent. Chat page will open when approved.
                </p>
              </div>
            )}

            {chatRequestStatus === 'rejected' && (
              <div className="chat-prompt-card rejected">
                <h4 style={{ color: 'var(--tg-destructive)', fontSize: '0.9rem' }}>Request Rejected, Please Try Again</h4>
                <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem' }}>
                  The chat request was declined. You can attempt to resend below.
                </p>
                <button
            ...
            {/* ONLY WHEN APPROVED DO THE CHAT MESSAGES OPEN */}
          ...
          {/* FLOATING INPUT BAR - UNLOCKED ONLY WHEN APPROVED */}
          <div className="chat-input-floating-wrapper">
            <div className="chat-input-bar">
              <textarea
                placeholder={chatRequestStatus === 'approved' ? "Message" : "Chat locked until approved"}
                rows={1}
                disabled={chatRequestStatus !== 'approved'}
                value={chatInputText}
                onChange={(e) => {
