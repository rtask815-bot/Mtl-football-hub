...
export default function DirectUserChats() {
  ...
  /* ============================================================
     1-MINUTE CHATS & DATA SYNC TIMER (REAL-TIME CHAT LOADING)
     ============================================================ */
  useEffect(() => {
    if (!currentUser) return;
  ...
  /* ============================================================
     SEND CHAT REQUEST & MESSAGE DELIVERY RESOLUTION
     ============================================================ */
  async function sendChatRequest(peerId) {
    ...
    if (!error) {
      addLocalNotification("Chat Request Sent", "Your request to connect has been sent for approval.");
      return true;
    }
    return false;
  }
  ...
  async function sendChatMessage() {
    ...
    // Check for explicit connection/approval between the two users
    let { data: connection, error: connErr } = await supabaseClient
      .from('user_connections')
      .select('*')
      .or(`and(user1_id.eq.${currentUser.id},user2_id.eq.${currentActivePeer.id}),and(user1_id.eq.${currentActivePeer.id},user2_id.eq.${currentUser.id})`)
      .single();

    if (connErr || !connection || connection.status !== 'approved') {
      // Check for an existing approved chat request
      let { data: request } = await supabaseClient
        .from('chat_requests')
        .select('*')
        .or(`and(sender_id.eq.${currentUser.id},receiver_id.eq.${currentActivePeer.id}),and(sender_id.eq.${currentActivePeer.id},receiver_id.eq.${currentUser.id})`)
        .eq('status', 'approved')
        .single();

      if (!request) {
        // Automatically issue send chat request to establish communication authorization upon approval
        const requestSent = await sendChatRequest(currentActivePeer.id);
        showErrorBanner(
          "Unable to deliver chat to user", 
          requestSent 
            ? "A chat request has been automatically sent. Communication will open once approved." 
            : "Approval required before messaging. Direct delivery failed."
        );
        return;
      }
    }
