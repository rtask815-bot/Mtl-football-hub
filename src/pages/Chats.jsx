...
export default function ChatPage({ onNavigateDashboard }) {
  ...
  // Telegram-style Category Filter Tabs
  const [telegramTab, setTelegramTab] = useState('All');
  ...
  const fetchOrCreateProfile = async (user) => {
    ...
    if (!profile) {
      ...
      const { data: newProfile } = await supabase
        .from('profiles')
        .insert([{ id: user.id, username: fallbackName, status_message: 'Telegram Node Active.' }])
        .select().single();
  ...
  const initSpaceCanvas = () => {
    ...
    function render() {
      ctx.fillStyle = '#0e1621'; // Telegram dark theme background tone
  ...
  // Filtered Chats for Telegram Style
  const getFilteredChats = () => {
    return activeChats.filter(chat => {
      const matchesSearch = chat.username.toLowerCase().includes(searchQuery.toLowerCase());
      if (telegramTab === 'All') return matchesSearch;
      if (telegramTab === 'P-ME') return matchesSearch && chat.username.toLowerCase().includes('p');
      if (telegramTab === 'SURE-ODDS') return matchesSearch;
    ...
    <div className="telegram-app-root">
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; }
        ...
        .tg-container { display: flex; flex-direction: column; width: 100vw; height: 100vh; position: relative; z-index: 5; }
        ...
        .tg-menu-btn, .tg-icon-btn { background: none; border: none; color: #708499; cursor: pointer; font-size: 1.2rem; display: flex; align-items: center; justify-content: center; width: 38px; height: 38px; border-radius: 50%; }
        .tg-menu-btn:hover, .tg-icon-btn:hover { background: rgba(255,255,255,0.08); color: #fff; }
        ...
        /* Main Content List */
        .tg-chat-list { flex: 1; overflow-y: auto; background: #0e1621; display: flex; flex-direction: column; }
        .tg-chat-item {
        ...
        .tg-chat-item:hover { background: #17212b; }
        ...
        /* Discover and Pending section tabs */
        ...
        /* Chat Room Overlay */
        .tg-room-overlay { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: #0e1621; z-index: 100; display: flex; flex-direction: column; }
        .tg-room-header { height: 60px; background: #17212b; display: flex; align-items: center; justify-content: space-between; padding: 0 16px; flex-shrink: 0; border-bottom: 1px solid rgba(255,255,255,0.05); }
        .tg-messages-box { flex: 1; overflow-y: auto; padding: 16px; display: flex; flex-direction: column; gap: 8px; background: url('https://web.telegram.org/a/bg-pattern-dark.png') repeat #0e1621; }
        
        .tg-bubble { max-width: 65%; padding: 8px 12px; border-radius: 12px; font-size: 0.9rem; line-height: 1.4; position: relative; word-break: break-word; color: #fff; }
        ...
        .tg-input-area { padding: 10px 16px; background: #17212b; display: flex; align-items: flex-end; gap: 10px; flex-shrink: 0; }
      ...
      <div className="tg-container">
        ...
        <header className="tg-header">
          <div className="tg-header-left">
            <button className="tg-menu-btn" onClick={() => onNavigateDashboard?.()} title="Dashboard">☰</button>
        ...
        <div className="tg-tabs-bar">
          <button className={`tg-tab-pill ${telegramTab === 'All' ? 'active' : ''}`} onClick={() => setTelegramTab('All')}>
            All ({activeChats.length})
          </button>
          <button className={`tg-tab-pill ${telegramTab === 'P-ME' ? 'active' : ''}`} onClick={() => setTelegramTab('P-ME')}>
            P-ME 🤬 ({chatRequests.filter(r => r.status === 'approved').length})
          </button>
          <button className={`tg-tab-pill ${telegramTab === 'SURE-ODDS' ? 'active' : ''}`} onClick={() => setTelegramTab('SURE-ODDS')}>
            SURE-ODDS ? ({availableUsers.length})
          </button>
        ...
        {/* Telegram Chat List Feed */}
        <div className="tg-chat-list">
          {telegramTab !== 'PENDING' && getFilteredChats().map(chat => (
            <div className="tg-chat-item" key={chat.id} onClick={() => setCurrentOpenChat(chat)}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1 }}>
                ...
                <div style={{ flex: 1, overflow: 'hidden' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: '0.7rem', color: '#708499' }}>14:00</span>
                  </div>
          ...
          {telegramTab === 'SURE-ODDS' && (
            <>
              <div className="section-header-tag">Available Global Nodes</div>
            ...
            <>
              ...
              {chatRequests.filter(r => r.receiver_id === currentUser?.id && r.status === 'pending').map(req => (
                <div className="tg-chat-item" key={req.id} style={{ padding: '14px 16px', flexDirection: 'column', alignItems: 'flex-start', gap: '8px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-width', width: '100%' }}>
      ...
      {/* --- CHAT ROOM INTERFACE --- */}
      {currentOpenChat && (
        <div className="tg-room-overlay">
          <div className="tg-room-header">
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <button className="tg-menu-btn" onClick={() => setCurrentOpenChat(null)}>←</button>
              <div className="tg-avatar" style={{width:'38px', height:'38px'}}>
          ...
          <div className="tg-input-area">
            ...
            />
            <button className="tg-menu-btn" style={{ background: '#2b5278', borderRadius: '50%', color: '#fff', width: '40px', height: '40px' }} onClick={handleSendMessage}>➤</button>
          </div>
        </div>
      )}
      ...
      {activeModal === 'editProfile' && (
        <div className="tg-modal-overlay" onClick={() => setActiveModal(null)}>
          <div className="tg-modal-box" onClick={e => e.stopPropagation()}>
            <h3 style={{ color: '#64b5f6', fontSize: '1rem' }}>Edit Telegram Profile</h3>
            <div>
