...
export default function DirectUserChats() {
  ...
  async function sendChatRequest(peerId) {
    ...
    if (existing) {
      ...
      showErrorBanner("Chat Request Pending", "A chat request is already pending approval.");
      return false;
    }
  ...
  async function handleRejectChatRequest(requestId) {
    const { error } = await supabaseClient
      .from('chat_request')
      .delete()
      .eq('id', requestId);
  ...
  async function sendChatMessage() {
    ...
    // Check for approved chat request between the two users
    const unlocked = await verifyChatApproval(currentActivePeer.id);

    if (!unlocked) {
      const requestSent = await sendChatRequest(currentActivePeer.id);
      showErrorBanner(
        "Chat Locked", 
        requestSent 
          ? "A chat_request was sent. Waiting for recipient to approve and unlock chatting." 
          : "Chat approval required to send messages."
      );
      return;
    }

    // Proceed to deliver message upon approval
    const { error } = await supabaseClient
      .from('messages')
      .insert([{
        sender_id: currentUser.id,
                  ...
                  {pendingChatRequests.map(req => (
                    <div key={req.id} className="futuristic-card" style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        ...
                        <button className="btn-3d btn-3d-danger" style={{ padding: '6px 12px', fontSize: '0.72rem' }} onClick={() => handleRejectChatRequest(req.id)}>
                          Decline
                        </button>
                      </div>
                    </div>
      ...
      <div className={`overlay-screen ${modals.chatRoomModal ? 'active' : ''}`} id="chatRoomModal">
        <div className="chat-room-container" ref={chatContainerRef}>
          ...
          <div className="chat-messages-area" ref={chatMessagesAreaRef}>
            {!isChatUnlocked && (
              <div className="futuristic-card" style={{ padding: '14px', margin: '12px auto', maxWidth: '420px', textAlign: 'center' }}>
                <h4 style={{ color: 'var(--tg-accent)', fontSize: '0.88rem', marginBottom: '4px' }}>🔒 Chat Approval Required</h4>
                <p style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>
                  Chat request approval is needed between users to unlock full messaging.
                </p>
              </div>
            )}
