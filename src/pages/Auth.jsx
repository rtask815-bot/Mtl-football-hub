...
export default function Auth() {
    ...
    // Canvas Background FX Matrix Engine
    useEffect(() => {
        ...
        const animate = () => {
            ...
            for (let i = 0; i < particles.length; i++) {
                for (let j = i; j < particles.length; j++) {
                    ...
                    if (dist < 130) {
                        ctx.beginPath();
                        ctx.strokeStyle = `rgba(168, 85, 247, ${0.05 - dist / 2600})`;
                        ctx.lineWidth = 0.8;
    ...
    useEffect(() => {
        const checkSession = async () => {
            ...
            if (session) {
                ...
                triggerSecureTransition("/user", "REDIRECTING...");
            }
        };
        checkSession();
    }, [navigate]);
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            triggerSecureTransition("/user", "processing...");
        } catch (err) {
    ...
    const getActiveSessionIdentity = () => {
        ...
        if (storedUser) {
            try {
                ...
                return parsed.user_metadata?.username || parsed.email.split("@")[0];
            } catch (e) {
                return "Operator";
            }
        }
    ...
    const openChatConsole = () => {
        if (!isChatOpen) {
            ...
            setChatMessages([
                {
                    role: "system",
                    content: `You are Mr Mourice, MTL Football Predictions Authorization dashboard technician. Use internal dashboard knowledge context layers. Be concise, structured, and helpful. Analysis Temporal Benchmark Timestamp: "${currentTimestampString}".`
                },
                { role: "bubble-ai", text: `Hello ${identity}, how are you doing today?` }
            ]);
            setIsChatOpen(true);
        } else {
            setIsChatOpen(false);
        ...
        <div className="auth-page-wrapper">
            <style>{`
                ...
                * { box-sizing: border-box; margin: 0; padding: 0; }
                ...
                .forgot-password-container { display: flex; justify-content: flex-end; margin-top: 8px; padding-right: 4px; }
                .forgot-password-link { font-size: 12px; font-weight: 600; color: var(--text-muted); text-decoration: none; transition: color 0.2s ease; }
