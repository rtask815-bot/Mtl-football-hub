...
export default function Auth() {
    ...
    // HIGH-PERFORMANCE REALISTIC 4D THREE.JS SYNCHRONIZED ROBOT CHASE ANIMATION
    useEffect(() => {
        ...
        // Synchronized WebGL Physics Engine
        let animFrameId;
        let smoothCurrentProgress = targetProgressRef.current;

        const animate3D = () => {
            // High precision linear interpolation synced directly to progress percentage
            ...
            // Lock X-Axis bounds to loader percentage
            const minX = -4.5;
            const maxX = 3.5;
            const currentX = minX + (maxX - minX) * progressRatio;

            // Gait oscillation phase tied directly to distance moved
            const phase = progressRatio * Math.PI * 18;

            // Robot Position & Realist Bounce Dynamics
            robotGroup.position.x = currentX;
            robotGroup.position.y = -0.2 + Math.abs(Math.sin(phase * 2)) * 0.12;

            // Arm/Leg IK Swings
            leftLegPivot.rotation.x = Math.sin(phase) * 0.75;
            rightLegPivot.rotation.x = -Math.sin(phase) * 0.75;
            leftArmPivot.rotation.x = -Math.sin(phase) * 0.65;
            rightArmPivot.rotation.x = Math.sin(phase) * 0.65;

            // Trolley Motion ahead of Robot
            trolleyGroup.position.x = currentX + 1.8;
            trolleyGroup.position.y = -0.6 + Math.sin(phase * 3) * 0.02;

            // Dynamic items inside trolley
            football.rotation.y += 0.04;
            football.rotation.x += 0.02;
            cashStack.position.y = 0.2 + Math.sin(phase * 2) * 0.03;

            // Camera Tracking Smooth Lerp
            camera.position.x += (currentX * 0.5 - camera.position.x) * 0.08;
        ...
        const progressInterval = setInterval(() => {
            setLoadProgress((prev) => {
                ...
                if (nextVal >= 100) {
                    ...
                    // Final Gate: Execution strictly on hit state 100%
                    setTimeout(() => {
                        playSound("success");
        ...
        <div className="auth-page-wrapper">
            <style>{`
                ...
                * { box-sizing: border-box; margin: 0; padding: 0; }
                ...
                /* SOLID REAL-LIFE ITEMED PANEL DEPT & SHADOWS */
                .auth-card {
                ...
                /* REAL SOLID SEGMENTED CONTROL TABS */
                .nav-switch {
                ...
                /* FUTURISTIC SYNCHRONIZED LOADER OVERLAY */
                #globalProcessLoader {
                    ...
                    <div className="auth-card">
                                ...
                                <form onSubmit={handleRegister} id="registerBox">
                                    ...
                                    <div className="form-group">
                                        ...
                                        <div id="strengthMeter">
                                            ...
                                            <div className="strength-bar" style={{ backgroundColor: passwordScore >= 2 ? "#eab308" : "" }}></div>
                                            <div className="strength-bar" style={{ backgroundColor: passwordScore >= 3 ? "#3b82f6" : "" }}></div>
                                            <div className="strength-bar" style={{ backgroundColor: passwordScore >= 4 ? "#10b981" : "" }}></div>
                                        </div>
                                    </div>
                                    <button type="submit" className="btn-prime" disabled={isLoading}>REGISTER NODE</button>
                                </form>
                            </div>
                        </div>
                        ...
                        <div className="btn-secondary-group">
                            <button type="button" className="btn-alt" onClick={handleGoogleLogin}>
                                CONTINUAL VIA GOOGLE
                            </button>
                            <button type="button" className="btn-alt" onClick={() => { playSound("click"); setIsOtpModalOpen(true); }}>
                                PHONE OTP CODE
                            </button>
                        </div>
                    </div>
                ...
                <div className="premium-modal-overlay">
                    <div className="premium-modal-card">
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                            <h3 style={{ fontFamily: 'Orbitron', fontSize: '16px', color: 'var(--neon-cyan)' }}>PHONE AUTHORIZATION</h3>
                            <button 
                        ...
                        <button type="button" className="btn-alt" style={{ marginBottom: '16px' }} onClick={handleStartOtpFlow}>
                            SEND OTP TOKEN
                        </button>
                        <div className="form-group">
                        ...
                        <div style={{ display: 'flex', gap: '10px' }}>
                            <button type="button" className="btn-prime" onClick={handleVerifyOtpFlow}>VERIFY TOKEN</button>
            ...
            {/* AI CHAT FLOATING LAUNCHER & CONSOLE */}
            <button id="chatLauncherBtn" onClick={openChatConsole} title="Chat Console">
                🤖
            </button>
