...
// Web Audio Synthesizer Engine for Cybernetic Audio Effects
...
export default function Auth() {
    ...
    // Session Check Gate Logic - Check Session FIRST before loading Auth View
    useEffect(() => {
        let isMounted = true;
    ...
    // 4D Cyber Robot Sprint Animation Engine - Speed depends on remaining progress
    useEffect(() => {
        if (!isLoading) return;
        ...
        const renderRobotRunner = () => {
            ctx.clearRect(0, 0, canvas.width, canvas.height);

            const remainingPercent = Math.max(0, 100 - loadProgress);
            // Speed factor: Runs much faster when there is high distance remaining
            const runSpeedMultiplier = 1 + (remainingPercent / 100) * 3.5;
            stepPhase += 0.18 * runSpeedMultiplier;

            const W = canvas.width;
            const H = canvas.height;
            const startX = 40;
            const targetX = W - 50;
            const currentX = startX + (targetX - startX) * (loadProgress / 100);
            const groundY = H - 35;

            // Perspective Grid / Target Line
            ...
            // Finish Target Pillar
            const targetGlow = ctx.createRadialGradient(targetX, groundY - 40, 5, targetX, groundY - 40, 35);
            targetGlow.addColorStop(0, "rgba(239, 68, 68, 0.9)");
            targetGlow.addColorStop(1, "transparent");
            ctx.fillStyle = targetGlow;
            ctx.beginPath();
            ctx.arc(targetX, groundY - 40, 35, 0, Math.PI * 2);
            ctx.fill();
            ...
            ctx.lineTo(targetX, groundY - 70);
            ctx.stroke();

            // Target Finish Ribbon
            ctx.fillStyle = "#ef4444";
            ctx.font = "9px Orbitron, sans-serif";
            ctx.fillText("TARGET LINE", targetX - 28, groundY - 76);

            // Kinetic Speed Thruster Particle Trail
            const trailGlow = ctx.createLinearGradient(currentX - 45, groundY - 20, currentX, groundY - 20);
            trailGlow.addColorStop(0, "transparent");
            trailGlow.addColorStop(1, "rgba(0, 245, 212, 0.6)");
            ctx.fillStyle = trailGlow;
            ctx.fillRect(currentX - 45, groundY - 35, 45, 25);

            // 4D Cyber Robot Body Kinematics
            const bounceY = Math.sin(stepPhase * 2) * 3;
            const robotY = groundY - 35 + bounceY;

            // Leg Swing Angles
            const legAngle1 = Math.sin(stepPhase) * 0.7;
            const legAngle2 = -legAngle1;

            ctx.save();
            ctx.translate(currentX, robotY);

            // Shadow
            ctx.fillStyle = "rgba(0, 0, 0, 0.4)";
            ctx.beginPath();
            ctx.ellipse(0, 35 - bounceY, 16, 4, 0, 0, Math.PI * 2);
            ctx.fill();
            ...
            ctx.beginPath();
            ctx.moveTo(0, 10);
            ctx.lineTo(Math.sin(legAngle2) * 18, 22);
            ctx.lineTo(Math.sin(legAngle2) * 22 + 6, 32);
            ctx.stroke();

            // Torso (Futuristic Cyber Chest)
            ...
            ctx.beginPath();
            ctx.moveTo(0, 10);
            ctx.lineTo(Math.sin(legAngle1) * 18, 22);
            ctx.lineTo(Math.sin(legAngle1) * 22 + 6, 32);
            ctx.stroke();
            ...
            ctx.beginPath();
            ctx.moveTo(0, -5);
            ctx.lineTo(-Math.sin(legAngle1) * 14, 2);
            ctx.stroke();

            ctx.restore();

            animationFrameId = requestAnimationFrame(renderRobotRunner);
        };
        ...
        <div className="auth-page-wrapper">
            <style>{`
                ...
                * { box-sizing: border-box; margin: 0; padding: 0; }
                ...
                /* Futuristic 4D Robot Race Progress Bar Loader */
                #globalProcessLoader {
                ...
                .robot-race-card {
                    width: 420px; max-width: 90vw; background: rgba(15, 23, 42, 0.8);
                    border: 1px solid rgba(0, 245, 212, 0.4); border-radius: 20px; padding: 18px;
                    box-shadow: 0 0 40px rgba(0, 245, 212, 0.15); display: flex; flex-direction: column; align-items: center; gap: 12px;
                }

                .robot-canvas {
                    width: 100%; height: 110px; border-radius: 12px; background: rgba(2, 6, 23, 0.9);
                ...
                <div className="auth-container">
                    <div className="auth-card">
                        ...
                        <div className="btn-secondary-group">
                            ...
                            <button type="button" onClick={() => { playSound("click"); setIsOtpModalOpen(true); }} className="btn-alt">
            ...
            {isOtpModalOpen && (
                <div className="premium-modal-overlay">
                    <div className="premium-modal-card">
                        ...
                        <button onClick={() => { playSound("click"); setIsOtpModalOpen(false); }} className="btn-alt" style={{ background: "transparent", borderColor: "rgba(255,255,255,0.1)", fontSize: "12px", padding: "10px", color: "var(--text-muted)" }}>Abort Verification</button>
                    </div>
                </div>
            )}

            {/* Futuristic 4D Robot Race Progress Bar Overlay */}
            {isLoading && (
                <div id="globalProcessLoader">
                    <div className="robot-race-card">
                        <div className="loader-counter-text">{Math.round(loadProgress)}%</div>
                        <canvas ref={robotCanvasRef} width="380" height="110" className="robot-canvas"></canvas>
            ...
            <div id="aiChatConsole" className={isChatOpen ? "open" : ""}>
                <div className="chat-header">
                    <div className="chat-title">
                        <h3>TERMINAL ASSISTANT</h3>
                        <p>Model: {selectedModel}</p>
                    </div>
                    <button id="closeChatConsole" onClick={() => { playSound("click"); setIsChatOpen(false); }}>&times;</button>
                </div>
                ...
                <div className="chat-input-area">
                    <div style={{ display: 'flex', gap: '8px' }}>
                        <input 
                            type="text" 
                            id="chatUserQuery" 
                            placeholder="Type prompt or use voice..."
                            value={chatQuery}
