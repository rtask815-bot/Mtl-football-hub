...
// Complete Allowed Model Priority List
...
export default function Auth() {
    ...
    // Dynamic Model Tier Auto-Selection State
    ...
    // Handle Mobile Virtual Keyboard visibility dynamically
    useEffect(() => {
    ...
    // Fetch and dynamically pick active model tier from user list
    useEffect(() => {
    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            if (error) throw error;
            showToast("Your Account Has Been Created Successfully. Sign In Now To continue To the Dashboard.", "success");
            setAuthMode("login");
        } catch (err) {
            showToast(err.message, "error");
        } finally {
    ...
    const executeNeuralGrokQuery = async (customPrompt = null) => {
        ...
        try {
            // Standardize payloads into valid roles recognized by OpenAI endpoints
    ...
    const renderListeningOverlay = (targetName) => {
        if (isListening && activeVoiceTarget === targetName) {
            return (
                <div className="floating-listening-overlay">
                    <div className="floating-listening-glow"></div>
                    <div className="floating-listening-content">
                        <div className="floating-listening-text">
            ...
            <style>{`
                ...
                * { box-sizing: border-box; margin: 0; padding: 0; }
                ...
                .auth-card {
                    ...
                    border-radius: 24px; padding: 40px 32px; width: 100%; position: relative; overflow: visible;
                ...
                .floating-listening-overlay {
                    ...
                    z-index: 100;
                    pointer-events: none;
                    animation: overlayFloatPop 0.35s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards;
                }
                @keyframes overlayFloatPop {
                    0% { opacity: 0; transform: translateY(-8px) scale(0.96); }
                    100% { opacity: 1; transform: translateY(0) scale(1); }
                }
                .floating-listening-glow {
                ...
                .floating-listening-content {
                    ...
                    box-shadow: 0 10px 25px rgba(0, 0, 0, 0.5), 0 0 15px rgba(0, 245, 212, 0.25);
                }
                .floating-listening-text {
                    font-size: 11px; font-family: 'Orbitron', sans-serif; color: var(--neon-cyan);
                    text-shadow: 0 0 8px rgba(0, 245, 212, 0.5); letter-spacing: 1px;
                    white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
                }
                .decorated-listening-waves { display: flex; align-items: center; gap: 3px; height: 12px; }
                .decorated-wave-bar {
                    width: 2px; background: var(--neon-cyan); border-radius: 1px;
                    animation: voiceBarDance 0.6s ease-in-out infinite alternate;
                }
                ...
                /* Responsive Fixed/Flexible Mobile Assistant Sheet Layout */
                #aiChatConsole {
                    position: fixed; 
                    bottom: calc(24px + ${keyboardPadding}px); 
                    right: 24px; 
                    width: 380px; 
                    height: 520px;
                    max-width: calc(100vw - 32px); 
                    max-height: calc(100dvh - 100px - ${keyboardPadding}px);
                    ...
                    transition: opacity 0.3s ease, transform 0.3s ease, bottom 0.2s cubic-bezier(0.16, 1, 0.3, 1);
                }
                ...
                /* Feed Container occupies flexible remaining space */
                #chatFeedStream { 
                    flex: 1 1 auto; 
                    padding: 20px; 
                    overflow-y: auto; 
                ...
                .assistant { background: rgba(255, 255, 255, 0.03); border: 1px solid rgba(255, 255, 255, 0.05); align-self: flex-start; color: #f3f4f6; }
                .user { background: linear-gradient(135deg, var(--neon-purple), #1d4ed8); align-self: flex-end; color: #ffffff; }
                .system { display: none; }
                
                /* Flexible sticky input area pin to bottom of panel above keyboard */
                .chat-input-area { 
                    padding: 14px; 
