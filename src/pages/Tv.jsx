...
export default function Tv({ targetWebsite = CONFIG.defaultTargetUrl }) {
    ...
    // Reload stream with Cloudflare Pages Function check and detailed errors
    const reloadStream = useCallback(async (automatic = false) => {
        if (!navigator.onLine) {
            showError("Offline Network", "No active internet connection detected. Please verify your connection.");
            return;
        }
        ...
        if (!baseUrl) {
            showError("Malformed Stream URL", `The configured URL ("${targetWebsite}") is not a valid HTTP/HTTPS web address.`);
            return;
        }
        ...
        if (error || !session) {
            ...
            showError(
                ...
                error ? `Session check failed: ${error.message}` : "Your session has expired. Please log in to continue."
            );
            return;
        }
        ...
        setIsLoaded(false);
        setIsLoading(true);
        setLoadingText(automatic ? "Connecting Cloudflare Proxy..." : "Reloading Stream...");
        setStatusState({
        ...
        // 3. Perform Preflight Ping to `functions/api/proxy.js`
        try {
            let response;
            try {
                // Try lightweight HEAD request first
                response = await fetch(fullProxyUrl, {
                    method: 'HEAD',
                    signal: abortControllerRef.current.signal
                });

                // Fall back to GET if server denies HEAD method (HTTP 405/403)
                ...
                // Network failure or CORS blocked on HEAD, attempt GET fallback
                response = await fetch(fullProxyUrl, {
                    method: 'GET',
                    signal: abortControllerRef.current.signal
                });
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    if (json && json.error) errorDetails = json.error;
                } catch {
                    // Non-JSON response
                }
                ...
                switch (response.status) {
                    case 404:
                        throw new Error(
                            "Cloudflare Function endpoint '/api/proxy' was not found (404). Ensure 'functions/api/proxy.js' is committed to your repo."
                        );
                    case 403:
                        throw new Error(
                            `Access denied (403): ${errorDetails || "The upstream provider or Cloudflare proxy blocked the request."}`
                        );
                    case 502:
                        throw new Error(
                            `Bad Gateway (502): ${errorDetails || "Cloudflare Function was unable to reach the origin stream provider."}`
                        );
                    case 504:
                        throw new Error(
                            `Gateway Timeout (504): ${errorDetails || "The stream provider took too long to respond."}`
                        );
                    default:
                        throw new Error(
                            `Proxy HTTP Error ${response.status}: ${errorDetails || response.statusText || "Unexpected failure."}`
                        );
                }
            }
            ...
            loadTimerRef.current = setTimeout(() => {
                if (!isLoadedRef.current && navigator.onLine) {
                    showError("Stream Load Timeout", "The Cloudflare proxy connected, but the stream media player failed to initialize within 15s.");
                }
            }, CONFIG.loadTimeout);
            ...
            const failureMsg = isTypeError 
                ? "Failed to reach Cloudflare Edge Functions. Verify your network or DNS settings."
                : fetchErr.message;

            showError("Proxy Connection Failed", failureMsg);
        }
    ...
    useEffect(() => {
        ...
        supabase.auth.getSession().then(({ data: { session }, error }) => {
            if (error || !session) {
                setIsAuthenticated(false);
                showError("Authentication Required", error ? error.message : "Please log in to view this stream.");
            } else {
                setIsAuthenticated(true);
                reloadStream(true);
            }
        ...
        <main className="stream-app" id="streamApp" ref={appRef}>
            <style>{`
                ...
                * {
                ...
                .error-message { color: var(--muted); line-height: 1.5; font-size: 13px; word-break: break-word; }
