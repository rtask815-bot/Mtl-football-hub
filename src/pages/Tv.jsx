...
import { supabase } from '../App'; // Adjust path if Supabase client is defined elsewhere
...
export default function MtlTVRoom() {
    ...
    const reloadStream = useCallback(async (automatic = false) => {
        // Verify active Supabase session before starting stream
        if (supabase) {
            const { data: { session } } = await supabase.auth.getSession();
            if (!session) {
                showError("Authentication Error", "Active Supabase session required.");
        ...
        setIsLoaded(false);
        setIsLoading(true);
        setLoadingText(automatic ? "Connecting Stream..." : "Reloading...");
        setStatusState({
