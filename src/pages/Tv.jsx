...
// Credentials fallback with env var support
const SUPABASE_URL = process.env.REACT_APP_SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL || "https://dfcgbwfralikyqxzxlbd.supabase.co";
const SUPABASE_ANON_KEY = process.env.REACT_APP_SUPABASE_ANON_KEY || process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRmY2did2ZyYWxpa3lxeHp4bGJkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODM1NTQwNDUsImV4cCI6MjA5OTEzMDA0NX0.EJM4uRCquMoWRj9VQI-fvfqLhnGM32WbZmipSjLdGA4";

const supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);

const CONFIG = Object.freeze({
    // Cloudflare Pages Function endpoint path
...
export default function Tv({ targetWebsite = CONFIG.defaultTargetUrl }) {
    ...
    // Reload stream with Cloudflare Pages Functions preflight check
    const reloadStream = useCallback(async (automatic = false) => {
        ...
        // 3. Preflight Request to Cloudflare Pages Function
        try {
            ...
            // Fallback to GET if HEAD method is blocked upstream
            ...
            if (!response.ok) {
                if (response.status === 404) {
                    throw new Error("Proxy route missing. Ensure functions/api/proxy.js exists in repository.");
                } else if (response.status === 403) {
                    throw new Error("Access forbidden by Cloudflare security rule or target domain.");
                } else if (response.status === 502 || response.status === 504) {
                    throw new Error("Target stream provider is unreachable or timed out.");
                } else {
                    throw new Error(`Cloudflare Proxy returned status ${response.status}`);
                }
            }

            // Route ping verified: Load into iframe
            ...
            showError("Proxy Connection Error", fetchErr.message || "Failed to establish Cloudflare proxy connection.");
        }
