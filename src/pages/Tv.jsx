...
export default function MtlTVRoom() {
    // Component State
    const [streamSrc, setStreamSrc] = useState(CONFIG.streamURL);
    const [isLoading, setIsLoading] = useState(true);
    const [loadingText, setLoadingText] = useState("Connecting Stream...");
    const [isLoaded, setIsLoaded] = useState(false);
    
    // Status State
    const [statusState, setStatusState] = useState({
        text: "Connecting",
        type: "loading", // "loading" | "connected" | "error" | "offline"
        visible: true
    });
    ...
    // Refs for timer IDs and persistent values across re-renders
    ...
    // Sync ref with state for asynchronous timer checks
    useEffect(() => {
    ...
    // Helper to control controls visibility with auto-hide timeout
    ...
    // Screen and viewport layout calculations
    ...
    // Main Stream Reloading Routine
    const reloadStream = useCallback((automatic = false) => {
        setErrorState(prev => ({ ...prev, visible: false }));
        setIsLoaded(false);
        setIsLoading(true);
        setLoadingText(automatic ? "Reconnecting..." : "Reloading...");
        setStatusState({
        ...
        setStreamSrc(`${CONFIG.streamURL}${separator}_t=${Date.now()}`);
        updateIframeLayout();

        if (loadTimerRef.current) clearTimeout(loadTimerRef.current);
        loadTimerRef.current = setTimeout(() => {
            if (!isLoadedRef.current && navigator.onLine) {
                showError("Timeout", "Provider failed to respond in time.");
            }
        }, CONFIG.loadTimeout);
    }, [updateIframeLayout, showError]);

    // Handle Native iFrame Loaded Callback
    ...
    // Window Listeners Initialization (Online/Offline, Resizing, Initial Stream Start)
    useEffect(() => {
        const handleOnline = () => {
        ...
        // Initial Stream Connect Execution
        updateIframeLayout();
        reloadStream(true);
    ...
    }, [createToast, reloadStream, updateIframeLayout]);
        ...
        <main className="stream-app" id="streamApp" ref={appRef}>
            ...
            <style>{`
                ...
                * {
                ...
                /* 3D Glassmorphism Header Banner */
                .banner-3d {
                    position: absolute;
            ...
            {/* Viewport & Embedded Stream iFrame */}
            ...
            >
                ...
                <button
                    ...
                    onClick={() => window.open("https://famelack.com/tv/sports/rSbwbBDpFexSew", "_blank")}
                >
                    ↗
                </button>
                <button
            ...
            <div id="capabilityInfo" className="capability-info" aria-hidden="true"></div>
        </main>
    );
}
