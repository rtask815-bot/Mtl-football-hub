...
export default function Dashboard() {
    ...
    // Canvas Physics Engine (Realistic Rolling Coins with Continuous Collisions & Periodically Wind-Blown Banknotes)
    useEffect(() => {
        ...
        class MoneyItem {
            constructor(x, y) {
                ...
                this.scaleX = 1; // Used for 3D rotation flip simulation
            }
            ...
            update(wind) {
                ...
                // Boundary walls with restitution (bounce)
                ...
                if (this.y + this.radius > canvas.height) { 
                    ...
                    this.vy *= -0.5; // bounce attenuation
                    this.vx *= 0.9;  // ground friction causing coins to roll
            ...
            draw() {
                ...
                if (this.type === 'coin') {
                    // Draw Gold Coin with metallic sheen
                    ctx.beginPath();
                    ...
                    // Draw Banknote with detailed border
                    ctx.beginPath();
        ...
        // Continuous Elastic Collision Resolver for Coins & Banknotes
            ...
            for (let i = 0; i < items.length; i++) {
                for (let j = i + 1; j < items.length; j++) {
                    ...
                    if (distance < minDist) {
                        ...
                        // Elastic velocity exchange response
                        ...
                        // Add spin impact and friction sparks upon contact
                        item1.spinSpeed += (Math.random() - 0.5) * 0.1;
                        item2.spinSpeed += (Math.random() - 0.5) * 0.1;

                        if (Math.abs(kx) + Math.abs(ky) > 2) {
                            createSparkles((item1.x + item2.x) / 2, (item1.y + item2.y) / 2, '#fde047');
                        }
                    }
                }
            }
        ...
        const animateGame = () => {
            ...
            // Periodic wind gust calculation (cycles every ~250 frames)
            windTimer++;
            if (windTimer % 300 === 0) {
            ...
            spawnTimer++;
            if (spawnTimer % 20 === 0 && items.length < 35) {
                items.push(new MoneyItem(Math.random() * canvas.width, -30));
            }

            // Update physics & handle collisions
            resolveCollisions();
            ...
            <section className="game-section">
                ...
                <div className="dashboard-panel" style={{ marginTop: '14px', position: 'relative', zIndex: 15 }}>
                    <style>{`
                        ...
                        .ai-bullet-list-animated li {
                            animation: slideInFade 0.5s ease forwards;
                            opacity: 0;
                        }

                        .ai-bullet-list-animated li:nth-child(1) { animation-delay: 0.1s; }
                        .ai-bullet-list-animated li:nth-child(2) { animation-delay: 0.2s; }
                        .ai-bullet-list-animated li:nth-child(3) { animation-delay: 0.3s; }
                        .ai-bullet-list-animated li:nth-child(4) { animation-delay: 0.4s; }
                    `}</style>
