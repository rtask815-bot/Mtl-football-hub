...
export default function Dashboard() {
    ...
    // Canvas Physics Engine (Continuous Rain & Liquid Rotation Response)
    useEffect(() => {
        ...
        class Ball {
            constructor(x, y) {
                ...
                this.radius = Math.random() * 4 + 10;
                this.color = '#10b981';
                this.liquidFactor = 1;
            }

            update() {
                // Incorporate device orientation / tilt into liquid flow
                this.vy += 0.2 + (tiltY * 0.08);
                this.vx += tiltX * 0.08;
                this.x += this.vx;
                this.y += this.vy;

                // Wall collisions & damping for liquid feel
                ...
                if (this.y + this.radius > canvas.height) { 
                    ...
                    this.vy *= -0.5; 
                    this.vx *= 0.9;
                    this.liquidFactor = 1.3; // flatten slightly on impact
                    if(Math.abs(this.vy) > 1) {
                        createParticles(this.x, this.y, '#06b6d4');
                    }
                } else {
                    this.liquidFactor += (1 - this.liquidFactor) * 0.1;
                }
            }
            ...
            draw() {
                ...
                ctx.scale(this.liquidFactor, 1 / this.liquidFactor);
        ...
        const animateGame = () => {
            ...
            // Continuous rain spawn
            spawnTimer++;
            if (spawnTimer % 25 === 0 && balls.length < 35) {
    ...
    const navigateTo = (route) => {
        ...
        // Handle custom route redirections if needed
        if (route === 'fixtures') {
            // e.g. window.location.href = '/fixtures';
        }
    };
    ...
    return (
        <div className="dashboard-root">
            ...
            {/* BALL ANIMATION & DISMANTLE GAME SECTION WITH FLOATING HERO */}
            <section className="game-section">
                ...
                <div className="section-header-flex" style={{ marginBottom: '10px', position: 'relative', zIndex: 15 }}>
                    <div className="section-title-badge">⚽ Continuous Rain & Liquid Ball Physics (Tilt or Click)</div>
                    <span style={{ fontSize: '7px', color: 'var(--muted)' }}>Tilt device or click inside box to add interactive fluid balls</span>
                </div>
