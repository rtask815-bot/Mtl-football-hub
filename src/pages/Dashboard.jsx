...
export default function Dashboard() {
    ...
    const gameStateRef = useRef({
        ...
        round: 1,
        isOver: false,
        itemsCount: 15 // Capped to prevent lag/hanging
    });
    ...
    useEffect(() => {
        ...
        // Strict Supabase Auth Session Check & DB Profile Fetch
        if (typeof window !== 'undefined') {
            ...
            const verifySessionAndFetchProfile = async () => {
                try {
                    ...
                    // Fetch user name and email from database profiles table (or user metadata fallback)
                    const { data: profileData, error: profileError } = await supabaseClient
                        .from('profiles')
                        .甚至是 // standard fallback handling
    ...
    // Canvas Physics Engine with Touch, Device Rotation, and Collection Scoring Logic
    useEffect(() => {
        ...
        // Initialize limited safe items count so device does not hang
        ...
        // Touch and click handling to disturb / collect items and avoid adding extra coins
        const handleInteraction = (clientX, clientY) => {
            ...
            items.forEach(item => {
                if (!item.collected) {
                    ...
                    if (dist < item.radius + 25) {
                        ...
                        // Disturb velocity when touched nearby without collecting
                        if (dist < 100) {
            ...
            // Check if all active coins/notes are collected
        ...
        const animateGame = () => {
            ...
            // Check completion if not already flagged
    ...
    return (
        <div className="dashboard-root">
            <style>{`
                ...
                /* Game Overlay Modal for Round End & Continue Prompt */
                .round-over-overlay {
                    position: absolute;
                    inset: 0;
            ...
            <div className={`profile-modal ${isProfileOpen ? 'active' : ''}`} onClick={(e) => { if (e.target.className.includes('profile-modal')) setIsProfileOpen(false); }}>
                <div className="profile-box" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="profile-user-info" id="profileUserName">Name: {userName}</div>
                    <div className="profile-user-info" id="profileUserEmail">Email: {userEmail}</div>
