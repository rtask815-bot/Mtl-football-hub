...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Initial Engine & Sync Startup
    let isMounted = true;

    async function initializeSystem() {
      triggerFloatingLoader("initializing 4d holographic grid...", 10);
      
      const sessionValid = await checkUserSession();
      if (!sessionValid) return;

      triggerFloatingLoader("fetching synchronized neural feeds...", 40);
    ...
    // Live Matches Progress Engine Refresh (every second)
  ...
  // Update Live Matches derived from matchesData state
  useEffect(() => {
    updateLiveMatches();
  }, [matchesData]);

  // 4D Background Canvas Animation Engine Implementation (Three.js WebGL Engine)
  useEffect(() => {
    ...
    // Glowing 4D Torus Knot Geometry
  ...
  async function submitComment() {
    ...
    try {
      ...
      triggerFloatingLoader("posting neural commentary...", 50);
  ...
  async function saveAdminEntry(e) {
    ...
    try {
      triggerFloatingLoader("securing and persisting record...", 60);
      ...
      showToast("Database entry persisted successfully!", false);
  ...
  return (
    <div className="dashboard-root">
      ...
      {/* 4D Background Animation Canvas Container */}
      <canvas ref={canvasRef} id="bg-4d-canvas" />
      <div className="background-grid" />

      {/* Main Dashboard Interactive Layout */}
        ...
        <header className="dashboard-header">
          <div className="logo-box" onClick={() => navigateTo('home')}>
            ...
            <div>
              ...
              <span style={{ fontSize: '10px', color: 'var(--green)', fontWeight: 700 }}>NEURAL PREDICTIONS ENGINE</span>
            </div>
          </div>

          <nav className="nav-links">
            <span className={`nav-link ${activeNav === 'home' ? 'active' : ''}`} onClick={() => navigateTo('home')}>HOME</span>
            <span className={`nav-link ${activeNav === 'fixtures' ? 'active' : ''}`} onClick={() => navigateTo('fixtures')}>FIXTURES</span>
            <span className={`nav-link ${activeNav === 'live' ? 'active' : ''}`} onClick={() => navigateTo('live')}>LIVE INTELLIGENCE</span>
            <span className={`nav-link ${activeNav === 'community' ? 'active' : ''}`} onClick={() => navigateTo('community')}>TELEGRAM CHAT</span>
          </nav>

          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <span style={{ fontSize: '12px', fontFamily: 'monospace', color: 'var(--green)', fontWeight: 700 }}>{clock}</span>
            <button className="btn-outline" onClick={() => openSearchModal()}>🔍 AI Search</button>
            <button className="btn-outline" onClick={() => setIsProfileOpen(true)}>👤 Profile ({userProfile.role.toUpperCase()})</button>
          </div>
        </header>

        {/* FUTURISTIC PROMPT & INTEL BANNER */}
        <section className="cyber-banner">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <span style={{ fontSize: '11px', color: 'var(--green)', fontWeight: 800, letterSpacing: '1px' }}>● REAL-TIME SUPABASE SYNCHRONIZED</span>
              <h2 style={{ fontSize: '24px', fontWeight: 800, marginTop: '4px' }}>Match Intelligence & Holographic Analytics</h2>
              <p style={{ fontSize: '13px', color: 'var(--muted)', marginTop: '4px', maxWidth: '600px' }}>
                Directly connected to live database nodes. Explore match probabilities, upcoming fixtures, and community discussions seamlessly.
              </p>
            </div>
            {isAdmin && (
              <div style={{ display: 'flex', gap: '8px' }}>
                <button className="btn-cyber" onClick={() => setAdminModalState({ open: true, section: 'matches', item: null })}>+ Add Match</button>
                <button className="btn-cyber" onClick={() => setAdminModalState({ open: true, section: 'fixtures', item: null })}>+ Add Fixture</button>
                <button className="btn-cyber" onClick={() => setAdminModalState({ open: true, section: 'trending', item: null })}>+ Add News</button>
              </div>
            )}
          </div>
        </section>

        {/* MAIN 3-COLUMN DIRECT SYNCHRONIZED GRID */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px' }}>
          
          {/* COLUMN 1: MATCHES & PREDICTIONS CONTAINER (SYNCED FROM SUPABASE) */}
          <div style={{ gridColumn: 'span 2', display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div className="pro-card">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button 
                    className={`btn-outline ${activeMatchTab === 'future' ? 'active' : ''}`} 
                    style={{ borderColor: activeMatchTab === 'future' ? 'var(--green)' : 'var(--border)' }}
                    onClick={() => setActiveMatchTab('future')}
                  >
                    Upcoming Matches
                  </button>
                  <button 
                    className={`btn-outline ${activeMatchTab === 'past' ? 'active' : ''}`}
                    style={{ borderColor: activeMatchTab === 'past' ? 'var(--green)' : 'var(--border)' }}
                ...
                <input 
                  ...
                  style={{ background: '#0a1422', border: '1px solid var(--border)', padding: '6px 12px', borderRadius: '8px', color: '#fff', fontSize: '12px' }}
                />
              </div>
                ...
                {getFilteredMatches().length === 0 ? (
                  ...
                  getFilteredMatches().map(match => (
                    <div key={match.id} style={{ background: '#0a1422', border: '1px solid var(--border)', borderRadius: '12px', padding: '16px' }}>
                      ...
                      {/* Interactive Reactions & Admin Actions Bar */}
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '12px', paddingTop: '10px', borderTop: '1px solid var(--border)' }}>
                        <div style={{ display: 'flex', gap: '8px' }}>
                          ...
                          <button className="btn-outline" onClick={() => reactToMatch(match.id, 'dislike')}>👎 {match.reactions?.dislike || 0}</button>
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: '6px' }}>
                          ...
                          {isAdmin && (
                            <>
                              <button className="btn-outline" style={{ color: 'var(--amber)' }} onClick={() => setAdminModalState({ open: true, section: 'matches', item: match })}>Edit</button>
          ...
          {/* COLUMN 2: LIVE MATCHES, FIXTURES & TRENDING FEEDS (SYNCED FROM SUPABASE) */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            
            {/* LIVE MATCHES CONTAINER */}
            <div className="pro-card">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                <h3 style={{ fontSize: '14px', fontWeight: 800 }}>● LIVE MATCHES ({liveMatchesData.length})</h3>
                <span style={{ fontSize: '10px', color: 'var(--red)', fontWeight: 800 }}>LIVE SYNC</span>
              </div>
              {liveMatchesData.length === 0 ? (
                <div style={{ fontSize: '11px', color: 'var(--muted)', textAlign: 'center', padding: '12px' }}>No matches currently live.</div>
              ) : (
                liveMatchesData.map(live => (
                  <div key={live.id} style={{ background: '#0a1422', border: '1px solid var(--border)', padding: '12px', borderRadius: '10px', marginBottom: '8px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', color: 'var(--green)' }}>
                      <span>{live.league}</span>
                      <span>{live.minute}</span>
                    </div>
                    <div style={{ fontSize: '13px', fontWeight: 700, margin: '4px 0' }}>{live.teams}</div>
                    <div className="water-progress-container">
                      <div className="water-progress-bar" style={{ width: `${live.progress}%` }} />
                    </div>
                  </div>
                ))
              )}
            ...
            {/* FIXTURES CONTAINER (SYNCED FROM SUPABASE) */}
            <div className="pro-card">
              <h3 style={{ fontSize: '14px', fontWeight: 800, marginBottom: '12px' }}>📅 UPCOMING FIXTURES</h3>
            ...
            {/* TRENDING FEEDS CONTAINER (SYNCED FROM SUPABASE) */}
            <div className="pro-card">
              <h3 style={{ fontSize: '14px', fontWeight: 800, marginBottom: '12px' }}>🔥 TRENDING FEEDS</h3>
      ...
      {/* GOOGLE FLOATING AI SEARCH MODAL */}
      {isSearchOpen && (
