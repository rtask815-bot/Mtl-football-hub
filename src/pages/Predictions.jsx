...
export default function App() {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        data.forEach(c => {
          ...
          store[mId].push({ id: c.id, user_id: c.user_id, user: c.username || c.user || 'User', comment: c.comment || c.text || '', time: c.created_at ? new Date(c.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'Just now' });
        });
  ...
  async function sendMatchChatMessage() {
    if (!matchChatInput.trim() || !matchChatModal.matchId) { showToast("Match message cannot be empty."); return; }
    try {
  ...
  async function saveAdminEntry() {
    ...
    try {
      ...
      if (adminModal.section === 'matches') {
        if (adminModal.item?.id) { const { error } = await db.from('matches').update(payload).eq('id', adminModal.item.id); err = error; }
        else { const { error } = await db.from('matches').insert([payload]); err = error; }
        await loadMatchesFromDB();
      } else if (adminModal.section === 'fixtures') {
        if (adminModal.item?.id) { const { error } = await db.from('fixtures').update(payload).eq('id', adminModal.item.id); err = error; }
        else { const { error } = await db.from('fixtures').insert([payload]); err = error; }
        await loadFixturesFromDB();
      } else if (adminModal.section === 'trending') {
        if (adminModal.item?.id) { const { error } = await db.from('trending').update(payload).eq('id', adminModal.item.id); err = error; }
        else { const { error } = await db.from('trending').insert([payload]); err = error; }
        await loadTrendingFromDB();
      }
  ...
  function useLongPress(callback, ms = 850) {
    const timerRef = useRef(null);
    const isLongPress = useRef(false);
    const start = (e) => {
      isLongPress.current = false;
      timerRef.current = setTimeout(() => { isLongPress.current = true; callback(e); }, ms);
    };
    const stop = () => { clearTimeout(timerRef.current); };
    return { onMouseDown: start, onMouseUp: stop, onMouseLeave: stop, onTouchStart: start, onTouchEnd: stop, isLongPress: () => isLongPress.current };
  }
  ...
  return (
    <div className={`min-h-screen flex flex-col justify-between selection:bg-emerald-500 selection:text-black bg-slate-950 text-slate-100 font-sans ${userProfile.high_contrast ? 'contrast-125 brightness-110' : ''}`}>
      ...
      <div className="app-content-wrapper flex flex-col min-h-screen justify-between relative z-10">
        ...
        <div id="toast-container" className="fixed top-5 right-5 z-50 flex flex-col gap-2 pointer-events-none">
          {toasts.map(t => (
            <div key={t.id} className={`px-4 py-3 rounded-xl border text-xs font-bold font-mono shadow-2xl flex items-center gap-2 pointer-events-auto transition-all duration-300 ${t.isError ? 'bg-red-950/90 border-red-500/50 text-red-300' : 'bg-emerald-950/90 border-emerald-500/50 text-emerald-300'}`}>
        ...
        {/* Floating Loader */}
        <div className={`fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center transition-opacity duration-300 ${loader.active ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-2xl max-w-xs w-full space-y-4 text-center">
            <div className="flex items-center justify-center gap-3">
              <span className="w-3 h-3 rounded-full bg-emerald-400 animate-ping"></span>
              <h4 className="text-xs font-bold uppercase tracking-widest text-emerald-400 font-mono">QUANTUM SYNC</h4>
            </div>
            <p className="text-sm font-medium text-slate-200">{loader.text}</p>
            <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden">
              <div className="bg-emerald-400 h-full transition-all duration-300" style={{ width: `${loader.progress}%` }}></div>
            </div>
          </div>
        </div>
        ...
        {reactionUsersModal.open && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-emerald-500/30 rounded-2xl w-full max-w-sm p-5 space-y-4 shadow-2xl relative">
              <div className="flex justify-between items-center border-b border-slate-800 pb-3">
                <h4 className="font-bold text-sm text-emerald-400 font-mono">REACTANTS ({reactionUsersModal.type.toUpperCase()})</h4>
                <button onClick={() => setReactionUsersModal({ open: false, type: '', matchId: null })} className="text-slate-400 hover:text-white font-bold">✕</button>
              </div>
              <div className="space-y-2 max-h-60 overflow-y-auto text-xs">
                {(matchReactionsMap[String(reactionUsersModal.matchId)] || []).filter(r => r.reaction === reactionUsersModal.type).length === 0 ? (
                  <p className="text-slate-500 italic py-2">No users have put this reaction yet.</p>
                ) : (
                  (matchReactionsMap[String(reactionUsersModal.matchId)] || []).filter(r => r.reaction === reactionUsersModal.type).map((u, i) => (
                    <div key={i} className="flex items-center gap-2 p-2 bg-slate-950 rounded-lg border border-slate-800">
                      <div className="w-6 h-6 rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center text-[10px] font-bold font-mono">{getFirstNameInitials(u.username)}</div>
                      <span className="text-xs font-semibold text-slate-200">{sanitizeInput(u.username)}</span>
                    </div>
                  ))
                )}
              </div>
              <button onClick={() => setReactionUsersModal({ open: false, type: '', matchId: null })} className="w-full bg-slate-800 border border-slate-700 text-slate-300 py-2 rounded-xl text-xs font-bold hover:text-white">Close</button>
            </div>
          </div>
        )}
        ...
        <aside className={`fixed top-0 right-0 h-full w-80 max-w-[85vw] bg-slate-900 border-l border-slate-800 z-50 transform ${sideNavOpen ? 'translate-x-0' : 'translate-x-full'} transition-transform duration-300 ease-in-out flex flex-col justify-between p-6 shadow-2xl`}>
          <div className="space-y-6">
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center text-sm font-bold font-mono">{getFirstNameInitials(userProfile.username)}</div>
                <div>
              ...
              <button onClick={() => setSideNavOpen(false)} className="w-8 h-8 rounded-full bg-slate-950 text-slate-400 hover:text-white flex items-center justify-center font-bold">✕</button>
            </div>
            <nav className="space-y-3">
              ...
              <button onClick={() => { setDialingModalOpen(true); setSideNavOpen(false); }} className="w-full flex items-center gap-3 p-3 rounded-xl bg-slate-950 border border-slate-800 hover:border-emerald-500 hover:text-emerald-400 transition text-xs font-semibold text-slate-200">
                <span className="text-base">📞</span> Contact Centre
              </button>
              <button onClick={() => { setSettingsModalOpen(true); setSideNavOpen(false); }} className="w-full flex items-center gap-3 p-3 rounded-xl bg-slate-950 border border-slate-800 hover:border-emerald-500 hover:text-emerald-400 transition text-xs font-semibold text-slate-200">
        ...
        {/* Root App Layout Header */}
        <div id="app-root">
          <header className="border-b border-slate-800 bg-slate-900/90 backdrop-blur-md sticky top-0 z-40 px-6 py-4">
            <div className="max-w-7xl mx-auto flex items-center justify-between">
              <div className="flex items-center gap-3">
                <a href="/dashboard" title="Back to Dashboard" className="w-9 h-9 rounded-xl bg-slate-950 border border-slate-800 text-emerald-400 flex items-center justify-center font-bold text-sm hover:bg-emerald-500 hover:text-black transition">⬅️</a>
                <div className="w-10 h-10 rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center text-lg font-bold font-mono cursor-pointer" onClick={() => setDialingModalOpen(true)}>{getFirstNameInitials(userProfile.username)}</div>
                <div>
                  ...
                  <span className="text-[10px] uppercase tracking-widest text-slate-400 font-semibold block">Feel Welcomed.</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                <button onClick={() => setSideNavOpen(true)} title="Open Navigation Options" className="w-10 h-10 rounded-full bg-slate-950 border border-slate-800 text-slate-200 flex items-center justify-center hover:text-emerald-400 transition">☰</button>
                <div className="hidden md:flex items-center gap-3 bg-slate-950 border border-slate-800 px-3 py-1.5 rounded-full cursor-pointer" onClick={() => setSideNavOpen(true)}>
                  <div className="w-7 h-7 rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center text-xs font-bold font-mono">{getFirstNameInitials(userProfile.username)}</div>
          ...
          {/* Banner */}
          <section className="relative overflow-hidden py-12 px-6 border-b border-slate-800 bg-gradient-to-b from-slate-900 to-slate-950">
            <div className="max-w-7xl mx-auto text-center relative z-10">
              ...
              <h2 className="text-3xl lg:text-5xl font-extrabold tracking-tight mt-3 uppercase font-mono text-white">FOOTBALL <span className="text-emerald-400">INTELLIGENCE</span></h2>
              <p className="text-slate-400 text-sm lg:text-base mt-2 max-w-2xl mx-auto font-sans font-medium">Real-time stats, AI match predictions, dynamic hotline dialing and secure encrypted feeds.</p>
            </div>
          </section>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-300 mt-1">{dbError.operation} Failed • Target Table: [{dbError.table}]</p>
                    </div>
                  </div>
                  <button onClick={() => setDbError(null)} className="text-slate-500 hover:text-white">✕</button>
                </div>
                <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="bg-black/20 border border-red-500/10 rounded-xl p-3">
                    <span className="text-[10px] uppercase tracking-wider text-slate-500 block">Diagnostic Reason</span>
                    <p className="text-xs text-red-300 mt-1 font-semibold">{dbError.diagnosis}</p>
                  </div>
                  <div className="bg-black/20 border border-red-500/10 rounded-xl p-3">
                    <span className="text-[10px] uppercase tracking-wider text-slate-500 block">Recommended Fix</span>
                    <p className="text-xs text-slate-300 mt-1">{dbError.action}</p>
                  </div>
                  <div className="bg-black/20 border border-red-500/10 rounded-xl p-3">
                    <span className="text-[10px] uppercase tracking-wider text-slate-500 block">Technical Trace</span>
          ...
          <main className="max-w-7xl mx-auto px-6 py-8 space-y-12">
            ...
            <section id="live-section">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="font-extrabold text-lg uppercase tracking-wide text-white font-mono flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-red-500 animate-pulse"></span> LIVE MATCHES
                  </h3>
                  <span className="text-xs text-slate-400 block">{liveMatchesData.length} Matches Active</span>
                </div>
                <button onClick={() => setStatsListModal({ open: true, title: 'Live Games Directory', dataset: liveMatchesData })} className="bg-slate-900 border border-slate-800 hover:border-emerald-400 text-emerald-400 px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2">
                  <span>SEE MORE MATCHES</span> ➔
                </button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {liveMatchesData.length === 0 ? (
                  <div className="col-span-3 text-center py-8 bg-slate-900 border border-slate-800 rounded-2xl">
                    <p className="text-xs text-slate-400">No live matches currently in play.</p>
                  </div>
                ) : (
                  liveMatchesData.slice(0, 3).map((match) => {
                    ...
                    return (
                      <div key={match.id} onClick={() => setFullscreenMatchModal({ open: true, match })} className="bg-slate-900 border border-slate-800 hover:border-emerald-500/50 rounded-2xl p-5 relative overflow-hidden cursor-pointer transition">
                        <div className="flex justify-between items-center text-xs text-slate-400 mb-3 font-semibold">
                          <span className="font-mono hover:text-emerald-400" onClick={(e) => { e.stopPropagation(); openGoogleSearchIframe(`Match live summary ${match.teams}`); }}>{sanitizeInput(match.league)}</span>
                          <span className="text-red-500 font-bold animate-pulse">● LIVE</span>
                        </div>
                        <div className="flex items-center justify-between my-4">
                          <div className="text-center flex-1">
                            <div className="w-10 h-10 mx-auto rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center mb-1 font-mono text-xs font-bold">{getTeamBadge(home)}</div>
                            <span className="text-xs font-bold tracking-wide text-white block">{sanitizeInput(home)}</span>
                          </div>
                          <div className="text-2xl font-extrabold tracking-wider px-2 font-mono text-emerald-400">- _ -</div>
                          <div className="text-center flex-1">
                            <div className="w-10 h-10 mx-auto rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center mb-1 font-mono text-xs font-bold">{getTeamBadge(away)}</div>
                            <span className="text-xs font-bold tracking-wide text-white block">{sanitizeInput(away)}</span>
                          </div>
                        </div>
                        ...
                        <div className="w-full bg-slate-950 h-2 rounded-full overflow-hidden mb-3">
                          <div className="bg-emerald-400 h-full transition-all" style={{ width: `${match.progress}%` }}></div>
                        </div>
            ...
            <section id="db-matches-section" className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                <div>
                  <h3 className="text-lg font-extrabold uppercase tracking-wide text-white font-mono cursor-pointer hover:text-emerald-400 transition" onClick={() => openGoogleSearchIframe('Live database matches and football predictions')}>⚽ MATCHES & PREDICTIONS</h3>
                  <p className="text-xs text-slate-400 mt-0.5">Hold match cards long press to trigger Google Search.</p>
                </div>
                <button onClick={() => setStatsListModal({ open: true, title: 'All Database Predictions', dataset: matchesData })} className="bg-slate-950 border border-slate-800 hover:border-emerald-400 text-emerald-400 px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2">
                  <span>SEE MORE</span> ➔
                </button>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 mb-6 border-b border-slate-800 pb-4">
                <div className="flex items-center gap-2 bg-slate-950 p-1.5 rounded-xl border border-slate-800 self-start">
                  <button onClick={() => setActiveMatchTab('future')} className={`px-4 py-1.5 rounded-lg text-xs font-bold font-mono transition ${activeMatchTab === 'future' ? 'bg-emerald-400 text-black' : 'text-slate-400 hover:text-white'}`}>UPCOMING MATCHES</button>
                  <button onClick={() => setActiveMatchTab('past')} className={`px-4 py-1.5 rounded-lg text-xs font-bold font-mono transition ${activeMatchTab === 'past' ? 'bg-emerald-400 text-black' : 'text-slate-400 hover:text-white'}`}>PAST PREDICTIONS</button>
                </div>

                <div className="relative flex-1 max-w-md">
                  <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400 text-xs">🔍</span>
                  <input type="text" value={matchSearchQuery} onChange={(e) => setMatchSearchQuery(e.target.value)} placeholder="Search match by name, date (YYYY-MM-DD), or time..." className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-8 pr-4 py-2 text-xs text-white focus:outline-none focus:border-emerald-400" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {getFilteredMatches().length === 0 ? (
                  <div className="col-span-3 text-center py-10 bg-slate-950 border border-slate-800 rounded-2xl">
                    <p className="text-xs text-slate-400">No matches found for query "{matchSearchQuery}".</p>
                  </div>
                ) : (
                  getFilteredMatches().slice(0, 3).map((match) => {
                    ...
                    return (
                      <div key={match.id} className="bg-slate-950 border border-slate-800 hover:border-slate-700 rounded-2xl p-5 flex flex-col justify-between space-y-4 transition">
                        <div className="space-y-2">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[10px] text-slate-400 font-mono">{sanitizeInput(match.match_date)} {sanitizeInput(match.match_time)}</span>
                            </div>
                          </div>
                          <h4 className="font-extrabold text-base text-white tracking-wide font-mono hover:text-emerald-400 cursor-pointer" onClick={() => openGoogleSearchIframe(`Prediction summary for ${match.teams}`)}>{sanitizeInput(match.teams)}</h4>
                          <p className="text-xs text-emerald-400 font-semibold">Prediction: {sanitizeInput(match.prediction)} ({stars})</p>
                          <p className="text-xs text-slate-400 line-clamp-2">{sanitizeInput(match.analysis_text || 'Tactical breakdown in detailed view.')}</p>
                        </div>
                        ...
                        <div className="space-y-1 bg-slate-900 p-3 rounded-xl border border-slate-800">
                          ...
                          <div className="w-full bg-slate-950 h-1.5 rounded-full overflow-hidden">
                            <div className="bg-emerald-400 h-full" style={{ width: `${match.prob_home}%` }}></div>
                          </div>
                        </div>

                        {/* Reactions Row */}
                        <div className="flex items-center gap-2 pt-1">
                          <button onClick={() => reactToMatch(match.id, 'fire')} className="bg-slate-900 border border-slate-800 px-2.5 py-1 rounded-lg text-xs hover:border-emerald-400 flex items-center gap-1 transition">🔥 <span>{match.reactions?.fire || 0}</span></button>
                          <button onClick={() => reactToMatch(match.id, 'heart')} className="bg-slate-900 border border-slate-800 px-2.5 py-1 rounded-lg text-xs hover:border-emerald-400 flex items-center gap-1 transition">❤️ <span>{match.reactions?.heart || 0}</span></button>
                          <button onClick={() => reactToMatch(match.id, 'dislike')} className="bg-slate-900 border border-slate-800 px-2.5 py-1 rounded-lg text-xs hover:border-emerald-400 flex items-center gap-1 transition">👎 <span>{match.reactions?.dislike || 0}</span></button>
                        </div>

                        {/* Comments Preview Container */}
                        <div onClick={() => setFullscreenCommentsModal({ open: true, matchId: match.id, teams: match.teams })} className="bg-slate-900 rounded-xl p-3 space-y-2 border border-slate-800 hover:border-emerald-400 transition cursor-pointer">
                          <div className="flex justify-between items-center text-[11px] font-bold text-slate-300">
                            <span>💬 Comments ({comments.length})</span>
                            <span className="text-emerald-400 text-[10px] uppercase font-bold">🖥️ Fullscreen View ➔</span>
                          </div>
                          <div className="space-y-1.5 max-h-20 overflow-y-auto text-[11px]">
                            ...
                            {comments.slice(-2).map((c) => (
                              <div key={c.id} className="bg-slate-950 p-1.5 rounded border border-slate-800 text-slate-300">
                                <span className="font-bold text-emerald-400">{sanitizeInput(c.user)}:</span> {sanitizeInput(c.comment)}
                              </div>
                            ))}
                          </div>
                        </div>
                        ...
                        <div className="pt-2 border-t border-slate-800 flex flex-wrap items-center justify-between gap-2">
                          <div className="flex items-center gap-2">
                            <button onClick={() => setFullscreenMatchModal({ open: true, match })} className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500 hover:text-black font-bold px-3 py-1.5 rounded-xl text-xs transition">🔍 Details</button>
                            <button onClick={() => setMatchChatModal({ open: true, matchId: match.id, teams: match.teams })} className="bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-xl text-xs text-slate-200 hover:text-emerald-400 transition flex items-center gap-1">💬 Telegram Chat</button>
                          </div>
                          {userProfile.role === 'admin' && (
              ...
              {userProfile.role === 'admin' && (
                <div className="mt-6 pt-4 border-t border-slate-800 flex justify-center">
                  <button onClick={() => openAdminModal('matches')} className="bg-emerald-400 text-black font-extrabold px-6 py-2.5 rounded-full text-xs uppercase tracking-wider hover:bg-emerald-300 transition flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              {/* Upcoming Fixtures */}
              <section id="fixtures-section" className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col justify-between space-y-4 shadow-2xl">
                <div>
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      <h3 className="font-extrabold uppercase tracking-wide text-base text-white font-mono cursor-pointer hover:text-emerald-400 transition" onClick={() => openGoogleSearchIframe('Upcoming football fixtures schedule')}>UPCOMING FIXTURES</h3>
                      <span className="text-xs text-slate-400 block">Upcoming fixtures</span>
                    </div>
                    <button onClick={() => setStatsListModal({ open: true, title: 'Complete Fixtures Schedule', dataset: fixturesData })} className="bg-slate-950 border border-slate-800 hover:border-emerald-400 text-emerald-400 px-3.5 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2">
                      <span>SEE MORE</span> ➔
                    </button>
                  </div>
                  ...
                  <div className="space-y-4">
                    {fixturesData.length === 0 ? (
                      <div className="text-center py-8 text-xs text-slate-500">Loading fixtures...</div>
                    ) : (
                      fixturesData.slice(0, 3).map((fix) => (
                        <div key={fix.id} className="bg-slate-950 border border-slate-800 rounded-xl p-4 flex items-center justify-between cursor-pointer hover:border-slate-700 transition">
                          <div className="flex items-center gap-3">
                            <div className="w-9 h-9 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-center font-bold text-xs text-emerald-400 font-mono">{sanitizeInput(fix.badge)}</div>
                            <div>
                              <h4 className="font-bold text-xs text-white hover:text-emerald-400" onClick={() => openGoogleSearchIframe(`Fixture schedule ${fix.teams}`)}>{sanitizeInput(fix.teams)}</h4>
                              <span className="text-[10px] text-slate-400 block">{sanitizeInput(fix.league)}</span>
                            </div>
                          </div>
                          <div className="flex items-center gap-3">
                            <div className="text-right">
                              <span className="text-xs font-bold text-emerald-400 block font-mono">{sanitizeInput(fix.match_time)}</span>
                              <span className="text-[10px] text-slate-500 block">{sanitizeInput(fix.match_date)}</span>
                            </div>
                            {userProfile.role === 'admin' && (
                              <div className="flex gap-1 ml-2">
                ...
                {userProfile.role === 'admin' && (
                  <div className="pt-4 border-t border-slate-800 flex justify-center">
                    <button onClick={() => openAdminModal('fixtures')} className="bg-slate-950 border border-emerald-500/40 text-emerald-400 font-bold px-5 py-2 rounded-xl text-xs hover:bg-emerald-500 hover:text-black transition flex items-center gap-2">
              ...
              {/* Trending News */}
              <section id="trending-section" className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col justify-between space-y-4 shadow-2xl">
                <div>
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      <h3 className="font-extrabold uppercase tracking-wide text-base text-white font-mono cursor-pointer hover:text-emerald-400 transition" onClick={() => openGoogleSearchIframe('Trending football news updates')}>🔥 TRENDING NEWS</h3>
                      <span className="text-xs text-slate-400 block">What's trending.</span>
                    </div>
                    <button onClick={() => setStatsListModal({ open: true, title: 'All Trending News', dataset: trendingData })} className="bg-slate-950 border border-slate-800 hover:border-emerald-400 text-emerald-400 px-3.5 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2">
                      <span>SEE MORE</span> ➔
                    </button>
                  </div>
                  ...
                  <div className="space-y-4">
                    {trendingData.length === 0 ? (
                      <div className="text-center py-8 text-xs text-slate-500">Loading news...</div>
                    ) : (
                      trendingData.slice(0, 3).map((item) => (
                        <div key={item.id} className="bg-slate-950 border border-slate-800 rounded-xl p-4 flex items-center justify-between cursor-pointer hover:border-slate-700 transition">
                          <div className="flex items-center gap-3">
                            <span className="text-sm font-extrabold text-emerald-400 font-mono">#{sanitizeInput(item.rank)}</span>
                            <div>
                              <h4 className="font-bold text-xs text-white hover:text-emerald-400" onClick={() => openGoogleSearchIframe(item.title)}>{sanitizeInput(item.title)}</h4>
                              <span className="text-[10px] text-slate-500 block">💬 {Number(item.comments_count) || 6237} discussions</span>
                            </div>
                          </div>
                          {userProfile.role === 'admin' && (
                ...
                {userProfile.role === 'admin' && (
                  <div className="pt-4 border-t border-slate-800 flex justify-center">
                    <button onClick={() => openAdminModal('trending')} className="bg-slate-950 border border-emerald-500/40 text-emerald-400 font-bold px-5 py-2 rounded-xl text-xs hover:bg-emerald-500 hover:text-black transition flex items-center gap-2">
                      <span>➕</span> ADD NEWS
                    </button>
                  </div>
                )}
          ...
          {/* Footer */}
          <footer className="border-t border-slate-800 bg-slate-900 mt-16 py-8 px-6 text-center text-xs text-slate-400">
            <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-6">
                ...
                <button onClick={() => setDialingModalOpen(true)} className="hover:text-emerald-400">Developed BY M. Lennox</button>
              </div>
            </div>
          </footer>
        </div>
        ...
        {googleIframeModal.open && (
          <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex flex-col p-3 sm:p-6">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-3 mb-3 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-emerald-400 text-black font-extrabold flex items-center justify-center font-mono">AI</div>
                <div>
                  <h4 className="text-xs font-bold font-mono text-emerald-400">GOOGLE QUICK SEARCH</h4>
                  <p className="text-[10px] text-slate-400 font-mono">Automated AI Mode: "{googleIframeModal.query}"</p>
                </div>
              </div>
        ...
        <div className="fixed bottom-6 right-6 z-40">
          <button onClick={() => setGlobalChatOpen(!globalChatOpen)} className="w-14 h-14 rounded-full bg-emerald-400 text-black flex items-center justify-center text-2xl font-bold shadow-lg hover:scale-105 transition transform">💬</button>
          
          {globalChatOpen && (
            <div className="absolute bottom-20 right-0 w-80 sm:w-96 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl flex flex-col h-[480px] overflow-hidden">
              ...
              <div className="flex-1 p-4 overflow-y-auto flex flex-col space-y-3 text-xs">
                {globalChatMessages.length === 0 ? (
                  <div className="text-center text-slate-500 text-xs py-8">Welcome to Telegram global chat!</div>
                ) : (
                  globalChatMessages.map(msg => {
                    ...
                    return (
                      <div key={msg.id} className={`flex flex-col max-w-[80%] ${isMe ? 'self-end items-end' : 'self-start items-start'}`}>
                        ...
                        <div className={`px-3.5 py-2 text-xs rounded-2xl ${isMe ? 'bg-emerald-500/20 text-emerald-200 border border-emerald-500/30 rounded-br-none' : 'bg-slate-950 text-slate-200 border border-slate-800 rounded-bl-none'}`}>
              ...
              <div className="p-3 border-t border-slate-800 bg-slate-950 flex gap-2">
                <input type="text" value={globalChatInput} onChange={(e) => setGlobalChatInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && sendGlobalChatMessage()} placeholder="Type Telegram message..." className="flex-1 bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-emerald-400" />
                <button onClick={sendGlobalChatMessage} className="bg-emerald-400 text-black font-bold px-4 py-2 rounded-xl text-xs hover:bg-emerald-300 transition">Send</button>
              </div>
            </div>
          )}
        </div>
        ...
        {fullscreenCommentsModal.open && (
          <div className="fixed inset-0 bg-slate-950/95 backdrop-blur-xl z-50 p-4 md:p-10 overflow-y-auto flex flex-col justify-between">
            <div className="max-w-4xl w-full mx-auto bg-slate-900 border border-emerald-500/40 rounded-3xl p-6 md:p-8 shadow-2xl relative flex-1 flex flex-col justify-between space-y-6">
              <div className="flex items-center justify-between border-b border-slate-800 pb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-2xl bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 flex items-center justify-center font-bold text-lg font-mono">💬</div>
                  <div>
                    <h3 className="text-lg md:text-xl font-extrabold text-white font-mono">Comments Stream: {fullscreenCommentsModal.teams}</h3>
                    <p className="text-xs text-emerald-400">Leave a comment.</p>
                  </div>
                </div>
                <button onClick={() => setFullscreenCommentsModal({ open: false, matchId: null, teams: '' })} className="w-10 h-10 rounded-full bg-slate-950 border border-slate-800 text-slate-400 hover:text-white flex items-center justify-center text-lg font-bold">✕</button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto space-y-4 pr-2 max-h-[60vh]">
                {(matchCommentsStore[fullscreenCommentsModal.matchId] || []).length === 0 ? (
                  <div className="text-center text-slate-500 py-12 text-xs font-medium">No comments posted for this match yet. Be the first to share analysis!</div>
                ) : (
                    ...
                    return (
                      <div key={c.id} className="bg-slate-950 border border-slate-800 p-4 rounded-2xl space-y-2 flex gap-3 items-start">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-between">
                            <span className="font-bold text-xs text-emerald-400 font-mono">{sanitizeInput(c.user)}</span>
                            <div className="flex items-center gap-2">
                              <span className="text-[10px] text-slate-500">{sanitizeInput(c.time)}</span>
                              {canEdit && (
                                <>
                          ...
                          <p className="text-xs text-slate-200 mt-1 leading-relaxed">{sanitizeInput(c.comment)}</p>
                        </div>
                      </div>
                    );
                  })
              ...
              <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3">
                ...
                <div className="flex gap-3">
                  <textarea rows={2} value={fullscreenCommentInput} onChange={(e) => setFullscreenCommentInput(e.target.value)} placeholder="Write detailed comment to be recorded in database..." className="flex-1 bg-slate-900 border border-slate-800 rounded-xl p-3 text-xs text-white focus:outline-none focus:border-emerald-400" />
                  <button onClick={submitFullscreenComment} className="bg-emerald-400 text-black font-extrabold px-6 py-2 rounded-xl text-xs hover:bg-emerald-300 transition self-end">Post Comment</button>
                </div>
              </div>

              <div className="flex justify-end pt-2 border-t border-slate-800">
                <button onClick={() => setFullscreenCommentsModal({ open: false, matchId: null, teams: '' })} className="bg-slate-950 border border-slate-800 hover:border-emerald-400 text-emerald-400 px-4 py-2 rounded-xl text-xs font-bold transition">Close</button>
              </div>
            </div>
          </div>
        )}
        ...
        {statsListModal.open && (
          <div className="fixed inset-0 bg-slate-950/95 backdrop-blur-md z-50 overflow-y-auto p-4 md:p-10">
            <div className="max-w-5xl mx-auto bg-slate-900 border border-slate-800 rounded-2xl p-6 md:p-10 shadow-2xl relative min-h-[85vh] flex flex-col justify-between">
              <button onClick={() => setStatsListModal({ open: false, title: '', dataset: [] })} className="absolute top-6 right-6 w-10 h-10 rounded-full bg-slate-950 border border-slate-800 text-slate-400 hover:text-white flex items-center justify-center text-lg font-bold">✕</button>
              <div className="space-y-6">
                <div className="border-b border-slate-800 pb-4">
                  <h3 className="text-2xl font-extrabold text-emerald-400 uppercase tracking-wider font-mono">{statsListModal.title}</h3>
                  ...
                  {statsListModal.dataset.length === 0 ? (
                    ...
                    statsListModal.dataset.map((item, idx) => (
                      <div key={idx} className="bg-slate-950 border border-slate-800 rounded-xl p-4 space-y-2">
                        <h4 className="font-bold text-white text-sm font-mono">{sanitizeInput(item.teams || item.title || 'Item')}</h4>
                        {item.prediction && <p className="text-xs text-emerald-400 font-semibold">Pred: {item.prediction}</p>}
                        {item.league && <p className="text-xs text-slate-400">{item.league}</p>}
                      </div>
                    ))
                  )}
                </div>
              ...
              <div className="mt-8 pt-6 border-t border-slate-800 flex justify-end">
                <button onClick={() => setStatsListModal({ open: false, title: '', dataset: [] })} className="bg-slate-950 border border-slate-800 hover:border-emerald-400 text-emerald-400 px-4 py-2 rounded-xl text-xs font-bold transition">Close</button>
              </div>
            </div>
          </div>
        )}
        ...
        {fullscreenMatchModal.open && fullscreenMatchModal.match && (
          <div className="fixed inset-0 bg-slate-950/95 backdrop-blur-md z-50 overflow-y-auto p-4 md:p-10">
            <div className="max-w-5xl mx-auto bg-slate-900 border border-slate-800 rounded-2xl p-6 md:p-10 shadow-2xl relative min-h-[85vh] flex flex-col justify-between">
              <button onClick={() => setFullscreenMatchModal({ open: false, match: null })} className="absolute top-6 right-6 w-10 h-10 rounded-full bg-slate-950 border border-slate-800 text-slate-400 hover:text-white flex items-center justify-center text-lg font-bold">✕</button>
              
              <div className="space-y-8">
                <div className="flex justify-between items-start border-b border-slate-800 pb-6">
                  <div>
                    <span className="px-3 py-1 rounded-full text-xs font-extrabold uppercase bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">{fullscreenMatchModal.match.type || 'free'} INTEL</span>
                    <h2 className="text-3xl lg:text-4xl font-extrabold text-white mt-2 font-mono hover:text-emerald-400 cursor-pointer" onClick={() => openGoogleSearchIframe(`Live analysis ${fullscreenMatchModal.match.teams}`)}>{sanitizeInput(fullscreenMatchModal.match.teams)}</h2>
                    <p className="text-xs text-slate-400 mt-1 font-mono">Date: {fullscreenMatchModal.match.match_date} | Kickoff: {fullscreenMatchModal.match.match_time}</p>
                  </div>
                  <div className="text-right">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800 space-y-2">
                    ...
                    <p className="text-xl font-extrabold text-white font-mono">{sanitizeInput(fullscreenMatchModal.match.prediction || 'N/A')}</p>
                  </div>
                  <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800 space-y-2">
                    <span className="text-xs text-amber-400 font-bold uppercase tracking-wider font-mono">Decimal Odds</span>
                    <p className="text-xl font-extrabold text-white font-mono">{formatOdds(fullscreenMatchModal.match.decimal_odds)}</p>
                  </div>
                  <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800 space-y-2">
                    <span className="text-xs text-blue-400 font-bold uppercase tracking-wider font-mono">Status & Score</span>
                    <p className="text-xl font-extrabold text-white font-mono">{sanitizeInput(fullscreenMatchModal.match.status || 'PENDING')} ({sanitizeInput(fullscreenMatchModal.match.final_score || 'Awaiting')})</p>
                  </div>
                </div>

                <div className="bg-slate-950 p-6 rounded-2xl border border-slate-800 space-y-3">
                  <h4 className="font-extrabold text-sm uppercase tracking-wider text-emerald-400 font-mono">Tactical Intelligence & Match Analysis</h4>
                  <p className="text-sm text-slate-300 leading-relaxed">{sanitizeInput(fullscreenMatchModal.match.analysis_text || 'No tactical analysis available.')}</p>
                </div>
              </div>

              <div className="mt-8 pt-6 border-t border-slate-800 flex justify-between items-center">
                <div className="flex items-center gap-3">
                  <button onClick={() => { setMatchChatModal({ open: true, matchId: fullscreenMatchModal.match.id, teams: fullscreenMatchModal.match.teams }); setFullscreenMatchModal({ open: false, match: null }); }} className="bg-slate-950 border border-slate-800 text-slate-200 px-4 py-2 rounded-xl text-xs hover:text-emerald-400 flex items-center gap-2">💬 Open Chat</button>
                  <button onClick={() => setDialingModalOpen(true)} className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 px-4 py-2 rounded-xl text-xs font-bold hover:bg-emerald-500 hover:text-black transition">📞 Call</button>
                </div>
                <button onClick={() => setFullscreenMatchModal({ open: false, match: null })} className="bg-slate-950 border border-slate-800 hover:border-emerald-400 text-emerald-400 px-4 py-2 rounded-xl text-xs font-bold transition">Close</button>
              </div>
            </div>
          </div>
        )}
        ...
        {matchChatModal.open && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl h-[80vh] flex flex-col overflow-hidden shadow-2xl">
              <div className="bg-slate-950 p-4 border-b border-slate-800 flex items-center justify-between">
                <div>
                  <h4 className="font-bold text-sm text-emerald-400 font-mono">Telegram Match Thread: {matchChatModal.teams}</h4>
                  <p className="text-[10px] text-slate-400">Match discussion Group</p>
                </div>
                <button onClick={() => setMatchChatModal({ open: false, matchId: null, teams: '' })} className="text-slate-400 hover:text-white font-bold text-lg">✕</button>
              </div>
              
              <div className="flex-1 p-4 overflow-y-auto flex flex-col space-y-3 text-xs">
                {(matchChatStore[matchChatModal.matchId] || []).length === 0 ? (
                  <div className="text-center text-slate-500 text-xs py-8">No messages in this match chat thread yet.</div>
                ) : (
              ...
              <div className="p-3 border-t border-slate-800 bg-slate-950 flex gap-2">
                <input type="text" value={matchChatInput} onChange={(e) => setMatchChatInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && sendMatchChatMessage()} placeholder="Discuss this match..." className="flex-1 bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-emerald-400" />
                <button onClick={sendMatchChatMessage} className="bg-emerald-400 text-black font-bold px-4 py-2 rounded-xl text-xs hover:bg-emerald-300 transition">Post</button>
              </div>
            </div>
          </div>
        )}
        ...
        {dialingModalOpen && (
          <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl p-6 md:p-8 space-y-6 shadow-2xl">
              <div className="flex justify-between items-center border-b border-slate-800 pb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 flex items-center justify-center text-xl">📞</div>
                  <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2 flex flex-col justify-between">
                  ...
                  <a href="https://wa.me/254716883895" target="_blank" rel="noopener noreferrer" className="w-full bg-emerald-400 text-black text-center text-xs font-bold py-2.5 rounded-xl hover:bg-emerald-300 transition mt-3 block">💬 WhatsApp +254716883895</a>
                </div>
                <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2 flex flex-col justify-between">
                  <div>
                  ...
                  <a href="tel:+254716883895" className="w-full bg-amber-500/10 border border-amber-500/30 text-amber-400 text-center text-xs font-bold py-2.5 rounded-xl hover:bg-amber-500 hover:text-black transition mt-3 block">📞 Call +254716883895</a>
                </div>
              </div>
          ...
          <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md p-6 space-y-6 shadow-2xl">
              ...
              <div className="space-y-4 text-xs">
                <div>
                  <label className="block text-slate-400 mb-1">ODDS FORMAT</label>
                  <select value={userProfile.odds_format} onChange={(e) => setUserProfile(prev => ({ ...prev, odds_format: e.target.value }))} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-white focus:outline-none focus:border-emerald-400">
                ...
                <div>
                  ...
                  <select value={userProfile.language} onChange={(e) => setUserProfile(prev => ({ ...prev, language: e.target.value }))} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-white focus:outline-none focus:border-emerald-400">
                    <option value="en">English (EN)</option>
                    <option value="sw">Swahili (SW)</option>
                  </select>
                </div>
                <div className="flex items-center justify-between pt-2">
                  <span className="text-slate-300">CONTRAST MODE</span>
                  <input type="checkbox" checked={userProfile.high_contrast} onChange={(e) => setUserProfile(prev => ({ ...prev, high_contrast: e.target.checked }))} className="w-4 h-4 accent-emerald-400" />
                </div>
              </div>
              <button onClick={() => setSettingsModalOpen(false)} className="w-full bg-emerald-400 text-black font-bold py-2 rounded-xl text-xs hover:bg-emerald-300 transition">Save & Close</button>
            </div>
          </div>
        )}
        ...
        {adminModal.open && (
          <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl p-8 space-y-6 shadow-2xl relative">
              <div className="flex justify-between items-center border-b border-slate-800 pb-4">
                <div>
                  <h3 className="text-xl font-extrabold text-emerald-400 uppercase tracking-wider font-mono">{adminModal.item ? `Edit ${adminModal.section}` : `Add ${adminModal.section}`}</h3>
                  <p className="text-xs text-slate-400">Insert or update Record</p>
                </div>
                <button onClick={() => setAdminModal({ open: false, section: null, item: null })} className="text-slate-400 hover:text-white font-bold text-xl">✕</button>
              </div>
              
              <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                {adminModal.section === 'matches' && (
                  <>
                    <input type="text" placeholder="Teams (e.g. Chelsea vs Arsenal)" value={adminFormData.teams || ''} onChange={(e) => setAdminFormData(prev => ({ ...prev, teams: e.target.value }))} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white" />
                    <input type="text" placeholder="League / Competition" value={adminFormData.league || ''} onChange={(e) => setAdminFormData(prev => ({ ...prev, league: e.target.value }))} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white" />
                    <div className="grid grid-cols-2 gap-3">
                      <input type="date" value={adminFormData.match_date || ''} onChange={(e) => setAdminFormData(prev => ({ ...prev, match_date: e.target.value }))} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white" />
                      <input type="time" value={adminFormData.match_time || ''} onChange={(e) => setAdminFormData(prev => ({ ...prev, match_time: e.target.value }))} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white" />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <input type="text" placeholder="Prediction (e.g. Home Win)" value={adminFormData.prediction || ''} onChange={(e) => setAdminFormData(prev => ({ ...prev, prediction: e.target.value }))} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white" />
                      <input type="number" step="0.01" placeholder="Odds (e.g. 2.10)" value={adminFormData.decimal_odds || ''} onChange={(e) => setAdminFormData(prev => ({ ...prev, decimal_odds: e.target.value }))} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white" />
                    </div>
                    <div className="grid grid-cols-3 gap-3">
                      <input type="number" placeholder="Home Prob %" value={adminFormData.prob_home || ''} onChange={(e) => setAdminFormData(prev => ({ ...prev, prob_home: e.target.value }))} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white" />
                      <input type="number" placeholder="Draw Prob %" value={adminFormData.prob_draw || ''} onChange={(e) => setAdminFormData(prev => ({ ...prev, prob_draw: e.target.value }))} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white" />
                      <input type="number" placeholder="Away Prob %" value={adminFormData.prob_away || ''} onChange={(e) => setAdminFormData(prev => ({ ...prev, prob_away: e.target.value }))} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white" />
                    </div>
                    <textarea placeholder="Tactical intelligence summary..." value={adminFormData.analysis_text || ''} onChange={(e) => setAdminFormData(prev => ({ ...prev, analysis_text: e.target.value }))} rows={3} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white" />
                  </>
                )}

                {adminModal.section === 'fixtures' && (
                  <>
                    <input type="text" placeholder="Teams" value={adminFormData.teams || ''} onChange={(e) => setAdminFormData(prev => ({ ...prev, teams: e.target.value }))} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white" />
                    <input type="text" placeholder="League" value={adminFormData.league || ''} onChange={(e) => setAdminFormData(prev => ({ ...prev, league: e.target.value }))} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white" />
                    <div className="grid grid-cols-2 gap-3">
                      <input type="date" value={adminFormData.match_date || ''} onChange={(e) => setAdminFormData(prev => ({ ...prev, match_date: e.target.value }))} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white" />
                      <input type="time" value={adminFormData.match_time || ''} onChange={(e) => setAdminFormData(prev => ({ ...prev, match_time: e.target.value }))} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white" />
                    </div>
                ...
                {adminModal.section === 'trending' && (
                  <>
                    <input type="number" placeholder="Rank Number (e.g. 1)" value={adminFormData.rank || ''} onChange={(e) => setAdminFormData(prev => ({ ...prev, rank: e.target.value }))} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white" />
                    <input type="text" placeholder="Headline Title" value={adminFormData.title || ''} onChange={(e) => setAdminFormData(prev => ({ ...prev, title: e.target.value }))} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white" />
                    <input type="number" placeholder="Comments count" value={adminFormData.comments_count || ''} onChange={(e) => setAdminFormData(prev => ({ ...prev, comments_count: e.target.value }))} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white" />
                  </>
                )}
              </div>

              <div className="flex justify-end gap-4 border-t border-slate-800 pt-4">
                <button onClick={() => setAdminModal({ open: false, section: null, item: null })} className="px-5 py-2.5 rounded-xl text-xs bg-slate-800 text-slate-300 hover:bg-slate-700">Cancel</button>
                <button onClick={saveAdminEntry} className="px-6 py-2.5 rounded-xl text-xs bg-emerald-400 text-black font-extrabold hover:bg-emerald-300 transition">Save Entry</button>
              </div>
            </div>
          </div>
        )}
