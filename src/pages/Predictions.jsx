...
export default function App() {
  ...
  return (
    <>
      {/* Embedded CSS / Theme definitions for MTL styling classes */}
      <style>{`
        :root {
          --mtlGreen: #00ff75;
          --mtlDark: #080c14;
        ...
        .pro-card:hover {
          ...
          box-shadow: 0 15px 40px rgba(0, 255, 117, 0.12), 0 0 20px rgba(0, 255, 117, 0.05);
        }
        .avatar-logo {
          display: flex;
          align-items: center;
          ...
          color: white;
          box-shadow: 0 0 18px rgba(0, 255, 117, 0.15);
        }
        .btn-see-more {
          display: inline-flex;
          align-items: center;
      ...
      <div className={`min-h-screen flex flex-col justify-between selection:bg-mtlGreen selection:text-black ${userProfile.high_contrast ? 'high-contrast-mode' : ''} bg-[#080c14] text-white`}>
        ...
        <div className="app-content-wrapper flex flex-col min-h-screen justify-between relative z-10">
          ...
          <div id="toast-container" className="fixed top-5 right-5 z-50 flex flex-col gap-2 pointer-events-none">
            {toasts.map(t => (
              <div
                key={t.id}
                className={`px-4 py-3 rounded-xl border text-xs font-bold font-cyber shadow-2xl flex items-center gap-2 pointer-events-auto transition-all duration-300 ${
          ...
          {reactionUsersModal.open && (
            <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
              <div className="bg-[#0e1626] border border-[#00ff75] rounded-2xl w-full max-w-sm p-5 space-y-4 shadow-2xl relative">
                <div className="flex justify-between items-center border-b border-[#24334d] pb-3">
                  <h4 className="font-bold text-sm text-[#00ff75] font-cyber">
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto text-xs">
                  {(matchReactionsMap[String(reactionUsersModal.matchId)] || []).filter(r => r.reaction === reactionUsersModal.type).length === 0 ? (
                    ...
                    (matchReactionsMap[String(reactionUsersModal.matchId)] || [])
                      ...
                      .map((u, i) => (
                        <div key={i} className="flex items-center gap-2 p-2 bg-[#080c14] rounded-lg border border-[#24334d]">
                ...
                <button onClick={() => setReactionUsersModal({ open: false, type: '', matchId: null })} className="w-full bg-[#111c2d] border border-[#24334d] text-gray-300 py-2 rounded-xl text-xs font-bold hover:text-white">Close</button>
              </div>
            </div>
          )}
          ...
          <aside className={`fixed top-0 right-0 h-full w-80 max-w-[85vw] bg-[#0e1626] border-l border-[#24334d] z-50 transform ${sideNavOpen ? 'translate-x-0' : 'translate-x-full'} transition-transform duration-300 ease-in-out flex flex-col justify-between p-6 shadow-2xl`}>
            <div className="space-y-6">
              <div className="flex items-center justify-between border-b border-[#24334d] pb-4">
                ...
                <button onClick={() => setSideNavOpen(false)} className="w-8 h-8 rounded-full bg-[#080c14] text-gray-400 hover:text-white flex items-center justify-center font-bold">✕</button>
              </div>

              <nav className="space-y-3">
                <a href="/dashboard" className="w-full flex items-center gap-3 p-3 rounded-xl bg-[#00ff75]/10 border border-[#00ff75] text-[#00ff75] hover:bg-[#00ff75] hover:text-black transition text-xs font-bold">
                  <span className="text-base">⬅️</span> Back to Dashboard
                </a>
                <button onClick={() => { setDialingModalOpen(true); setSideNavOpen(false); }} className="w-full flex items-center gap-3 p-3 rounded-xl bg-[#111c2d] border border-[#24334d] hover:border-[#00ff75] hover:text-[#00ff75] transition text-xs font-semibold text-gray-200">
                  <span className="text-base">📞</span> Contact Centre
                </button>
                <button onClick={() => { setSettingsModalOpen(true); setSideNavOpen(false); }} className="w-full flex items-center gap-3 p-3 rounded-xl bg-[#111c2d] border border-[#24334d] hover:border-[#00ff75] hover:text-[#00ff75] transition text-xs font-semibold text-gray-200">
                  <span className="text-base">⚙️</span> Preferences & Settings
                </button>
              </nav>
            </div>

            <div className="pt-6 border-t border-[#24334d] space-y-3">
          ...
          <div id="app-root">
            {/* Header */}
            <header className="border-b border-[#24334d] bg-[#0e1626]/90 backdrop-blur-md sticky top-0 z-40 px-6 py-4">
              <div className="max-w-7xl mx-auto flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <a href="/dashboard" title="Back to Dashboard" className="w-9 h-9 rounded-xl bg-[#111c2d] border border-[#24334d] text-[#00ff75] flex items-center justify-center font-bold text-sm hover:bg-[#00ff75] hover:text-black transition">⬅️</a>
                  ...
                  <div>
                    <h1 className="font-bold tracking-wider text-lg leading-tight font-cyber text-white">PREDICTIONS <span className="text-[#00ff75]">HUB</span></h1>
                    <span className="text-[10px] uppercase tracking-widest text-gray-400 font-semibold">Feel Welcomed.</span>
                  </div>
                </div>

                <nav className="hidden lg:flex items-center gap-8 text-sm font-medium">
                  <a href="/dashboard" className="flex items-center gap-2 text-[#00ff75] border-b-2 border-[#00ff75] pb-1 font-bold">DASHBOARD</a>
                ...
                <div className="flex items-center gap-3">
                  <button onClick={() => setSideNavOpen(true)} title="Open Navigation Options" className="w-10 h-10 rounded-full bg-[#111c2d] border border-[#24334d] text-gray-200 flex items-center justify-center hover:text-[#00ff75] transition">
                    ☰
                  </button>
                    
                  <div className="hidden md:flex items-center gap-3 bg-[#111c2d] border border-[#24334d] px-3 py-1.5 rounded-full cursor-pointer" onClick={() => setSideNavOpen(true)}>
            ...
            {/* Banner */}
            <section className="relative overflow-hidden py-12 px-6 border-b border-[#24334d] bg-gradient-to-b from-[#0e1626] to-[#080c14]">
              <div className="max-w-7xl mx-auto text-center relative z-10">
                <span className="text-xs uppercase tracking-[0.25em] text-[#00ff75] font-bold bg-[#00ff75]/10 px-4 py-1.5 rounded-full border border-[#00ff75]/20">Sports Analytics & 4D Intelligence</span>
                <h2 className="text-3xl lg:text-5xl font-extrabold tracking-tight mt-3 uppercase font-cyber text-white">FOOTBALL <span className="text-[#00ff75]">INTELLIGENCE</span></h2>
            ...
            <main className="max-w-7xl mx-auto px-6 py-8 space-y-12">
                      ...
                      return (
                        ...
                        >
                          <div className="flex justify-between items-center text-xs text-gray-400 mb-3 font-semibold">
                            <span className="font-cyber hover:text-[#00ff75]" onClick={(e) => { e.stopPropagation(); openGoogleSearchIframe(`Match live summary ${match.teams}`); }}>
                          ...
                          <div className="flex items-center justify-between my-4">
                            ...
                            <div className="text-2xl font-extrabold tracking-wider px-2 font-cyber text-[#00ff75]">- _ -</div>
                            <div className="text-center flex-1">
                          ...
                          <div className="text-center text-xs font-semibold text-[#00ff75] mb-2 font-cyber">{match.minute} Minutes</div>
                          <div className="water-progress-container mb-3">
                            <div className="water-progress-bar" style={{ width: `${match.progress}%` }}></div>
                          </div>
                          <div className="text-[11px] text-gray-400 pt-2 border-t border-[#24334d] flex justify-between items-center">
              ...
              <section id="db-matches-section" className="bg-[#0e1626] border border-[#24334d] rounded-2xl p-6 shadow-2xl">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4 section-header">
                  <div>
                    <h3 className="text-lg font-extrabold uppercase tracking-wide text-white font-cyber cursor-pointer hover:text-[#00ff75] transition" onClick={() => openGoogleSearchIframe('Live database matches and football predictions')}>
                      ⚽ MATCHES & PREDICTIONS
                    </h3>
                ...
                {/* Match Controls Row */}
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 mb-6 border-b border-[#24334d] pb-4">
                  <div className="flex items-center gap-2 bg-[#080c14] p-1.5 rounded-xl border border-[#24334d] self-start">
                    <button onClick={() => setActiveMatchTab('future')} className={`px-4 py-1.5 rounded-lg text-xs font-bold font-cyber transition ${activeMatchTab === 'future' ? 'bg-[#00ff75] text-black' : 'text-gray-400 hover:text-white'}`}>
                      UPCOMING MATCHES
                    </button>
                    <button onClick={() => setActiveMatchTab('past')} className={`px-4 py-1.5 rounded-lg text-xs font-bold font-cyber transition ${activeMatchTab === 'past' ? 'bg-[#00ff75] text-black' : 'text-gray-400 hover:text-white'}`}>
                      PAST PREDICTIONS
                    </button>
                  </div>
                  ...
                  <div className="relative flex-1 max-w-md">
                    ...
                    <input
                      ...
                      className="w-full bg-[#080c14] border border-[#24334d] rounded-xl pl-8 pr-4 py-2 text-xs text-white focus:outline-none focus:border-[#00ff75]"
                    />
                  </div>
                </div>
                      ...
                      return (
                        <div key={match.id} className="pro-card p-5 flex flex-col justify-between space-y-4">
                          <div className="space-y-2">
                            ...
                            <h4 className="font-extrabold text-base text-white tracking-wide font-cyber hover:text-[#00ff75] cursor-pointer" onClick={() => openGoogleSearchIframe(`Prediction summary for ${match.teams}`)}>
                              {sanitizeInput(match.teams)}
                            </h4>
                            <p className="text-xs text-[#00ff75] font-semibold">Prediction: {sanitizeInput(match.prediction)} ({stars})</p>
                            <p className="text-xs text-gray-400 line-clamp-2">{sanitizeInput(match.analysis_text || 'Tactical breakdown in detailed view.')}</p>
                          </div>

                          <div className="space-y-1 bg-[#080c14] p-3 rounded-xl border border-[#24334d]">
                          ...
                          <div className="flex items-center gap-2 pt-1">
                            <button onClick={() => reactToMatch(match.id, 'fire')} className="bg-[#080c14] border border-[#24334d] px-2.5 py-1 rounded-lg text-xs hover:border-[#00ff75] flex items-center gap-1 transition">
                              🔥 <span>{match.reactions?.fire || 0}</span>
                            </button>
                            <button onClick={() => reactToMatch(match.id, 'heart')} className="bg-[#080c14] border border-[#24334d] px-2.5 py-1 rounded-lg text-xs hover:border-[#00ff75] flex items-center gap-1 transition">
                              ❤️ <span>{match.reactions?.heart || 0}</span>
                            </button>
                            <button onClick={() => reactToMatch(match.id, 'dislike')} className="bg-[#080c14] border border-[#24334d] px-2.5 py-1 rounded-lg text-xs hover:border-[#00ff75] flex items-center gap-1 transition">
                              👎 <span>{match.reactions?.dislike || 0}</span>
                            </button>
                          </div>

                          {/* Comments Preview Container */}
                          <div onClick={() => setFullscreenCommentsModal({ open: true, matchId: match.id, teams: match.teams })} className="bg-[#080c14] rounded-xl p-3 space-y-2 border border-[#24334d] hover:border-[#00ff75] transition cursor-pointer">
                            <div className="flex justify-between items-center text-[11px] font-bold text-gray-300">
                              <span>💬 Comments ({comments.length})</span>
                              <span className="text-[#00ff75] text-[10px] uppercase font-bold">🖥️ Fullscreen View ➔</span>
                            </div>
                            <div className="space-y-1.5 max-h-20 overflow-y-auto text-[11px]">
                              ...
                              {comments.slice(-2).map((c) => (
                                <div key={c.id} className="bg-[#111c2d] p-1.5 rounded border border-[#24334d] text-gray-300">
                          ...
                          {/* Card Buttons */}
                          <div className="pt-2 border-t border-[#24334d] flex flex-wrap items-center justify-between gap-2">
                            <div className="flex items-center gap-2">
                              <button onClick={() => setFullscreenMatchModal({ open: true, match })} className="bg-[#00ff75]/10 border border-[#00ff75] text-[#00ff75] hover:bg-[#00ff75] hover:text-black font-bold px-3 py-1.5 rounded-xl text-xs transition">🔍 Details</button>
                              <button onClick={() => setMatchChatModal({ open: true, matchId: match.id, teams: match.teams })} className="bg-[#080c14] border border-[#24334d] px-3 py-1.5 rounded-xl text-xs text-gray-200 hover:text-[#00ff75] transition flex items-center gap-1">💬 Telegram Chat</button>
                            </div>
                            {userProfile.role === 'admin' && (
                ...
                {userProfile.role === 'admin' && (
                  <div className="mt-6 pt-4 border-t border-[#24334d] flex justify-center">
                    <button onClick={() => openAdminModal('matches')} className="bg-[#00ff75] text-black font-extrabold px-6 py-2.5 rounded-full text-xs uppercase tracking-wider hover:bg-[#00d866] transition flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {/* Upcoming Fixtures */}
                <section id="fixtures-section" className="bg-[#0e1626] border border-[#24334d] rounded-2xl p-6 flex flex-col justify-between space-y-4 shadow-2xl">
                  <div>
                    <div className="flex items-center justify-between mb-6 section-header">
                      <div>
                        <h3 className="font-extrabold uppercase tracking-wide text-base text-white font-cyber cursor-pointer hover:text-[#00ff75] transition" onClick={() => openGoogleSearchIframe('Upcoming football fixtures schedule')}>
                          UPCOMING FIXTURES
                        </h3>
                      ...
                      {fixturesData.length === 0 ? (
                        ...
                        fixturesData.slice(0, 3).map((fix) => (
                          <div key={fix.id} className="pro-card p-4 flex items-center justify-between cursor-pointer">
                            <div className="flex items-center gap-3">
                              <div className="w-9 h-9 rounded-xl bg-[#080c14] border border-[#24334d] flex items-center justify-center font-bold text-xs text-[#00ff75] font-cyber">{sanitizeInput(fix.badge)}</div>
                              <div>
                            ...
                            <div className="flex items-center gap-3">
                              <div className="text-right">
                                <span className="text-xs font-bold text-[#00ff75] block font-cyber">{sanitizeInput(fix.match_time)}</span>
                  ...
                  {userProfile.role === 'admin' && (
                    <div className="pt-4 border-t border-[#24334d] flex justify-center">
                      <button onClick={() => openAdminModal('fixtures')} className="bg-[#111c2d] border border-[#00ff75] text-[#00ff75] font-bold px-5 py-2 rounded-xl text-xs hover:bg-[#00ff75] hover:text-black transition flex items-center gap-2">
                ...
                {/* Trending News */}
                <section id="trending-section" className="bg-[#0e1626] border border-[#24334d] rounded-2xl p-6 flex flex-col justify-between space-y-4 shadow-2xl">
                  <div>
                    <div className="flex items-center justify-between mb-6 section-header">
                      <div>
                        <h3 className="font-extrabold uppercase tracking-wide text-base text-white font-cyber cursor-pointer hover:text-[#00ff75] transition" onClick={() => openGoogleSearchIframe('Trending football news updates')}>
                          🔥 TRENDING NEWS
                        </h3>
                      ...
                      {trendingData.length === 0 ? (
                        ...
                        trendingData.slice(0, 3).map((item) => (
                          <div key={item.id} className="pro-card p-4 flex items-center justify-between cursor-pointer">
                            <div className="flex items-center gap-3">
                              <span className="text-sm font-extrabold text-[#00ff75] font-cyber">#{sanitizeInput(item.rank)}</span>
                              <div>
                  ...
                  {userProfile.role === 'admin' && (
                    <div className="pt-4 border-t border-[#24334d] flex justify-center">
                      <button onClick={() => openAdminModal('trending')} className="bg-[#111c2d] border border-[#00ff75] text-[#00ff75] font-bold px-5 py-2 rounded-xl text-xs hover:bg-[#00ff75] hover:text-black transition flex items-center gap-2">
                        <span>➕</span> ADD NEWS
                      </button>
                    </div>
                  )}
            ...
            {/* Footer */}
            <footer className="border-t border-[#24334d] bg-[#0e1626] mt-16 py-8 px-6 text-center text-xs text-gray-400">
              <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
                <p>© 2026 MTL Football Intelligence Hub. All rights reserved.</p>
                <div className="flex items-center gap-6">
                  <a href="/dashboard" className="text-[#00ff75] font-bold hover:underline">Dashboard</a>
                  <a href="#" className="hover:text-[#00ff75]">Privacy Policy</a>
                  <a href="#" className="hover:text-[#00ff75]">Terms of Service</a>
                  <button onClick={() => setDialingModalOpen(true)} className="hover:text-[#00ff75]">Developed BY M. Lennox</button>
                </div>
              </div>
            </footer>
          </div>
          ...
          {googleIframeModal.open && (
            <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex flex-col p-3 sm:p-6">
              <div className="bg-[#0e1626] border border-[#24334d] rounded-2xl p-3 mb-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-[#00ff75] text-black font-extrabold flex items-center justify-center font-cyber">AI</div>
                  <div>
                    <h4 className="text-xs font-bold font-cyber text-[#00ff75]">GOOGLE QUICK SEARCH</h4>
              ...
              <div className="flex-1 rounded-2xl overflow-hidden border border-[#24334d] bg-white">
                <iframe
          ...
          <div className="fixed bottom-6 right-6 z-40">
            <button onClick={() => setGlobalChatOpen(!globalChatOpen)} className="w-14 h-14 rounded-full bg-[#00ff75] text-black flex items-center justify-center text-2xl font-bold shadow-lg hover:scale-105 transition transform">💬</button>
              
            {globalChatOpen && (
              <div className="absolute bottom-20 right-0 w-80 sm:w-96 bg-[#0e1626] border border-[#24334d] rounded-2xl shadow-2xl flex flex-col h-[480px] overflow-hidden">
                ...
                <div className="p-3 border-t border-[#24334d] bg-[#080c14] flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-[#111c2d] border border-[#24334d] rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-[#00ff75]"
                  />
          ...
          {fullscreenCommentsModal.open && (
            <div className="fixed inset-0 bg-[#080c14]/95 backdrop-blur-xl z-50 p-4 md:p-10 overflow-y-auto flex flex-col justify-between">
              <div className="max-w-4xl w-full mx-auto bg-[#0e1626] border border-[#00ff75]/40 rounded-3xl p-6 md:p-8 shadow-2xl relative flex-1 flex flex-col justify-between space-y-6">
                <div className="flex items-center justify-between border-b border-[#24334d] pb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-2xl bg-[#00ff75]/20 border border-[#00ff75] text-[#00ff75] flex items-center justify-center font-bold text-lg font-cyber">💬</div>
                    <div>
                      <h3 className="text-lg md:text-xl font-extrabold text-white font-cyber">Comments Stream: {fullscreenCommentsModal.teams}</h3>
                      <p className="text-xs text-[#00ff75]">Leave a comment.</p>
                    </div>
                  </div>
                ...
                <div className="flex-1 overflow-y-auto space-y-4 pr-2 max-h-[60vh]">
                  {(matchCommentsStore[fullscreenCommentsModal.matchId] || []).length === 0 ? (
                    ...
                    (matchCommentsStore[fullscreenCommentsModal.matchId] || []).map(c => {
                      ...
                      return (
                        <div key={c.id} className="bg-[#111c2d] border border-[#24334d] p-4 rounded-2xl space-y-2 flex gap-3 items-start">
                          <div className="w-9 h-9 rounded-full bg-[#00ff75]/20 text-[#00ff75] font-bold flex items-center justify-center text-xs font-cyber flex-shrink-0">{getFirstNameInitials(c.user)}</div>
                          <div className="flex-1">
                            <div className="flex items-center justify-between">
                              <span className="font-bold text-xs text-[#00ff75] font-cyber">{sanitizeInput(c.user)}</span>
                              <div className="flex items-center gap-2">
                ...
                <div className="bg-[#080c14] p-4 rounded-2xl border border-[#24334d] space-y-3">
                  ...
                  <div className="flex gap-3">
                    <textarea
                      ...
                      className="flex-1 bg-[#111c2d] border border-[#24334d] rounded-xl p-3 text-xs text-white focus:outline-none focus:border-[#00ff75]"
                    />
                    <button onClick={submitFullscreenComment} className="bg-[#00ff75] text-black font-extrabold px-6 py-2 rounded-xl text-xs hover:bg-[#00d866] transition self-end">Post Comment</button>
                  </div>
                </div>

                <div className="flex justify-end pt-2 border-t border-[#24334d]">
          ...
          {statsListModal.open && (
            <div className="fixed inset-0 bg-[#080c14]/95 backdrop-blur-md z-50 overflow-y-auto p-4 md:p-10">
              <div className="max-w-5xl mx-auto bg-[#0e1626] border border-[#24334d] rounded-2xl p-6 md:p-10 shadow-2xl relative min-h-[85vh] flex flex-col justify-between">
                <button onClick={() => setStatsListModal({ open: false, title: '', dataset: [] })} className="absolute top-6 right-6 w-10 h-10 rounded-full bg-[#111c2d] border border-[#24334d] text-gray-400 hover:text-white flex items-center justify-center text-lg font-bold">✕</button>
                <div className="space-y-6">
                  <div className="border-b border-[#24334d] pb-4 section-header">
                    <h3 className="text-2xl font-extrabold text-[#00ff75] uppercase tracking-wider font-cyber">{statsListModal.title}</h3>
                ...
                <div className="mt-8 pt-6 border-t border-[#24334d] flex justify-end">
          ...
          {fullscreenMatchModal.open && fullscreenMatchModal.match && (
            <div className="fixed inset-0 bg-[#080c14]/95 backdrop-blur-md z-50 overflow-y-auto p-4 md:p-10">
              <div className="max-w-5xl mx-auto bg-[#0e1626] border border-[#24334d] rounded-2xl p-6 md:p-10 shadow-2xl relative min-h-[85vh] flex flex-col justify-between">
                <button onClick={() => setFullscreenMatchModal({ open: false, match: null })} className="absolute top-6 right-6 w-10 h-10 rounded-full bg-[#111c2d] border border-[#24334d] text-gray-400 hover:text-white flex items-center justify-center text-lg font-bold">✕</button>
                  
                <div className="space-y-8">
                  <div className="flex justify-between items-start border-b border-[#24334d] pb-6 section-header">
                    <div>
                      <span className="px-3 py-1 rounded-full text-xs font-extrabold uppercase bg-[#00ff75]/20 text-[#00ff75] border border-[#00ff75]/30">
                        {fullscreenMatchModal.match.type || 'free'} INTEL
                      </span>
                      <h2 className="text-3xl lg:text-4xl font-extrabold text-white mt-2 font-cyber hover:text-[#00ff75] cursor-pointer" onClick={() => openGoogleSearchIframe(`Live analysis ${fullscreenMatchModal.match.teams}`)}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-[#080c14] p-5 rounded-2xl border border-[#24334d] space-y-2">
                      <span className="text-xs text-[#00ff75] font-bold uppercase tracking-wider font-cyber">Prediction</span>
                  ...
                  <div className="bg-[#080c14] p-6 rounded-2xl border border-[#24334d] space-y-3">
                    <h4 className="font-extrabold text-sm uppercase tracking-wider text-[#00ff75] font-cyber">Tactical Intelligence & Match Analysis</h4>
                ...
                <div className="mt-8 pt-6 border-t border-[#24334d] flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    <button onClick={() => { setMatchChatModal({ open: true, matchId: fullscreenMatchModal.match.id, teams: fullscreenMatchModal.match.teams }); setFullscreenMatchModal({ open: false, match: null }); }} className="bg-[#111c2d] border border-[#24334d] text-gray-200 px-4 py-2 rounded-xl text-xs hover:text-[#00ff75] flex items-center gap-2">💬 Open Chat</button>
                    <button onClick={() => setDialingModalOpen(true)} className="bg-[#00ff75]/10 border border-[#00ff75] text-[#00ff75] px-4 py-2 rounded-xl text-xs font-bold hover:bg-[#00ff75] hover:text-black transition">📞 Call</button>
                  </div>
          ...
          {matchChatModal.open && (
            <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
              <div className="bg-[#0e1626] border border-[#24334d] rounded-2xl w-full max-w-2xl h-[80vh] flex flex-col overflow-hidden shadow-2xl">
                <div className="bg-[#080c14] p-4 border-b border-[#24334d] flex items-center justify-between">
                  <div>
                    <h4 className="font-bold text-sm text-[#00ff75] font-cyber">Telegram Match Thread: {matchChatModal.teams}</h4>
          ...
          {dialingModalOpen && (
            <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-4">
              <div className="bg-[#0e1626] border border-[#24334d] rounded-2xl w-full max-w-2xl p-6 md:p-8 space-y-6 shadow-2xl">
                <div className="flex justify-between items-center border-b border-[#24334d] pb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-[#00ff75]/20 border border-[#00ff75] text-[#00ff75] flex items-center justify-center text-xl">📞</div>
                    <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-[#080c14] p-4 rounded-xl border border-[#24334d] space-y-2 flex flex-col justify-between">
                    <div>
                    ...
                    <a href="https://wa.me/254716883895" target="_blank" rel="noopener noreferrer" className="w-full bg-[#00ff75] text-black text-center text-xs font-bold py-2.5 rounded-xl hover:bg-[#00d866] transition mt-3 block">💬 WhatsApp +254716883895</a>
                  </div>
                  <div className="bg-[#080c14] p-4 rounded-xl border border-[#24334d] space-y-2 flex flex-col justify-between">
                    <div>
          ...
          {settingsModalOpen && (
            <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-4">
              <div className="bg-[#0e1626] border border-[#24334d] rounded-2xl w-full max-w-md p-6 space-y-6 shadow-2xl">
                <div className="flex justify-between items-center border-b border-[#24334d] pb-3">
                  <h3 className="text-base font-bold text-[#00ff75] uppercase tracking-wider font-cyber">User Preferences</h3>
                ...
                <div className="space-y-4 text-xs">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-[#080c14] border border-[#24334d] rounded-xl p-2.5 text-white"
                    >
                ...
                <button onClick={() => setSettingsModalOpen(false)} className="w-full bg-[#00ff75] text-black font-bold py-2 rounded-xl">Save & Close</button>
              </div>
            </div>
          )}
          ...
          {adminModal.open && (
            <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex items-center justify-center p-4">
              <div className="bg-[#0e1626] border border-[#24334d] rounded-2xl w-full max-w-2xl p-8 space-y-6 shadow-2xl relative">
                <div className="flex justify-between items-center border-b border-[#24334d] pb-4">
                  <div>
                    <h3 className="text-xl font-extrabold text-[#00ff75] uppercase tracking-wider font-cyber">
                ...
                <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                  {adminModal.section === 'matches' && (
                    <>
                      <input
                        ...
                        className="w-full bg-[#080c14] border border-[#24334d] rounded-xl p-3 text-xs text-white"
                      />
                      <input
                        type="text"
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <input
                          ...
                          className="bg-[#080c14] border border-[#24334d] rounded-xl p-3 text-xs text-white"
                        />
                        <input
                          type="time"
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <input
                          ...
                          value={adminFormData.decimal_odds || ''}
                          onChange={(e) => setAdminFormData(prev => ({ ...prev, decimal_odds: e.target.value }))}
                          className="bg-[#080c14] border border-[#24334d] rounded-xl p-3 text-xs text-white"
                        />
                      </div>
                      <div className="grid grid-cols-3 gap-3">
                        <input
                          type="number"
                          placeholder="Home Prob %"
                          value={adminFormData.prob_home || ''}
                          onChange={(e) => setAdminFormData(prev => ({ ...prev, prob_home: e.target.value }))}
                          className="bg-[#080c14] border border-[#24334d] rounded-xl p-3 text-xs text-white"
                        />
                        <input
                          type="number"
                          placeholder="Draw Prob %"
                          value={adminFormData.prob_draw || ''}
                          onChange={(e) => setAdminFormData(prev => ({ ...prev, prob_draw: e.target.value }))}
                          className="bg-[#080c14] border border-[#24334d] rounded-xl p-3 text-xs text-white"
                        />
                        <input
                          type="number"
                          placeholder="Away Prob %"
                          value={adminFormData.prob_away || ''}
                          onChange={(e) => setAdminFormData(prev => ({ ...prev, prob_away: e.target.value }))}
                          className="bg-[#080c14] border border-[#24334d] rounded-xl p-3 text-xs text-white"
                        />
                      </div>
                      <textarea
                        placeholder="Tactical intelligence summary..."
                        value={adminFormData.analysis_text || ''}
                        onChange={(e) => setAdminFormData(prev => ({ ...prev, analysis_text: e.target.value }))}
                        rows={3}
                        className="w-full bg-[#080c14] border border-[#24334d] rounded-xl p-3 text-xs text-white"
                      />
                    </>
                  ...
                  {adminModal.section === 'trending' && (
                    <>
                      <input
                        type="number"
                        placeholder="Rank Number (e.g. 1)"
                        value={adminFormData.rank || ''}
                        onChange={(e) => setAdminFormData(prev => ({ ...prev, rank: e.target.value }))}
                        className="w-full bg-[#080c14] border border-[#24334d] rounded-xl p-3 text-xs text-white"
                      />
                      <input
                        type="text"
                        ...
                        onChange={(e) => setAdminFormData(prev => ({ ...prev, title: e.target.value }))}
                        className="w-full bg-[#080c14] border border-[#24334d] rounded-xl p-3 text-xs text-white"
                      />
                      <input
                        type="number"
                        placeholder="Comments count"
                        value={adminFormData.comments_count || ''}
                        onChange={(e) => setAdminFormData(prev => ({ ...prev, comments_count: e.target.value }))}
                        className="w-full bg-[#080c14] border border-[#24334d] rounded-xl p-3 text-xs text-white"
                      />
                    </>
                  )}
                ...
                <div className="flex justify-end gap-4 border-t border-[#24334d] pt-4">
                  ...
                  <button onClick={saveAdminEntry} className="px-6 py-2.5 rounded-xl text-xs bg-[#00ff75] text-black font-extrabold hover:bg-[#00d866] transition">Save Entry</button>
                </div>
              </div>
            </div>
          )}
