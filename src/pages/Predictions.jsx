...
export default function App() {
  ...
  return (
    <>
      ...
      <div id="app-container" className={`min-h-screen flex flex-col justify-between selection:bg-[#00ff75] selection:text-black ${userProfile.high_contrast ? 'high-contrast-mode' : ''} bg-[#080c14] text-white font-sans`}>
        <canvas ref={canvasRef} id="bg-4d-canvas" className="fixed top-0 left-0 w-screen h-screen pointer-events-none z-0 opacity-40" />

        <div id="app-content-wrapper" className="app-content-wrapper flex flex-col min-h-screen justify-between relative z-10">
          <div id="toast-container" className="fixed top-5 right-5 z-50 flex flex-col gap-2 pointer-events-none">
            {toasts.map(t => (
              <div
                key={t.id}
                id={`toast-${t.id}`}
                className={`toast-item px-4 py-3 rounded-xl border text-xs font-bold shadow-2xl flex items-center gap-2.5 pointer-events-auto transition-all duration-300 ${
                  t.isError ? 'bg-red-950/95 border-red-500/50 text-red-200' : 'bg-emerald-950/95 border-emerald-500/50 text-emerald-200'
                }`}
              >
                <span className="toast-icon">{t.isError ? '⚠️' : '✔️'}</span>
                <span className="toast-message">{sanitizeInput(t.message)}</span>
              </div>
            ))}
          </div>

          <div id="floating-loader" className={`floating-loader-overlay ${loader.active ? 'active' : ''}`}>
            <div id="loader-card" className="loader-card space-y-4">
              <div className="flex items-center justify-center gap-2.5">
                <span className="w-2.5 h-2.5 rounded-full bg-[#00f0ff] animate-ping"></span>
                <h4 id="loader-title" className="text-xs font-extrabold uppercase tracking-widest text-[#00f0ff] font-mono">QUANTUM SYNC</h4>
              </div>
              <p id="loader-text" className="text-sm font-medium text-gray-200">{loader.text}</p>
              <div id="loader-progress-container" className="water-progress-container">
                <div id="loader-progress-bar" className="water-progress-bar" style={{ width: `${loader.progress}%` }}></div>
              </div>
            </div>
          </div>

          {reactionUsersModal.open && (
            <div id="modal-reaction-users" className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-4">
              <div className="bg-[#0e1626] border border-[#00ff75]/50 rounded-2xl w-full max-w-sm p-6 space-y-4 shadow-2xl relative">
                <div className="flex justify-between items-center border-b border-[#24334d] pb-3">
                  <h4 id="reaction-modal-title" className="font-bold text-xs uppercase tracking-widest text-[#00ff75] font-mono">
                    REACTANTS ({reactionUsersModal.type.toUpperCase()})
                  </h4>
                  <button id="btn-close-reaction-modal" onClick={() => setReactionUsersModal({ open: false, type: '', matchId: null })} className="text-gray-400 hover:text-white font-bold w-7 h-7 flex items-center justify-center rounded-lg hover:bg-white/5">✕</button>
                </div>
                <div id="reaction-users-list" className="space-y-2 max-h-60 overflow-y-auto text-xs pr-1">
                ...
                <button id="btn-dismiss-reaction-modal" onClick={() => setReactionUsersModal({ open: false, type: '', matchId: null })} className="w-full bg-[#111c2d] border border-[#24334d] text-gray-300 py-2.5 rounded-xl text-xs font-bold hover:text-white hover:border-[#00ff75]/40 transition">Close</button>
              </div>
            </div>
          )}

          {sideNavOpen && (
            <div id="sidenav-backdrop" onClick={() => setSideNavOpen(false)} className="fixed inset-0 bg-black/75 backdrop-blur-sm z-50 transition-opacity"></div>
          )}
          <aside id="side-navigation" className={`fixed top-0 right-0 h-full w-80 max-w-[85vw] bg-[#0e1626] border-l border-[#24334d] z-50 transform ${sideNavOpen ? 'translate-x-0' : 'translate-x-full'} transition-transform duration-300 ease-out flex flex-col justify-between p-6 shadow-2xl`}>
            <div className="space-y-6">
              <div className="flex items-center justify-between border-b border-[#24334d] pb-4">
                ...
                <button id="btn-close-sidenav" onClick={() => setSideNavOpen(false)} className="w-8 h-8 rounded-lg bg-[#080c14] text-gray-400 hover:text-white flex items-center justify-center font-bold">✕</button>
              </div>
            ...
            <div className="pt-6 border-t border-[#24334d]">
              <button id="btn-sign-out" onClick={signOutUser} className="w-full bg-red-950/50 text-red-300 border border-red-500/35 py-2.5 rounded-xl text-xs font-extrabold uppercase tracking-wider hover:bg-red-800 hover:text-white transition flex items-center justify-center gap-2">
                <span>❌</span> Sign Out
              </button>
            </div>
          </aside>
          ...
          <div id="app-root">
            <header id="main-header" className="border-b border-[#24334d] bg-[#0e1626]/90 backdrop-blur-md sticky top-0 z-40 px-6 py-3.5">
              <div className="max-w-7xl mx-auto flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <button id="btn-toggle-sidenav" onClick={() => setSideNavOpen(true)} title="Menu" className="w-10 h-10 rounded-xl bg-[#111c2d] border border-[#24334d] text-gray-200 flex items-center justify-center hover:border-[#00ff75]/40 hover:text-[#00ff75] transition">
                    ☰
                  </button>
            ...
            <section id="hero-section" className="relative overflow-hidden py-10 px-6 border-b border-[#24334d] bg-gradient-to-b from-[#0e1626] to-[#080c14]">
            ...
            {dbError && (
              <div id="database-error-banner" className="max-w-7xl mx-auto px-6 pt-6">
                <div className="bg-red-950/60 border border-red-500/45 rounded-2xl p-5 shadow-2xl">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <button id="btn-dismiss-db-error" onClick={() => setDbError(null)} className="text-gray-400 hover:text-white w-7 h-7 flex items-center justify-center rounded-lg hover:bg-white/5">✕</button>
                  </div>
            ...
            <main id="main-content-area" className="max-w-7xl mx-auto px-6 py-8 space-y-12">
              <section id="live-section">
                <div className="flex items-center justify-between mb-5 section-header">
                  ...
                  <button id="btn-see-more-live" onClick={() => setStatsListModal({ open: true, title: 'Live Games Directory', dataset: liveMatchesData })} className="btn-see-more">
                    <span>SEE MORE MATCHES</span> ➔
                  </button>
                </div>
                {/* Strict Two-Column Grid Layout Style */}
                <div id="live-matches-grid" className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                  {liveMatchesData.length === 0 ? (
                    <div className="col-span-2 text-center py-10 pro-card">
                      <p className="text-xs text-gray-400">No live matches currently in play.</p>
                    </div>
                  ) : (
                    liveMatchesData.slice(0, 4).map((match) => {
              ...
              <section id="db-matches-section" className="bg-[#0e1626]/90 backdrop-blur-md border border-[#24334d] rounded-2xl p-6 sm:p-7 shadow-2xl space-y-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-[#24334d] pb-5">
                  ...
                  <button id="btn-see-more-predictions" onClick={() => setStatsListModal({ open: true, title: 'All Database Predictions', dataset: matchesData })} className="btn-see-more">
                    <span>SEE MORE</span> ➔
                  </button>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-1.5 bg-[#080c14] p-1.5 rounded-xl border border-[#24334d] self-start">
                    <button id="btn-tab-future" onClick={() => setActiveMatchTab('future')} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition ${activeMatchTab === 'future' ? 'bg-[#00ff75] text-black shadow-sm' : 'text-gray-400 hover:text-white'}`}>
                      UPCOMING MATCHES
                    </button>
                    <button id="btn-tab-past" onClick={() => setActiveMatchTab('past')} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition ${activeMatchTab === 'past' ? 'bg-[#00ff75] text-black shadow-sm' : 'text-gray-400 hover:text-white'}`}>
                      PAST PREDICTIONS
                    </button>
                  </div>
                ...
                {/* Strict Two-Column Grid Layout Style */}
                <div id="database-matches-grid" className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                  {getFilteredMatches().length === 0 ? (
                    <div className="col-span-2 text-center py-12 pro-card">
                      <p className="text-xs text-gray-400">No matches found matching query "{matchSearchQuery}".</p>
                    </div>
                  ) : (
                    getFilteredMatches().slice(0, 4).map((match) => {
                      ...
                      return (
                        <div key={match.id} id={`match-card-${match.id}`} className="pro-card flex flex-col justify-between space-y-4">
                          ...
                          <div className="flex items-center gap-2">
                            <button id={`btn-react-fire-${match.id}`} onClick={() => reactToMatch(match.id, 'fire')} className="bg-[#080c14] border border-[#24334d] px-2.5 py-1 rounded-lg text-xs hover:border-[#00ff75]/50 flex items-center gap-1 transition">
                              🔥 <span className="font-mono text-[11px]">{match.reactions?.fire || 0}</span>
                            </button>
                            <button id={`btn-react-heart-${match.id}`} onClick={() => reactToMatch(match.id, 'heart')} className="bg-[#080c14] border border-[#24334d] px-2.5 py-1 rounded-lg text-xs hover:border-[#00ff75]/50 flex items-center gap-1 transition">
                              ❤️ <span className="font-mono text-[11px]">{match.reactions?.heart || 0}</span>
                            </button>
                            <button id={`btn-react-dislike-${match.id}`} onClick={() => reactToMatch(match.id, 'dislike')} className="bg-[#080c14] border border-[#24334d] px-2.5 py-1 rounded-lg text-xs hover:border-[#00ff75]/50 flex items-center gap-1 transition">
                              👎 <span className="font-mono text-[11px]">{match.reactions?.dislike || 0}</span>
                            </button>
                          </div>

                          <div id={`comments-preview-${match.id}`} onClick={() => setFullscreenCommentsModal({ open: true, matchId: match.id, teams: match.teams })} className="bg-[#080c14] rounded-xl p-3 space-y-2 border border-[#24334d] hover:border-[#00ff75]/50 transition cursor-pointer">
                          ...
                          <div className="pt-3 border-t border-[#24334d] flex flex-wrap items-center justify-between gap-2">
                            <div className="flex items-center gap-2">
                              <button id={`btn-match-details-${match.id}`} onClick={() => setFullscreenMatchModal({ open: true, match })} className="bg-[#00ff75]/15 border border-[#00ff75]/40 text-[#00ff75] hover:bg-[#00ff75] hover:text-black font-bold px-3 py-1.5 rounded-xl text-xs transition">🔍 Details</button>
                              <button id={`btn-match-chat-${match.id}`} onClick={() => setMatchChatModal({ open: true, matchId: match.id, teams: match.teams })} className="bg-[#080c14] border border-[#24334d] px-3 py-1.5 rounded-xl text-xs text-gray-200 hover:text-[#00ff75] hover:border-[#00ff75]/40 transition flex items-center gap-1">💬 Chat</button>
                            </div>
                            {userProfile.role === 'admin' && (
                ...
                {userProfile.role === 'admin' && (
                  <div className="pt-4 border-t border-[#24334d] flex justify-center">
                    <button id="btn-add-prediction" onClick={() => openAdminModal('matches')} className="bg-[#00ff75] text-black font-extrabold px-6 py-2.5 rounded-full text-xs uppercase tracking-wider hover:bg-[#00d866] transition flex items-center gap-2 shadow-[0_0_20px_rgba(0,255,117,0.3)]">
                ...
                <section id="fixtures-section" className="bg-[#0e1626]/90 backdrop-blur-md border border-[#24334d] rounded-2xl p-6 sm:p-7 flex flex-col justify-between space-y-5 shadow-2xl">
                  <div>
                    <div className="flex items-center justify-between mb-5 section-header">
                      ...
                      <button id="btn-see-more-fixtures" onClick={() => setStatsListModal({ open: true, title: 'Complete Fixtures Schedule', dataset: fixturesData })} className="btn-see-more">
                        <span>SEE MORE</span> ➔
                      </button>
                    </div>

                    <div id="fixtures-list" className="space-y-3.5">
                      {fixturesData.length === 0 ? (
                        <div className="text-center py-10 text-xs text-gray-500 pro-card">Loading fixtures...</div>
                      ) : (
                        fixturesData.slice(0, 3).map((fix) => (
                          <div key={fix.id} id={`fixture-item-${fix.id}`} className="pro-card p-3.5 flex items-center justify-between cursor-pointer">
                  ...
                  {userProfile.role === 'admin' && (
                    <div className="pt-4 border-t border-[#24334d] flex justify-center">
                      <button id="btn-add-fixture" onClick={() => openAdminModal('fixtures')} className="bg-[#111c2d] border border-[#00ff75]/50 text-[#00ff75] font-bold px-5 py-2 rounded-xl text-xs hover:bg-[#00ff75] hover:text-black transition flex items-center gap-2">
                ...
                <section id="trending-section" className="bg-[#0e1626]/90 backdrop-blur-md border border-[#24334d] rounded-2xl p-6 sm:p-7 flex flex-col justify-between space-y-5 shadow-2xl">
                  <div>
                    <div className="flex items-center justify-between mb-5 section-header">
                      ...
                      <button id="btn-see-more-trending" onClick={() => setStatsListModal({ open: true, title: 'All Trending News', dataset: trendingData })} className="btn-see-more">
                        <span>SEE MORE</span> ➔
                      </button>
                    </div>

                    <div id="trending-list" className="space-y-3.5">
                      {trendingData.length === 0 ? (
                        <div className="text-center py-10 text-xs text-gray-500 pro-card">Loading news...</div>
                      ) : (
                        trendingData.slice(0, 3).map((item) => (
                          <div key={item.id} id={`trending-item-${item.id}`} className="pro-card p-3.5 flex items-center justify-between cursor-pointer">
                            <div className="flex items-center gap-3 min-w-0 pr-2">
                  ...
                  {userProfile.role === 'admin' && (
                    <div className="pt-4 border-t border-[#24334d] flex justify-center">
                      <button id="btn-add-trending" onClick={() => openAdminModal('trending')} className="bg-[#111c2d] border border-[#00ff75]/50 text-[#00ff75] font-bold px-5 py-2 rounded-xl text-xs hover:bg-[#00ff75] hover:text-black transition flex items-center gap-2">
                        <span>➕</span> ADD NEWS
                      </button>
                    </div>
                  )}
            ...
            <footer id="main-footer" className="border-t border-[#24334d] bg-[#0e1626]/90 mt-16 py-8 px-6 text-xs text-gray-400">
          ...
          {googleIframeModal.open && (
            <div id="modal-google-search" className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex flex-col p-4 sm:p-8">
              <div className="bg-[#0e1626] border border-[#24334d] rounded-2xl p-4 mb-4 flex items-center justify-between">
                ...
                <button id="btn-close-google-modal" onClick={() => setGoogleIframeModal({ open: false, query: '' })} className="w-8 h-8 rounded-full bg-red-950 text-red-300 border border-red-500/35 flex items-center justify-center font-bold text-xs hover:bg-red-800 transition">✕</button>
              </div>
          ...
          <div id="global-chat-widget-container" className="fixed bottom-6 right-6 z-40">
            <button id="btn-toggle-global-chat" onClick={() => setGlobalChatOpen(!globalChatOpen)} className="w-14 h-14 rounded-full bg-[#00ff75] text-black flex items-center justify-center text-xl font-bold shadow-[0_0_25px_rgba(0,255,117,0.4)] hover:scale-105 active:scale-95 transition transform">💬</button>
              
            {globalChatOpen && (
              <div id="global-chat-box" className="absolute bottom-20 right-0 w-80 sm:w-96 bg-[#0e1626] border border-[#24334d] rounded-2xl shadow-2xl flex flex-col h-[480px] overflow-hidden backdrop-blur-xl">
                <div id="global-chat-header" className="bg-[#080c14] p-4 border-b border-[#24334d] flex items-center justify-between">
                  <div className="flex items-center gap-2">
                  ...
                  <button id="btn-close-global-chat" onClick={() => setGlobalChatOpen(false)} className="text-gray-400 hover:text-white font-bold w-6 h-6 flex items-center justify-center rounded hover:bg-white/5">✕</button>
                </div>
                  
                <div id="global-chat-messages-container" className="flex-1 p-4 overflow-y-auto flex flex-col space-y-3 text-xs">
                  {globalChatMessages.length === 0 ? (
                    ...
                    globalChatMessages.map(msg => {
                      ...
                      return (
                        <div key={msg.id} id={`chat-msg-${msg.id}`} className={`flex flex-col max-w-[85%] ${isMe ? 'self-end items-end' : 'self-start items-start'}`}>
                ...
                <div id="global-chat-input-area" className="p-3 border-t border-[#24334d] bg-[#080c14] flex gap-2">
                  <input
                  ...
                  />
                  <button id="btn-send-global-chat" onClick={sendGlobalChatMessage} className="bg-[#00ff75] text-black font-bold px-3 py-2 rounded-xl text-xs hover:bg-[#00d866] transition">Send</button>
                </div>
              </div>
            )}
          </div>
