...
export default function PrePage() {
  ...
  const [loader, setLoader] = useState({ active: true, promptText: 'initializing secure intelligence core...', progress: 25 });
  ...
  const triggerFloatingLoader = (promptText, progressPercentage) => {
    setLoader({
      ...
      progress: progressPercentage !== undefined ? progressPercentage : 50
    });
  };
  ...
  const saveAdminEntry = async () => {
    ...
    try {
      ...
      triggerFloatingLoader("Saving Admin Record...", 50);
  ...
  const deleteMatchFromDB = async (id) => {
    triggerFloatingLoader("Deleting Match Record...", 50);
    try {
  ...
  const deleteFixtureFromDB = async (id) => {
    triggerFloatingLoader("Deleting Fixture Record...", 50);
    try {
  ...
  const deleteTrendingFromDB = async (id) => {
    triggerFloatingLoader("Deleting Headline...", 50);
    try {
  ...
  const renderLiveMatchCard = (match) => {
    ...
    return (
      <div 
        key={match.id} 
        className="bg-[#0f172a] rounded-2xl p-5 border border-cyan-500/20 shadow-[0_10px_30px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.1)] relative overflow-hidden cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:border-cyan-400/60 hover:shadow-[0_15px_40px_rgba(0,240,255,0.2)]"
  ...
  const renderMatchPredictionCard = (match) => {
  ...
  const renderFixtureCard = (fix) => {
    return (
      <div key={fix.id} className="bg-[#0f172a] rounded-xl p-4 border border-slate-800 flex items-center justify-between cursor-pointer transition-all duration-300 hover:border-cyan-500/50 hover:bg-slate-900/50">
  ...
  const renderTrendingCard = (item) => {
    return (
      <div key={item.id} className="bg-[#0f172a] rounded-xl p-4 border border-slate-800 flex items-center justify-between cursor-pointer transition-all duration-300 hover:border-amber-500/50 hover:bg-slate-900/50">
        <div className="flex items-center gap-3">
  ...
  return (
    <div className={`min-h-screen bg-[#090d16] text-slate-100 flex flex-col justify-between selection:bg-emerald-500 selection:text-black ${contrastMode ? 'contrast-200 bg-black' : ''}`}>
      ...
      <div className="app-content-wrapper flex flex-col min-h-screen justify-between relative z-10">
        ...
        <div id="toast-container" className="fixed top-5 right-5 z-50 flex flex-col gap-2 pointer-events-none">
          {toasts.map(t => (
            <div 
              key={t.id} 
              className={`px-4 py-3 rounded-xl border text-xs font-bold font-mono shadow-2xl flex items-center gap-2 pointer-events-auto backdrop-blur-md transition-all duration-300 animate-bounce ${
        ...
        <div id="floating-loader" className={`fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4 transition-opacity duration-300 ${loader.active ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
          <div className="bg-[#0f172a] border border-cyan-500/40 rounded-2xl p-6 max-w-sm w-full space-y-4 shadow-[0_0_50px_rgba(0,240,255,0.2)] text-center transform transition-transform duration-300">
            <div className="flex items-center justify-center gap-3">
              <span className="w-3 h-3 rounded-full bg-cyan-400 animate-ping"></span>
              <h4 className="text-xs font-bold uppercase tracking-widest text-cyan-400 font-mono">QUANTUM SYNC</h4>
            </div>
            <p id="loader-prompt-text" className="text-sm font-medium text-slate-200">{loader.promptText}</p>
            <div className="w-full bg-slate-900 h-2 rounded-full overflow-hidden p-0.5 border border-slate-800">
              <div id="loader-progress" className="bg-gradient-to-r from-cyan-500 to-emerald-400 h-full rounded-full transition-all duration-300" style={{ width: `${loader.progress}%` }}></div>
            </div>
          </div>
        </div>
        ...
        {reactionModal.open && (
          <div id="reaction-users-modal" className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-[#0f172a] border border-emerald-500/40 rounded-2xl w-full max-w-sm p-5 space-y-4 shadow-2xl relative">
        ...
        <div id="app-root">
          ...
          {dbError && (
            <div id="database-error-center" className="max-w-7xl mx-auto px-6 pt-6">
              <div className="bg-red-950/50 border border-red-500/40 rounded-2xl p-5 shadow-2xl">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3">
                    <div className="w-9 h-9 rounded-xl bg-red-500/10 border border-red-500/30 flex items-center justify-center text-red-400 font-bold">⚠</div>
                    <div>
                      <h3 className="font-extrabold text-red-400 text-sm uppercase tracking-wider">DATABASE ERROR</h3>
                      <p className="text-xs text-slate-300 mt-1">{dbError.title}</p>
                    </div>
                  </div>
                  <button onClick={hideDatabaseError} className="text-slate-500 hover:text-white">✕</button>
                </div>
                <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="bg-black/40 border border-red-500/10 rounded-xl p-3">
                    <span className="text-[10px] uppercase tracking-wider text-slate-500">Diagnostic Reason</span>
                    <p className="text-xs text-red-300 mt-1 font-semibold">{dbError.diagnosis}</p>
                  </div>
                  <div className="bg-black/40 border border-red-500/10 rounded-xl p-3">
                    <span className="text-[10px] uppercase tracking-wider text-slate-500">Recommended Fix</span>
                    <p className="text-xs text-slate-300 mt-1">{dbError.action}</p>
                  </div>
                  <div className="bg-black/40 border border-red-500/10 rounded-xl p-3">
                    <span className="text-[10px] uppercase tracking-wider text-slate-500">Technical Trace</span>
          ...
          <main className="max-w-7xl mx-auto px-6 py-8 space-y-12">
            ...
            {/* Live Section */}
            <section id="live-section" className="bg-[#0b0f19] border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-6">
              <div className="flex items-center justify-between section-header">
                <div>
                  <h3 className="font-extrabold text-lg uppercase tracking-wide text-white font-mono flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-rose-500 animate-pulse"></span> LIVE MATCHES
                  </h3>
                  <span className="text-xs text-slate-400">{liveMatchesData.length} Active Match{liveMatchesData.length === 1 ? '' : 'es'}</span>
                </div>
                <button onClick={() => openStatsListModal('Live Games Directory', liveMatchesData)} className="bg-slate-900 border border-slate-800 text-emerald-400 hover:bg-emerald-500 hover:text-black font-bold px-4 py-2 rounded-xl text-xs transition flex items-center gap-1.5">
                  <span>SEE MORE MATCHES</span> ➔
                </button>
              </div>
              <div id="live-matches-container" className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {liveMatchesData.length === 0 ? (
                  <div className="col-span-3 text-center py-8 bg-[#0f172a] rounded-2xl border border-slate-800"><p className="text-xs text-slate-400">No live matches currently in play.</p></div>
                ) : (
                  liveMatchesData.slice(0, 3).map(m => renderLiveMatchCard(m))
                )}
              </div>
            </section>

            {/* Predictions Section */}
            <section id="db-matches-section" className="bg-[#0b0f19] border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-6">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 section-header">
                <div>
                  <h3 
                    className="text-lg font-extrabold uppercase tracking-wide text-white font-mono cursor-pointer hover:text-emerald-400 transition" 
                    onClick={() => openGoogleSearchIframe('Live database matches and football predictions')}
                  >
                    ⚽ MATCHES & PREDICTIONS
                  </h3>
                  <p className="text-xs text-slate-400 mt-0.5">Click title for Google search context.</p>
                </div>
                <button onClick={() => openStatsListModal('All Database Predictions', matchesData)} className="bg-slate-900 border border-slate-800 text-emerald-400 hover:bg-emerald-500 hover:text-black font-bold px-4 py-2 rounded-xl text-xs transition flex items-center gap-1.5">
                  <span>SEE MORE</span> ➔
                </button>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
                ...
                <div className="flex items-center gap-2 bg-slate-950 p-1.5 rounded-xl border border-slate-800 self-start">
                  <button 
                    ...
                    className={`px-4 py-1.5 rounded-lg text-xs font-bold font-mono transition ${activeMatchTab === 'future' ? 'bg-emerald-500 text-black shadow-md' : 'text-slate-400 hover:text-white'}`}
                  >
                    UPCOMING MATCHES
                  </button>
                  <button 
                    onClick={() => setActiveMatchTab('past')} 
                    className={`px-4 py-1.5 rounded-lg text-xs font-bold font-mono transition ${activeMatchTab === 'past' ? 'bg-emerald-500 text-black shadow-md' : 'text-slate-400 hover:text-white'}`}
                  >
                    PAST PREDICTIONS
                  </button>
                </div>
                ...
                <div className="relative flex-1 max-w-md">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-8 pr-4 py-2 text-xs text-white focus:outline-none focus:border-emerald-500 transition"
                  />
                </div>
              </div>

              <div id="matches-container" className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {(() => {
                  const filtered = getFilteredMatches();
                  if (!filtered.length) {
                    return (
                      <div className="col-span-3 text-center py-10 bg-[#0f172a] rounded-2xl border border-slate-800">
                        <p className="text-xs text-slate-400">No {activeMatchTab === 'past' ? 'past' : 'upcoming'} matches found matching query "{matchSearchQuery}".</p>
                      </div>
                    );
                  }
                  return filtered.slice(0, 3).map(m => renderMatchPredictionCard(m));
                })()}
              </div>

              {userProfile.role === 'admin' && (
                <div className="mt-6 pt-4 border-t border-slate-800 flex justify-center">
                  <button onClick={() => openFloatingAdminModal('matches')} className="bg-emerald-500 text-black font-extrabold px-6 py-2.5 rounded-full text-xs uppercase tracking-wider hover:bg-emerald-400 transition flex items-center gap-2 shadow-lg">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              {/* Fixtures */}
              <section id="fixtures-section" className="bg-[#0b0f19] border border-slate-800 rounded-3xl p-6 flex flex-col justify-between space-y-4 shadow-2xl">
                <div>
                  <div className="flex items-center justify-between mb-6 section-header">
                    <div>
                      <h3 
                        className="font-extrabold uppercase tracking-wide text-base text-white font-mono cursor-pointer hover:text-cyan-400 transition" 
                    ...
                    <button onClick={() => openStatsListModal('Complete Fixtures Schedule', fixturesData)} className="bg-slate-900 border border-slate-800 text-cyan-400 hover:bg-cyan-500 hover:text-black font-bold px-3.5 py-1.5 rounded-xl text-xs transition flex items-center gap-1">
                      <span>SEE MORE</span> ➔
                    </button>
                  </div>
                  <div id="fixtures-container" className="space-y-4">
                    {fixturesData.length === 0 ? (
                      ...
                      fixturesData.slice(0, 3).map(f => renderFixtureCard(f))
                    )}
                  </div>
                </div>
                {userProfile.role === 'admin' && (
                  <div className="pt-4 border-t border-slate-800 flex justify-center">
                    <button onClick={() => openFloatingAdminModal('fixtures')} className="bg-slate-900 border border-cyan-500 text-cyan-400 font-bold px-5 py-2 rounded-xl text-xs hover:bg-cyan-500 hover:text-black transition flex items-center gap-2">
              ...
              {/* Trending News */}
              <section id="trending-section" className="bg-[#0b0f19] border border-slate-800 rounded-3xl p-6 flex flex-col justify-between space-y-4 shadow-2xl">
                <div>
                  <div className="flex items-center justify-between mb-6 section-header">
                    <div>
                      <h3 
                        className="font-extrabold uppercase tracking-wide text-base text-white font-mono cursor-pointer hover:text-amber-400 transition" 
                        onClick={() => openGoogleSearchIframe('Trending football news updates')}
                      >
                        🔥 TRENDING NEWS
                      </h3>
                      <span className="text-xs text-slate-400">What's trending.</span>
                    </div>
                    <button onClick={() => openStatsListModal('All Trending News', trendingData)} className="bg-slate-900 border border-slate-800 text-amber-400 hover:bg-amber-500 hover:text-black font-bold px-3.5 py-1.5 rounded-xl text-xs transition flex items-center gap-1">
                      <span>SEE MORE</span> ➔
                    </button>
                  </div>
                  <div id="trending-container" className="space-y-4">
                    {trendingData.length === 0 ? (
                      ...
                      trendingData.slice(0, 3).map(t => renderTrendingCard(t))
                    )}
                  </div>
                </div>
                {userProfile.role === 'admin' && (
                  <div className="pt-4 border-t border-slate-800 flex justify-center">
                    <button onClick={() => openFloatingAdminModal('trending')} className="bg-slate-900 border border-amber-500 text-amber-400 font-bold px-5 py-2 rounded-xl text-xs hover:bg-amber-500 hover:text-black transition flex items-center gap-2">
                      <span>➕</span> ADD NEWS
                    </button>
                  </div>
                )}
        ...
        {googleIframeModal.open && (
          <div id="google-iframe-modal" className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex flex-col p-3 sm:p-6">
            <div className="bg-[#0f172a] border border-slate-800 rounded-2xl p-3 mb-3 flex items-center justify-between shadow-xl">
              <div className="flex items-center gap-3">
          ...
          <div id="chat-drawer" className={`${globalChatOpen ? '' : 'hidden'} absolute bottom-20 right-0 w-80 sm:w-96 bg-[#0f172a] border border-slate-800 rounded-2xl shadow-2xl flex flex-col h-[480px] overflow-hidden`}>
            ...
            <div className="p-3 border-t border-slate-800 bg-slate-950 flex gap-2">
              <input 
                ...
                placeholder="Type community message..." 
                className="flex-1 bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-emerald-500 transition"
              />
        ...
        {commentsModal.open && (
          <div id="fullscreen-comments-modal" className="fixed inset-0 bg-slate-950/95 backdrop-blur-xl z-50 p-4 md:p-10 overflow-y-auto flex flex-col justify-between">
            <div className="max-w-4xl w-full mx-auto bg-[#0f172a] border border-emerald-500/40 rounded-3xl p-6 md:p-8 shadow-2xl relative flex-1 flex flex-col justify-between space-y-6">
              ...
              <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3">
                ...
                <div className="flex gap-3">
                  <textarea 
                    ...
                    className="flex-1 bg-slate-900 border border-slate-800 rounded-xl p-3 text-xs text-white focus:outline-none focus:border-emerald-500 transition"
                  />
        ...
        {statsModal.open && (
          <div id="stats-list-modal" className="fixed inset-0 bg-slate-950/95 backdrop-blur-md z-50 overflow-y-auto p-4 md:p-10">
            <div className="max-w-5xl mx-auto bg-[#0f172a] border border-slate-800 rounded-3xl p-6 md:p-10 shadow-2xl relative min-h-[85vh] flex flex-col justify-between">
        ...
        {fullscreenMatchModal.open && fullscreenMatchModal.match && (
          <div id="fullscreen-match-modal" className="fixed inset-0 bg-slate-950/95 backdrop-blur-md z-50 overflow-y-auto p-4 md:p-10">
            <div className="max-w-5xl mx-auto bg-[#0f172a] border border-slate-800 rounded-3xl p-6 md:p-10 shadow-2xl relative min-h-[85vh] flex flex-col justify-between">
        ...
        {/* Match Chat Modal */}
        {matchChatModal.open && (
          <div id="match-chat-modal" className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-[#0f172a] border border-slate-800 rounded-3xl w-full max-w-2xl h-[80vh] flex flex-col overflow-hidden shadow-2xl">
              <div className="bg-slate-950 p-4 border-b border-slate-800 flex items-center justify-between">
                <div>
        ...
        {/* Hotline Modal */}
        {dialingModalOpen && (
          <div id="dialing-modal" className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-[#0f172a] border border-slate-800 rounded-3xl w-full max-w-2xl p-6 md:p-8 space-y-6 shadow-2xl">
        ...
        {/* Admin Modal */}
        {adminModal.open && (
          <div id="admin-floating-modal" className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-[#0f172a] border border-slate-800 rounded-3xl w-full max-w-2xl p-8 space-y-6 shadow-2xl relative">
              <div className="flex justify-between items-center border-b border-slate-800 pb-4">
                <div>
        ...
        {/* Settings Modal */}
        {settingsModalOpen && (
          <div id="settings-modal" className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-[#0f172a] border border-slate-800 rounded-3xl w-full max-w-md p-6 space-y-6 shadow-2xl">
              <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              ...
              <div className="space-y-4 text-xs">
                <div>
                  ...
                  <select id="pref-odds-format" value={userProfile.odds_format} onChange={updateProfileSettings} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-white focus:outline-none focus:border-emerald-500 transition">
                ...
                <div>
                  ...
                  <select id="pref-language" value={userProfile.language} onChange={updateProfileSettings} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-white focus:outline-none focus:border-emerald-500 transition">
                ...
                <div className="flex items-center justify-between pt-2">
                  ...
                  <input type="checkbox" id="pref-contrast" checked={userProfile.high_contrast} onChange={toggleContrastMode} className="w-4 h-4 accent-emerald-500 cursor-pointer" />
                </div>
              </div>
              <button onClick={() => setSettingsModalOpen(false)} className="w-full bg-emerald-500 text-black font-bold py-2 rounded-xl hover:bg-emerald-400 transition">Save & Close</button>
            </div>
          </div>
        )}
