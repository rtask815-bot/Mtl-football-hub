  ...
  return (
    <div className={`min-h-screen flex flex-col justify-between selection:bg-mtlGreen selection:text-black ${userProfile.high_contrast ? 'high-contrast-mode' : ''}`} style={{ backgroundColor: '#0b0f19', color: '#f9fafb', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="app-content-wrapper flex flex-col min-h-screen justify-between relative z-10">
                    ...
                    return (
                      <div key={match.id} onClick={() => { setActiveFullscreenMatch(match); setFullscreenMatchModalOpen(true); }} className="bg-gradient-to-br from-[#1f2937]/90 to-[#111827]/95 backdrop-blur-md border border-[#374151]/40 rounded-2xl p-5 relative overflow-hidden cursor-pointer transition hover:border-[#10b981] hover:-translate-y-1 shadow-xl">
                        ...
                        <div className="flex items-center justify-between my-4">
                          ...
                          <div className="text-2xl font-extrabold tracking-wider px-2 font-cyber text-[#10b981]">{sanitizeInput(match.score || '0 - 0')}</div>
                          <div className="text-center flex-1">
        ...
        {matchChatModalOpen && (
          <div className="fixed inset-0 bg-[#0b0f19]/95 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-[#111827] border border-[#374151] rounded-2xl w-full max-w-lg p-5 space-y-4 shadow-2xl relative flex flex-col h-[520px]">
              <div className="flex justify-between items-center border-b border-[#374151] pb-3">
                <h4 className="font-bold text-sm text-[#10b981] font-cyber">Telegram Match Chat: {activeMatchChat.teams}</h4>
                <button onClick={() => setMatchChatModalOpen(false)} className="text-gray-400 hover:text-white font-bold">✕</button>
              </div>
              <div ref={matchChatScrollRef} className="flex-1 overflow-y-auto space-y-3 p-2 text-xs">
                {(!matchChatStore[activeMatchChat.id] || matchChatStore[activeMatchChat.id].length === 0) ? (
                  <p className="text-gray-500 italic text-center py-10">No match messages yet. Start chatting!</p>
                ) : (
                  matchChatStore[activeMatchChat.id].map((msg) => {
                    const isMe = currentUser && msg.user_id === currentUser.id;
                    return (
                      <div key={msg.id} className={`flex flex-col max-w-[80%] ${isMe ? 'self-end items-end' : 'self-start items-start'}`}>
                        <span className="text-[9px] text-gray-400 mb-0.5">{sanitizeInput(msg.user)} • {sanitizeInput(msg.time)}</span>
                        <div className={`px-3 py-2 rounded-xl ${isMe ? 'bg-[#10b981] text-black font-medium' : 'bg-[#1f2937] text-white border border-[#374151]'}`}>
                          {sanitizeInput(msg.text)}
                        </div>
                      </div>
                    );
              ...
              <div className="flex gap-2 pt-2 border-t border-[#374151]">
                <input type="text" value={matchChatInput} onChange={(e) => setMatchChatInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && sendMatchChatMessage()} placeholder="Type match message..." className="flex-1 bg-[#0b0f19] border border-[#374151] rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-[#10b981]" />
                <button onClick={sendMatchChatMessage} className="bg-[#10b981] text-black font-bold px-4 py-2 rounded-xl text-xs hover:bg-[#059669] transition">Send</button>
              </div>
            </div>
          </div>
        )}

        {/* Contact Centre / Hotline Modal */}
        {dialingModalOpen && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-[#111827] border border-[#10b981] rounded-2xl w-full max-w-md p-6 space-y-5 shadow-2xl relative text-center">
              <button onClick={() => setDialingModalOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-white font-bold">✕</button>
              <div className="w-12 h-12 mx-auto rounded-full bg-[#10b981]/20 text-[#10b981] flex items-center justify-center text-xl font-bold">📞</div>
              <h3 className="text-lg font-bold font-cyber text-white">INTELLIGENCE CONTACT CENTRE</h3>
              <p className="text-xs text-gray-300 leading-relaxed">Direct connection to lead developer & analytics coordinator **M. Lennox**.</p>
              <div className="bg-[#0b0f19] p-4 rounded-xl border border-[#374151] space-y-2 text-left">
                <div className="flex justify-between text-xs"><span className="text-gray-400">Developer:</span> <span className="text-white font-bold">M. Lennox</span></div>
                <div className="flex justify-between text-xs"><span className="text-gray-400">Direct Hotline:</span> <span className="text-[#10b981] font-mono font-bold">+254 (0) 700 000 000</span></div>
                <div className="flex justify-between text-xs"><span className="text-gray-400">Status:</span> <span className="text-emerald-400 font-bold">Active / Online</span></div>
              </div>
              <a href="tel:+254700000000" className="block w-full bg-[#10b981] text-black font-extrabold py-3 rounded-xl text-xs uppercase tracking-wider hover:bg-[#059669] transition">
                Call Support Line
              </a>
            </div>
          </div>
        ...
        {settingsModalOpen && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-[#111827] border border-[#374151] rounded-2xl w-full max-w-md p-6 space-y-5 shadow-2xl relative">
              <div className="flex justify-between items-center border-b border-[#374151] pb-3">
                <h3 className="text-sm font-bold font-cyber text-[#10b981]">PREFERENCES & SETTINGS</h3>
              ...
              <div className="space-y-4 text-xs">
                ...
                <div className="flex items-center justify-between p-3 bg-[#0b0f19] rounded-xl border border-[#374151]">
              ...
              <button onClick={() => setSettingsModalOpen(false)} className="w-full bg-[#10b981] text-black font-bold py-2.5 rounded-xl text-xs">Save Preferences</button>
            </div>
          </div>
        )}
        ...
        {adminModalOpen && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-[#111827] border border-[#10b981] rounded-2xl w-full max-w-lg p-6 space-y-4 shadow-2xl relative max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center border-b border-[#374151] pb-3">
                <h3 className="text-sm font-bold font-cyber text-[#10b981] uppercase">{editingItem ? 'Edit' : 'Create'} {adminSection}</h3>
                <button onClick={() => setAdminModalOpen(false)} className="text-gray-400 hover:text-white font-bold">✕</button>
              </div>
              ...
              <div className="space-y-3 text-xs">
                {adminSection === 'matches' && (
                  <>
                    <div>
                      <label className="text-gray-400 block mb-1">Teams (e.g., Arsenal vs Chelsea)</label>
                      <input type="text" value={adminFormData.teams || ''} onChange={(e) => setAdminFormData({ ...adminFormData, teams: e.target.value })} className="w-full bg-[#0b0f19] border border-[#374151] rounded-xl p-2 text-white" />
                    </div>
                    <div>
                      <label className="text-gray-400 block mb-1">League</label>
                      <input type="text" value={adminFormData.league || ''} onChange={(e) => setAdminFormData({ ...adminFormData, league: e.target.value })} className="w-full bg-[#0b0f19] border border-[#374151] rounded-xl p-2 text-white" />
                    </div>
                    <div>
                      <label className="text-gray-400 block mb-1">Prediction</label>
                      <input type="text" value={adminFormData.prediction || ''} onChange={(e) => setAdminFormData({ ...adminFormData, prediction: e.target.value })} className="w-full bg-[#0b0f19] border border-[#374151] rounded-xl p-2 text-white" />
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="text-gray-400 block mb-1">Date (YYYY-MM-DD)</label>
                        <input type="text" value={adminFormData.match_date || ''} onChange={(e) => setAdminFormData({ ...adminFormData, match_date: e.target.value })} className="w-full bg-[#0b0f19] border border-[#374151] rounded-xl p-2 text-white" />
                      </div>
                      <div>
                        <label className="text-gray-400 block mb-1">Time (HH:MM)</label>
                        <input type="text" value={adminFormData.match_time || ''} onChange={(e) => setAdminFormData({ ...adminFormData, match_time: e.target.value })} className="w-full bg-[#0b0f19] border border-[#374151] rounded-xl p-2 text-white" />
                      </div>
                    </div>
                    <div>
                      <label className="text-gray-400 block mb-1">Decimal Odds</label>
                      <input type="number" step="0.01" value={adminFormData.decimal_odds || ''} onChange={(e) => setAdminFormData({ ...adminFormData, decimal_odds: parseFloat(e.target.value) })} className="w-full bg-[#0b0f19] border border-[#374151] rounded-xl p-2 text-white" />
                    </div>
                  </>
                )}
                ...
                {adminSection === 'fixtures' && (
                  <>
                    <div>
                      <label className="text-gray-400 block mb-1">Teams</label>
                      <input type="text" value={adminFormData.teams || ''} onChange={(e) => setAdminFormData({ ...adminFormData, teams: e.target.value })} className="w-full bg-[#0b0f19] border border-[#374151] rounded-xl p-2 text-white" />
                    </div>
                    <div>
                      <label className="text-gray-400 block mb-1">League</label>
                ...
                {adminSection === 'trending' && (
                  <>
                    <div>
                      ...
                      <input type="text" value={adminFormData.title || ''} onChange={(e) => setAdminFormData({ ...adminFormData, title: e.target.value })} className="w-full bg-[#0b0f19] border border-[#374151] rounded-xl p-2 text-white" />
                    </div>
                    <div>
                      <label className="text-gray-400 block mb-1">Rank (#1, #2, etc.)</label>
                      <input type="text" value={adminFormData.rank || ''} onChange={(e) => setAdminFormData({ ...adminFormData, rank: e.target.value })} className="w-full bg-[#0b0f19] border border-[#374151] rounded-xl p-2 text-white" />
                    </div>
                  </>
                )}
              </div>
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-[#374151]">
                <button onClick={() => setAdminModalOpen(false)} className="bg-[#1f2937] text-gray-300 font-bold px-4 py-2 rounded-xl text-xs">Cancel</button>
                <button onClick={saveAdminEntry} className="bg-[#10b981] text-black font-bold px-5 py-2 rounded-xl text-xs hover:bg-[#059669]">Save Entry</button>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="max-w-5xl mx-auto bg-[#111827] border border-[#374151] rounded-2xl p-6 md:p-10 shadow-2xl relative min-h-[85vh] flex flex-col justify-between">
              ...
              <div className="space-y-8">
                <div className="flex justify-between items-start border-b border-[#374151] pb-6 border-l-4 border-[#10b981] pl-3">
                  <div>
                    <span className="px-3 py-1 rounded-full text-xs font-extrabold uppercase bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">MATCH INTEL</span>
                    <h2 className="text-3xl lg:text-4xl font-extrabold text-white font-cyber mt-2">{sanitizeInput(activeFullscreenMatch.teams)}</h2>
                    <p className="text-xs text-gray-400 mt-1">{sanitizeInput(activeFullscreenMatch.league)} • {sanitizeInput(activeFullscreenMatch.match_date)} {sanitizeInput(activeFullscreenMatch.match_time)}</p>
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-[#0b0f19] p-5 rounded-2xl border border-[#374151] space-y-3">
                    <h4 className="text-xs font-bold text-[#10b981] uppercase tracking-wider font-cyber">AI Prediction Output</h4>
                    <p className="text-2xl font-bold text-white font-cyber">{sanitizeInput(activeFullscreenMatch.prediction || 'N/A')}</p>
                    <p className="text-xs text-amber-400 font-bold font-cyber">Decimal Odds: {formatOdds(activeFullscreenMatch.decimal_odds)}</p>
                  </div>

                  <div className="bg-[#0b0f19] p-5 rounded-2xl border border-[#374151] space-y-3">
                    <h4 className="text-xs font-bold text-[#10b981] uppercase tracking-wider font-cyber">Probability Distribution</h4>
                    <div className="space-y-2 text-xs">
                      <div className="flex justify-between text-gray-300"><span>Home Win:</span> <span>{activeFullscreenMatch.prob_home || 0}%</span></div>
                      <div className="flex justify-between text-gray-300"><span>Draw:</span> <span>{activeFullscreenMatch.prob_draw || 0}%</span></div>
                      <div className="flex justify-between text-gray-300"><span>Away Win:</span> <span>{activeFullscreenMatch.prob_away || 0}%</span></div>
                    </div>
                  </div>
                </div>

                <div className="bg-[#0b0f19] p-5 rounded-2xl border border-[#374151] space-y-2">
                  <h4 className="text-xs font-bold text-[#10b981] uppercase tracking-wider font-cyber">Tactical Analysis & Breakdown</h4>
                  <p className="text-xs text-gray-300 leading-relaxed whitespace-pre-wrap">{sanitizeInput(activeFullscreenMatch.analysis_text || 'No tactical breakdown supplied.')}</p>
                </div>
              </div>
