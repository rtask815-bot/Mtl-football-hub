...
export default function PrePage() {
  ...
  // Long Press Helper Component / Hook simulator
  const useLongPress = (onLongPress, onClick, delay = 850) => {
  ...
  const renderMatchPredictionCard = (match) => {
    ...
    return (
      <div key={match.id} className="pro-card p-5 flex flex-col justify-between space-y-4 cursor-pointer animate-slide-in active">
        ...
        <div className="pt-2 border-t border-mtlCardBorder flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <button 
              ...
              className="bg-mtlDark border border-mtlCardBorder px-3 py-1.5 rounded-xl text-xs text-gray-200 hover:text-mtlGreen transition flex items-center gap-1"
            >
              💬 Telegram Chat
            </button>
          </div>
  ...
  const renderFixtureCard = (fix) => {
    return (
      <div key={fix.id} className="pro-card p-4 flex items-center justify-between cursor-pointer animate-slide-in active">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-mtlDark border border-mtlCardBorder flex items-center justify-center font-bold text-xs text-mtlGreen font-cyber">{fix.badge}</div>
          <div>
            <h4 className="font-bold text-xs text-white hover:text-mtlGreen" onClick={(e) => { e.stopPropagation(); openGoogleSearchIframe(`Fixture schedule ${fix.teams}`); }}>{fix.teams || 'Fixture'}</h4>
            <span className="text-[10px] text-gray-400">{fix.league || 'League'}</span>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className="text-right">
            <span className="text-xs font-bold text-mtlGreen block font-cyber">{fix.match_time || 'TBD'}</span>
            <span className="text-[10px] text-gray-500">{fix.match_date || 'TBD'}</span>
          </div>
          {userProfile.role === 'admin' && (
            <div className="flex gap-1 ml-2">
              <button onClick={(e) => { e.stopPropagation(); triggerAdminEdit('fixtures', fix.id); }} className="text-[10px] bg-amber-500/20 text-amber-400 border border-amber-500/30 px-2 py-1 rounded hover:bg-amber-500 hover:text-black">Edit</button>
  ...
  const renderTrendingCard = (item) => {
    return (
      <div key={item.id} className="pro-card p-4 flex items-center justify-between cursor-pointer animate-slide-in active">
        <div className="flex items-center gap-3">
  ...
  return (
    <div className={`min-h-screen flex flex-col justify-between selection:bg-mtlGreen selection:text-black ${contrastMode ? 'high-contrast-mode' : ''}`}>
      ...
      <div className="app-content-wrapper flex flex-col min-h-screen justify-between relative z-10">
        ...
        <div id="toast-container" className="fixed top-5 right-5 z-50 flex flex-col gap-2 pointer-events-none">
          {toasts.map(t => (
            <div 
              key={t.id} 
              className={`px-4 py-3 rounded-xl border text-xs font-bold font-cyber shadow-2xl flex items-center gap-2 pointer-events-auto ${
        ...
        <div id="floating-loader" className={`floating-loader-overlay ${loader.active ? 'active' : ''}`}>
          <div className="loader-card space-y-4">
            ...
            <p id="loader-prompt-text" className="text-sm font-medium text-gray-200">{loader.promptText}</p>
            <div className="water-progress-container">
          ...
          <div id="reaction-users-modal" className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-mtlSurface border border-mtlGreen rounded-2xl w-full max-w-sm p-5 space-y-4 shadow-2xl relative animate-slide-in active">
              <div className="flex justify-between items-center border-b border-mtlCardBorder pb-3">
                ...
                <button onClick={closeReactionUsersModal} className="text-gray-400 hover:text-white font-bold">✕</button>
              </div>
              <div className="space-y-2 max-h-60 overflow-y-auto text-xs">
                {(() => {
                  ...
                  if (!usersForType.length) return <p className="text-gray-500 italic py-2">No users have put this reaction yet.</p>;
                  return usersForType.map((u, idx) => (
                    <div key={idx} className="flex items-center gap-2 p-2 bg-mtlDark rounded-lg border border-mtlCardBorder">
              ...
              <button onClick={closeReactionUsersModal} className="w-full bg-mtlCard border border-mtlCardBorder text-gray-300 py-2 rounded-xl text-xs font-bold hover:text-white">Close</button>
            </div>
          </div>
        )}
        ...
        <aside 
          id="side-nav-menu" 
          className={`fixed top-0 right-0 h-full w-80 max-w-[85vw] bg-mtlSurface border-l border-mtlCardBorder z-50 transform ${sideNavOpen ? 'translate-x-0' : 'translate-x-full'} transition-transform duration-300 ease-in-out flex flex-col justify-between p-6 shadow-2xl`}
        >
          <div className="space-y-6">
            <div className="flex items-center justify-between border-b border-mtlCardBorder pb-4">
              ...
              <button onClick={toggleSideNav} className="w-8 h-8 rounded-full bg-mtlDark text-gray-400 hover:text-white flex items-center justify-center font-bold">✕</button>
            </div>
            ...
            <nav className="space-y-3">
              ...
              <button onClick={() => { setDialingModalOpen(true); toggleSideNav(); }} className="w-full flex items-center gap-3 p-3 rounded-xl bg-mtlCard border border-mtlCardBorder hover:border-mtlGreen hover:text-mtlGreen transition text-xs font-semibold text-gray-200">
                <span className="text-base">📞</span> Contact Centre
              </button>
              <button onClick={() => { setSettingsModalOpen(true); toggleSideNav(); }} className="w-full flex items-center gap-3 p-3 rounded-xl bg-mtlCard border border-mtlCardBorder hover:border-mtlGreen hover:text-mtlGreen transition text-xs font-semibold text-gray-200">
          ...
          <header className="border-b border-mtlCardBorder bg-mtlSurface/90 backdrop-blur-md sticky top-0 z-40 px-6 py-4">
            <div className="max-w-7xl mx-auto flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div id="brand-logo" className="w-10 h-10 avatar-logo text-lg cursor-pointer" onClick={() => setDialingModalOpen(true)}>
              ...
              <div className="flex items-center gap-3">
                <button onClick={toggleSideNav} title="Open Navigation Options" className="w-10 h-10 rounded-full bg-mtlCard border border-mtlCardBorder text-gray-200 flex items-center justify-center hover:text-mtlGreen transition">
                  ☰
                </button>
                
                <div id="user-profile-badge" className="hidden md:flex items-center gap-3 bg-mtlCard border border-mtlCardBorder px-3 py-1.5 rounded-full cursor-pointer" onClick={toggleSideNav}>
          ...
          <main className="max-w-7xl mx-auto px-6 py-8 space-y-12">
            ...
            <section id="live-section" className="animate-slide-in">
              <div className="flex items-center justify-between mb-6 section-header">
                ...
                <button onClick={() => openStatsListModal('Live Games Directory', liveMatchesData)} className="btn-see-more">
            ...
            <section id="db-matches-section" className="bg-mtlSurface border border-mtlCardBorder rounded-2xl p-6 animate-slide-in shadow-2xl">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4 section-header">
                ...
                <button onClick={() => openStatsListModal('All Database Predictions', matchesData)} className="btn-see-more">
                  <span>SEE MORE</span> ➔
                </button>
              </div>
              ...
              {userProfile.role === 'admin' && (
                <div className="mt-6 pt-4 border-t border-mtlCardBorder flex justify-center">
                  <button onClick={() => openFloatingAdminModal('matches')} className="bg-mtlGreen text-black font-extrabold px-6 py-2.5 rounded-full text-xs uppercase tracking-wider hover:bg-mtlGreenHover transition flex items-center gap-2">
              ...
              <section id="fixtures-section" className="bg-mtlSurface border border-mtlCardBorder rounded-2xl p-6 flex flex-col justify-between space-y-4 animate-slide-in shadow-2xl">
                <div>
                  <div className="flex items-center justify-between mb-6 section-header">
                    ...
                    <button onClick={() => openStatsListModal('Complete Fixtures Schedule', fixturesData)} className="btn-see-more text-xs py-2 px-3.5">
                      <span>SEE MORE</span> ➔
                    </button>
                  </div>
              ...
              <section id="trending-section" className="bg-mtlSurface border border-mtlCardBorder rounded-2xl p-6 flex flex-col justify-between space-y-4 animate-slide-in shadow-2xl">
                <div>
                  <div className="flex items-center justify-between mb-6 section-header">
                    ...
                    <button onClick={() => openStatsListModal('All Trending News', trendingData)} className="btn-see-more text-xs py-2 px-3.5">
                      <span>SEE MORE</span> ➔
                    </button>
                  </div>
            ...
            <div className="bg-mtlSurface border border-mtlCardBorder rounded-2xl p-3 mb-3 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p id="google-search-query-display" className="text-[10px] text-gray-400 font-mono">Automated AI Mode: "{googleIframeModal.query}"</p>
                </div>
              </div>
        ...
        <div id="floating-chat-container" className="fixed bottom-6 right-6 z-40">
          <button onClick={() => setGlobalChatOpen(prev => !prev)} className="w-14 h-14 rounded-full bg-mtlGreen text-black flex items-center justify-center text-2xl font-bold shadow-lg hover:scale-105 transition transform">💬</button>
          <div id="chat-drawer" className={`${globalChatOpen ? '' : 'hidden'} absolute bottom-20 right-0 w-80 sm:w-96 bg-mtlSurface border border-mtlCardBorder rounded-2xl shadow-2xl flex flex-col h-[480px] overflow-hidden`}>
                ...
                globalChatMessages.map(msg => {
                  ...
                  return (
                    <div key={msg.id} className={`flex flex-col max-w-[80%] ${isMe ? 'self-end items-end' : 'self-start items-start'} animate-slide-in active`}>
                      ...
                      <div className={`px-3.5 py-2 text-xs ${isMe ? 'chat-bubble-me' : 'chat-bubble-other'}`}>{msg.text}</div>
                    </div>
                  );
                })
              )}
        ...
        {commentsModal.open && (
          <div id="fullscreen-comments-modal" className="fixed inset-0 bg-mtlDark/95 backdrop-blur-xl z-50 p-4 md:p-10 overflow-y-auto flex flex-col justify-between">
            <div className="max-w-4xl w-full mx-auto bg-mtlSurface border border-mtlGreen/40 rounded-3xl p-6 md:p-8 shadow-2xl relative flex-1 flex flex-col justify-between space-y-6">
              ...
              <div className="flex items-center justify-between border-b border-mtlCardBorder pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg md:text-xl font-extrabold text-white font-cyber">Comments Stream: {commentsModal.teams || 'Match Thread'}</h3>
                    <p className="text-xs text-mtlGreen">Leave a comment.</p>
                  </div>
                </div>
                <button onClick={closeFullscreenCommentsModal} className="w-10 h-10 rounded-full bg-mtlCard border border-mtlCardBorder text-gray-400 hover:text-white flex items-center justify-center text-lg font-bold">✕</button>
              </div>
              ...
              <div className="flex justify-end pt-2 border-t border-mtlCardBorder">
                <button onClick={closeFullscreenCommentsModal} className="btn-see-more">Close</button>
              </div>
            </div>
          </div>
        )}
        ...
        {statsModal.open && (
          <div id="stats-list-modal" className="fixed inset-0 bg-mtlDark/95 backdrop-blur-md z-50 overflow-y-auto p-4 md:p-10">
            <div className="max-w-5xl mx-auto bg-mtlSurface border border-mtlCardBorder rounded-2xl p-6 md:p-10 shadow-2xl relative min-h-[85vh] flex flex-col justify-between">
              <button onClick={closeStatsListModal} className="absolute top-6 right-6 w-10 h-10 rounded-full bg-mtlCard border border-mtlCardBorder text-gray-400 hover:text-white flex items-center justify-center text-lg font-bold">✕</button>
              <div className="space-y-6">
                <div className="border-b border-mtlCardBorder pb-4 section-header">
                  <h3 className="text-2xl font-extrabold text-mtlGreen uppercase tracking-wider font-cyber">{statsModal.title}</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-h-[65vh] overflow-y-auto pr-2">
                  {!statsModal.dataset || !statsModal.dataset.length ? (
                    ...
                    statsModal.dataset.map((item, i) => {
                      ...
                      return (
                        <div key={i} className="pro-card p-4 space-y-2">
              ...
              <div className="mt-8 pt-6 border-t border-mtlCardBorder flex justify-end">
                <button onClick={closeStatsListModal} className="btn-see-more">Close</button>
              </div>
            </div>
          </div>
        )}
        ...
        {fullscreenMatchModal.open && fullscreenMatchModal.match && (
          <div id="fullscreen-match-modal" className="fixed inset-0 bg-mtlDark/95 backdrop-blur-md z-50 overflow-y-auto p-4 md:p-10">
            <div className="max-w-5xl mx-auto bg-mtlSurface border border-mtlCardBorder rounded-2xl p-6 md:p-10 shadow-2xl relative min-h-[85vh] flex flex-col justify-between">
              <button onClick={closeFullscreenMatchModal} className="absolute top-6 right-6 w-10 h-10 rounded-full bg-mtlCard border border-mtlCardBorder text-gray-400 hover:text-white flex items-center justify-center text-lg font-bold">✕</button>
                    ...
                    <>
                      <div className="flex justify-between items-start border-b border-mtlCardBorder pb-6 section-header">
                        <div>
                          ...
                          <h2 
                            className="text-3xl lg:text-4xl font-extrabold text-white mt-2 font-cyber hover:text-mtlGreen cursor-pointer" 
                      ...
                      <div className="bg-mtlDark p-6 rounded-2xl border border-mtlCardBorder space-y-3">
                        <h4 className="font-extrabold text-sm uppercase tracking-wider text-mtlGreen font-cyber">Tactical Analysis Feed</h4>
                        <p className="text-xs text-gray-300 leading-relaxed font-sans">{match.analysis_text || 'No tactical breakdown provided.'}</p>
                      </div>
                    </>
                  );
                })()}
              ...
              <div className="mt-8 pt-6 border-t border-mtlCardBorder flex justify-between items-center">
                <div className="flex items-center gap-3">
                  <button onClick={openMatchChatModalFromFullscreen} className="bg-mtlCard border border-mtlCardBorder text-gray-200 px-4 py-2 rounded-xl text-xs hover:text-mtlGreen flex items-center gap-2">💬 Open Chat</button>
                ...
                <button onClick={closeFullscreenMatchModal} className="btn-see-more">Close</button>
              </div>
            </div>
          </div>
        )}
        ...
        {matchChatModal.open && (
          <div id="match-chat-modal" className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-mtlSurface border border-mtlCardBorder rounded-2xl w-full max-w-2xl h-[80vh] flex flex-col overflow-hidden shadow-2xl">
              <div className="bg-mtlDark p-4 border-b border-mtlCardBorder flex items-center justify-between">
                <div>
                  <h4 className="font-bold text-sm text-mtlGreen font-cyber">Group Chat: {matchChatModal.teams}</h4>
                  <p className="text-[10px] text-gray-400">Match discussion Group</p>
                </div>
                <button onClick={closeMatchChatModal} className="text-gray-400 hover:text-white font-bold text-lg">✕</button>
              </div>
              ...
              <div id="match-chat-messages" className="flex-1 p-4 overflow-y-auto flex flex-col space-y-3 text-xs">
                {(() => {
                  ...
                  if (!msgs.length) return <div className="text-center text-gray-500 text-xs py-8">Start the discussion for this match!</div>;
                  return msgs.map(msg => {
                    const isMe = currentUser && msg.user_id === currentUser.id;
                    const canEdit = userProfile.role === 'admin' || isMe;
                    return (
            ...
            <div className="bg-mtlSurface border border-mtlCardBorder rounded-2xl w-full max-w-2xl p-8 space-y-6 shadow-2xl relative">
              <div className="flex justify-between items-center border-b border-mtlCardBorder pb-4">
                <div>
                  ...
                  <p className="text-xs text-gray-400">Insert or update Record for {adminModal.section}</p>
                </div>
              ...
              <div id="admin-modal-form-fields" className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                {Object.keys(adminFormFields).map((fieldKey) => (
                  <div key={fieldKey}>
                    <label className="block text-xs font-bold uppercase text-gray-400 mb-1">{fieldKey.replace('_', ' ')}</label>
                    <input 
                      type="text" 
            ...
            <div className="bg-mtlSurface border border-mtlCardBorder rounded-2xl w-full max-w-md p-6 space-y-6 shadow-2xl">
              ...
              <div className="space-y-4 text-xs">
                <div>
                  ...
                  <select id="pref-odds-format" value={userProfile.odds_format} onChange={updateProfileSettings} className="w-full bg-mtlDark border border-mtlCardBorder rounded-xl p-2.5 text-white">
                ...
                <div>
                  ...
                  <select id="pref-language" value={userProfile.language} onChange={updateProfileSettings} className="w-full bg-mtlDark border border-mtlCardBorder rounded-xl p-2.5 text-white">
                ...
                <div className="flex items-center justify-between pt-2">
                  ...
                  <input type="checkbox" id="pref-contrast" checked={userProfile.high_contrast} onChange={toggleContrastMode} className="w-4 h-4 accent-mtlGreen" />
                </div>
              </div>
